...
## <a name="installing-the-power-packs-controls-as-a-prerequisite"></a>Power Packs 컨트롤을 필수 구성 요소로 설치  
 ...
 때 [!INCLUDE[vsprvs](~/includes/vsprvs-md.md)] 부트스트래퍼 패키지에 추가 됩니다 Powerpacks 개발 컴퓨터에 설치 된 [!INCLUDE[vsprvs](~/includes/vsprvs-md.md)] 부트스트래퍼 디렉터리입니다. 이 패키지는 [!INCLUDE[ndptecclick](~/includes/ndptecclick-md.md)] 또는 Windows Installer 배포에 대한 필수 조건을 추가하는 절차를 수행할 때 사용할 수 있습니다.  

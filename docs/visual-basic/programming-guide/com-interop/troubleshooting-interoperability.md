...
## <a name="overloaded-methods"></a>오버로드된 메서드  
 Com Visual Basic을 사용한 오버 로드 된 메서드를 만들 수는 있지만 지원 되지 않습니다. 오버 로드 된 메서드를 포함 하는 클래스를 COM 개체로 노출 되는 오버 로드 된 메서드에 대 한 새 메서드 이름이 생성 됩니다.  
...
### <a name="understanding-com-interop-errors"></a>COM interop 오류 이해  
 오류를 처리 하지 interop 호출 종종 적은 양의 정보를 제공 하는 오류를 생성 합니다. 가능 하면 항상 구조적된 오류 발생 시 문제에 대 한 자세한 정보를 제공 하는 처리를 사용 합니다. 이 응용 프로그램을 디버깅 하는 경우에 특히 유용 합니다. 예를 들어:  
...
##  <a name="vbconinteroperabilitymarshalinganchor11"></a> ByRef 컨트롤의 읽기 전용 속성에 전달  
 ...
 호출 되는 프로시저에 대 한 액세스를 사용 하는 경우 사용 하 여이 오류를 방지할 수 있습니다는 `ByVal` 키워드를 허용 하는 매개 변수를 선언 `ReadOnly` 속성입니다. 예를 들어:  

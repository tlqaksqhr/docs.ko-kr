...
`#Region` 지시문을 사용 하면 코드 섹션 축소 및 숨기기 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 파일입니다. `#Region` 지시문을 사용 하면 사용 하는 경우 축소 또는 확장할 수 있는 코드 블록을 지정할 수는 [!INCLUDE[vsprvs](~/includes/vsprvs-md.md)] 코드 편집기. 선택적으로 코드를 숨기는 기능은 좀 더 관리 및 보다 쉽게 읽을 수 파일을 만듭니다. 자세한 내용은 [개요](/visualstudio/ide/outlining)를 참조하세요.  

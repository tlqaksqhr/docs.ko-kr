...
## 예제  
 ...
 오버로드된 버전의 코드에서는 호출 코드에서 `ParamArray` 매개 변수에 하나 이상의 값을 제공했는지 여부나 제공했다면 몇 개를 제공했는지를 테스트하지 않아도 됩니다.  [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)]에서는 호출 인수 목록과 일치하는 버전으로 제어가 전달됩니다.  

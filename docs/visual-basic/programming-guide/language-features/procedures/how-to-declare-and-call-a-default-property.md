...
*기본 속성*은 지정하지 않은 상태에서 코드에서 액세스할 수 있는 클래스 또는 구조체 속성입니다.  호출 코드에서 클래스나 구조체의 이름을 지정하지만 속성의 이름이 지정되지 않았으며 컨텍스트에서 속성에 대한 액세스가 허용할 경우 [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)]은 해당 클래스나 구조체의 기본 속성\(있을 경우\)에 대한 액세스를 확인합니다.  

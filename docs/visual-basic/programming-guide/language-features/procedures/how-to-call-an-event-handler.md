...
4.  이벤트가 발생할 때 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 자동으로 호출 된 `Sub` 프로시저입니다. 코드에서 사용할 수는 `RaiseEvent` 문을 이벤트를 발생 합니다.  
...
3.  이벤트가 발생할 때 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 자동으로 호출 된 `Sub` 프로시저입니다. 코드에서 사용할 수는 `RaiseEvent` 문을 이벤트를 발생 합니다.  

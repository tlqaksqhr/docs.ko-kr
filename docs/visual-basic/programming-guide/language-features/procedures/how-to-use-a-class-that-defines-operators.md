...
연산자를 자체적으로 정의하는 클래스나 구조체를 사용하는 경우 [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)]에서는 해당 연산자에 액세스할 수 있습니다.  
  
 클래스 또는 구조체에서 연산자를 정의하는 것을 연산자 *오버로딩*이라고도 합니다.  
  
## 예제  
 다음 예제에서는 SQL 문자열과 [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)] 문자열 사이의 양방향 변환 연산자\([CType 함수](../../../../visual-basic/language-reference/functions/ctype-function.md)\)를 정의하는 SQL 구조체 <xref:System.Data.SqlTypes.SqlString>에 액세스합니다.  `CType(`*SQL string expression*, `String)`을 사용하여 SQL 문자열을 [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)] 문자열로 변환하고 `CType(`*Visual Basic string expression*, <xref:System.Data.SqlTypes.SqlString>`)`을 사용하여 반대 방향으로 변환합니다.  

...
`Sub` 프로시저는 `Sub` 문과 `End Sub` 문 사이에 포함된 일련의 [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)] 문입니다.  `Sub` 프로시저는 작업을 수행한 다음 호출 코드로 제어를 반환하지만 호출 코드에 값을 반환하지는 않습니다.  

...
값을 속성에 저장하거나 해당 값을 검색하여 속성 프로시저를 호출합니다.  변수에 액세스할 때와 같은 방법으로 속성에 액세스할 수 있습니다.  
 ...
 속성의 `Set` 프로시저는 값을 저장하며 속성의 `Get` 프로시저는 값을 검색합니다.  그러나 사용자가 이러한 프로시저 이름을 명시적으로 호출하지는 않습니다.  변수의 값을 저장하거나 검색할 때와 마찬가지로 대입문이나 식에서 속성을 사용합니다.  [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)]에서 속성의 프로시저가 호출됩니다.  

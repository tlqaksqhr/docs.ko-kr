...
이 연습에서는 `Widget`이라는 클래스에 대한 이벤트를 선언하고 발생시키는 방법을 설명합니다.  이 연습 단계를 완료한 다음 `Widget` 개체의 이벤트를 사용하여 응용 프로그램의 상태 정보를 제공하는 방법을 설명하는 [Walkthrough: Handling Events](../Topic/Walkthrough:%20Handling%20Events%20\(Visual%20Basic\).md) 항목을 참고할 수 있습니다.  
...
1.  [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)] Windows 응용 프로그램 프로젝트를 열고 `Form1`이라는 폼을 만듭니다.  
...
2.  `Widget` 클래스에 다음 코드를 추가합니다.  
 ...
 이제 `Widget` 클래스로 이벤트를 발생시킬 수 있으므로 다음 연습을 수행해 봅니다.  [Walkthrough: Handling Events](../Topic/Walkthrough:%20Handling%20Events%20\(Visual%20Basic\).md)에서는 `WithEvents`를 사용하여 이벤트 처리기를 `PercentDone` 이벤트와 연결하는 방법을 보여 줍니다.  

...
Visual Basic 런타임 및 [!INCLUDE[dnprdnshort](~/includes/dnprdnshort-md.md)] 평가 `Nothing` 문자열에 나올 때와 다르게 합니다.  
...
## <a name="visual-basic-runtime-and-the-net-framework"></a>Visual Basic 런타임 및.NET Framework  
 ...
 Visual Basic 런타임 일반적으로 평가 `Nothing` 빈 문자열 (""). 그러나 [!INCLUDE[dnprdnshort](~/includes/dnprdnshort-md.md)] 하지 않는 고에 문자열 작업을 수행 하려고 시도 될 때마다 예외를 throw `Nothing`합니다.  

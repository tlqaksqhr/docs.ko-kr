...
분석 및 나 문자열을 조작 하는 여러 가지가 있습니다. 에 포함 된 일부 메서드는 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 언어 및 다른 사용자에 내재 된는 `String` 클래스입니다.  
  
## <a name="visual-basic-language-and-the-net-framework"></a>Visual Basic 언어와.NET Framework  
 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]메서드는 언어의 기본 기능으로 사용 됩니다. 코드에서 자격 증명 없이 사용할 수 있습니다. 다음 예제에서는의 일반적인 용도 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 문자열 조작 명령:  
...
#### <a name="instance-methods"></a>인스턴스 메서드  
 인스턴스 메서드와 달리,의 특정 인스턴스에서 형태소 분석 `String` 및 인스턴스 이름으로 한정 되어야 합니다. 예:  

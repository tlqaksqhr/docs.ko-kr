...
### <a name="retrieving-values"></a>값 검색  
 Nullable 형식의 변수 값을 검색 하려면 먼저을 테스트 해야 해당 <xref:System.Nullable%601.HasValue%2A> 속성을 값이 있는지 확인 합니다. 값을 읽으려고 하면 때 <xref:System.Nullable%601.HasValue%2A> 은 `False`, [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] throw 한 <xref:System.InvalidOperationException> 예외입니다. 다음 예에서는 변수를 읽은 하는 권장된 방법은 `numberOfChildren` 앞의 예제입니다.  

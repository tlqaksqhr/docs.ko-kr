...
## <a name="remarks"></a>설명  
 `/nostdlib` 옵션 System.dll 어셈블리에 대 한 자동 참조를 제거 하 고 컴파일러가 Vbc.rsp 파일을 읽는 하지 않도록 합니다. Vbc.rsp 파일-Vbc.exe 파일과 동일한 디렉터리에 있는-자주 사용 되는 참조 [!INCLUDE[dnprdnshort](~/includes/dnprdnshort-md.md)] 어셈블리 및 가져오기는 `System` 및 `Microsoft.VisualBasic` 네임 스페이스입니다.  

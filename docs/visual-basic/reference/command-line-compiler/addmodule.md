...
## <a name="arguments"></a>인수  
 `fileList`  
 필수 요소. 쉼표로 구분 된 목록 메타 데이터를 포함 하지만 어셈블리 매니페스트를 포함 하지 않는 파일입니다. 공백이 포함 된 파일 이름은 따옴표로 묶어야 ("").  
...
## <a name="remarks"></a>설명  
 ...
 (암시적 또는 명시적으로)을 지정 하는 경우 모든[/target (Visual Basic)](../../../visual-basic/reference/command-line-compiler/target.md) 이외의 옵션 `/target:module` 와 `/addmodule`에 전달할 파일 `/addmodule` 프로젝트의 어셈블리의 일부가 됩니다. 어셈블리에 있는 출력 파일을 실행 해야 합니다. 더 많은 파일 추가 또는 `/addmodule`합니다.  

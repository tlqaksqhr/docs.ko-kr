...
'\<`eventname`\>' 이벤트에 대한 정의를 포함하는 '\<`assemblyname`\>' 어셈블리에 대한 참조가 필요합니다. 참조를 프로젝트에 추가합니다.  
 ...
 이벤트는 프로젝트에서 직접 참조하지 않는 어셈블리 또는 DLL\(동적 연결 라이브러리\)에서 정의됩니다. 둘 이상이 DLL 또는 어셈블리에서 이벤트가 정의된 경우 모호성을 방지하기 위해 [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)] 컴파일러에 참조가 필요합니다.  

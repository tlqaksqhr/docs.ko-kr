...
```  
 ...
 [!INCLUDE[vb_orcas_long](../../visual-basic/misc/includes/vb_orcas_long_md.md)]에서 시작하여 무명 형식의 인스턴스가 될 개체를 선언할 수 있으며, 이 경우 데이터 형식을 지정하지 않습니다. 무명 형식 선언에서 `With` 키워드는 `New` 뒤에 나옵니다.  

...
클래스 또는 인터페이스가 `<CLSCompliant(True)>` 로 표시되거나 또는 표시되지 않는 형식에 중첩되면 `<CLSCompliant(False)>` 로 표시됩니다.  
 ...
 클래스 또는 인터페이스를 준수 해야 합니다는 [언어 독립성 및 언어 독립적 구성 요소](../../../docs/standard/language-independence-and-language-independent-components.md) (CLS)는 전체 포함 계층 구조를 따라야 합니다. 즉, 중첩된 모든 형식이 규격을 준수해야 함을 의미합니다.  

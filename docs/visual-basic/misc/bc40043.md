...
속성 정의는 <xref:System.CLSCompliantAttribute> 특성을 해당 `Get` 또는 `Set` 문에 적용합니다.  
 ...
 과 호환 되도록 속성에 대 한는 [언어 독립성 및 언어 독립적 구성 요소](../../../docs/standard/language-independence-and-language-independent-components.md) (CLS)는 전체 속성으로 표시 되어야 합니다 `<CLSCompliant(True)>`합니다. 적용 해야 <xref:System.CLSCompliantAttribute> 에 [Property 문](../../visual-basic/language-reference/statements/property-statement.md)아닌는 `Get` 또는 `Set` 문.  

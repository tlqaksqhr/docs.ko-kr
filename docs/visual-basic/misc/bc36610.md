...
1.  참조하는 문에서 이름의 철자를 검사합니다.[!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)]는 대\/소문자를 구분하지 않지만 철자를 변형하면 다른 이름이 구성됩니다. 밑줄\(`_`\)은 이름의 일부이므로 철자의 일부입니다.  

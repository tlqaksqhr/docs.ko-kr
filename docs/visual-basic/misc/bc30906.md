...
파생 클래스의 프로시저가 기본 클래스에서 이름이 같은 프로시저를 재정의하지만 매개 변수 목록이 서로 다릅니다.  
 ...
 상속된 클래스의 프로시저를 재정의하려면 재정의 프로시저의 매개 변수 목록, 액세스 수준 및 반환 형식\(있는 경우\)이 일치해야 합니다. 특히, [Optional](../../visual-basic/language-reference/modifiers/optional.md) 또는 [ParamArray](../../visual-basic/language-reference/modifiers/paramarray.md) 선언과 일치해야 합니다.  

...
[!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)]에서 컴파일과 연결에 필요한 표준 DLL 라이브러리 중 하나를 찾거나 열 수 없습니다.  
...
1.  오류 메시지에 언급된 파일이 [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)]을 실행 중인 하드 디스크에 있는지 확인합니다. 일반적으로 표준 라이브러리는 \\WINNT\\Microsoft.NET\\Framework 또는 \\WINDOWS\\Microsoft.NET\\Framework의 하위 디렉터리에 있어야 합니다.  
  
2.  파일 또는 디렉터리에 [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)]의 읽기 권한을 금지하는 설정이나 특성이 없는지 확인합니다.  
  
3.  파일 이름과 확장명의 철자가 맞는지 확인합니다. 대\/소문자를 구분하지 않습니다.  
  
4.  파일이 제대로 배치되고 액세스할 수 있는 것처럼 보이면 디스크에서 손상되었을 수 있습니다. 가능한 경우 [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)]을 다시 설치합니다.  

...
6.  이미 스택에 있는 이벤트 프로시저를 호출하는 이벤트를 트리거하여 "이벤트 캐스케이딩"이 발생하지 않도록 합니다.  이벤트 캐스케이딩은 끝나지 않은 재귀 프로시저 호출과 비슷하지만 코드에서 명시적으로 호출되지 않고 [!INCLUDE[vbprvb](../../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)]에서 호출하므로 비교적 명확하지 않습니다.  `Calls`대화 상자를 사용하여 스택에 활성화되어 있는 프로시저를 확인합니다.  

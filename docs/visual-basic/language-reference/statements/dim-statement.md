...
|예|예|`Dim qty  As Integer = 5`|이니셜라이저의 데이터 형식을 지정한 데이터 형식으로 변환할 수 없으면 컴파일 시간 오류가 발생합니다.|  
 ...
 데이터 형식을 지정 하지만 이니셜라이저를 지정 하지 않는 경우 [!INCLUDE[vbprvb](../../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] 변수 데이터 형식에 대 한 기본 값으로 초기화 합니다. 다음 표에서 기본 초기화 값을 보여 줍니다.  

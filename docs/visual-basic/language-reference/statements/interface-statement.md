 ...
 *인터페이스*는 클래스 및 구조체에서 구현할 수 있는 속성, 프로시저 등과 같은 멤버 집합을 정의합니다.  인터페이스는 멤버의 내부 작업이 아니라 멤버의 시그니처만 정의합니다.  
 ...
 클래스나 구조체는 인터페이스에 정의된 모든 멤버에 대해 코드를 제공하여 인터페이스를 구현합니다.  응용 프로그램에서 해당 클래스나 구조체로부터 인스턴스를 만들 경우 개체가 메모리에서 실행됩니다.  자세한 내용은 [Objects and Classes](../../../visual-basic/programming-guide/language-features/objects-and-classes/index.md) 및 [Interfaces](../../../visual-basic/reference/command-line-compiler/index.md)을 참조하십시오.  

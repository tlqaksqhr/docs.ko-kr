...
## <a name="remarks"></a>설명  
 `Delegate` 문에서 대리자 클래스의 매개 변수 및 반환 형식을 정의 합니다. 매개 변수 및 반환 형식을 일치 하는 프로시저는이 대리자 클래스의 인스턴스를 만드는 데 사용할 수 있습니다. 프로시저 수 나중에 호출 될 대리자 인스턴스를 통해 대리자의 호출 하 여 `Invoke` 메서드.  
...
## <a name="example"></a>예제  
 다음 예제에서는 `Delegate` 두 개의 숫자에서 작동 하 고 숫자를 반환 하는 것에 대 한 대리자를 선언 하는 문입니다. `DelegateTest` 메서드가 형식의 대리자의 인스턴스를 사용 하 여 숫자의 쌍입니다.  

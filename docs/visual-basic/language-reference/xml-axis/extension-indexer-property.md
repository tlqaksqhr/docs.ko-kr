...
## 설명  
 ...
 [!INCLUDE[vbprvb](../../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] 컴파일러가 확장명 인덱서 속성을 `ElementAtOrDefault` 메서드에 대한 호출로 변환합니다. 배열 인덱서와 달리, 인덱스의 범위가 초과될 경우 `ElementAtOrDefault` 메서드는 `Nothing`을 반환합니다.  이러한 동작은 컬렉션의 요소 수를 확인하는 데 매우 유용합니다.  

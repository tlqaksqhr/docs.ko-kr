...
|용어|정의|  
|---|---|  
|`argument1`|필수 요소. `Boolean`. 이외의 인수는 평가 하 고 반환을 결정 합니다.|  
|`argument2`|필수 요소. `Object`. 계산 되 고 반환 if `argument1` 계산 `True`합니다.|  
|`argument3`|필수 요소. `Object`. 계산 되 고 반환 하는 경우 `argument1` 계산 `False` 경우 `argument1` 는 [Nullable](../../../visual-basic/programming-guide/language-features/data-types/nullable-value-types.md) `Boolean` 계산 되는 변수 [Nothing](../../../visual-basic/language-reference/nothing.md)합니다.|  
...
|용어|정의|  
|---|---|  
|`argument2`|필수 요소. `Object`. 참조 또는 nullable 형식 이어야 합니다. 계산 되 고 아닌 다른 값으로 계산 될 때 반환 `Nothing`합니다.|  
|`argument3`|필수 요소. `Object`. 계산 되 고 반환 if `argument2` 계산 `Nothing`합니다.|  

...
-   [이중](../../../visual-basic/language-reference/data-types/double-data-type.md) -최대 범위 1.7-E + 308 (절대 값)  
 ...
 Visual Basic 데이터 형식에 대 한 자세한 내용은 참조 하십시오. [데이터 형식](../../../visual-basic/language-reference/data-types/data-type-summary.md)합니다.  
  
 피연산자를 [Nothing](../../../visual-basic/language-reference/nothing.md), Visual Basic의 산술 연산자 0으로 처리 합니다.  
...
## <a name="decimal-arithmetic"></a>10 진수 산술 연산  
 ...
 하는 경우의 피연산자는 `+`, `–`, `*`, `/`, 또는 `Mod` 작업이 `Decimal` 하나도 및 `Single` 또는 `Double`, Visual Basic 확대 다른피연산자`Decimal`. 동작을 수행 `Decimal`, 고 결과 데이터 형식이 `Decimal`합니다.  
...
|정수 계열 형식|Decimal|Single|Double|Double|  
 ...
 `^` 연산자에 대해서만 정의 되는 `Double` 데이터 형식입니다. Visual Basic에는 필요에 따라 각 피연산자 확장 `Double` 작업과 데이터 형식이 항상 결과 하기 전에 `Double`합니다.  
  
## <a name="integer-arithmetic"></a>정수 연산  
 정수 연산 결과 데이터 형식 피연산자의 데이터 형식에 따라 달라 집니다. 일반적으로 Visual Basic에서는 결과 데이터 형식을 결정 하기 위해 다음 정책을 사용:  
...
### <a name="-and--operators"></a><\< 및 >> 연산자  
 다음 표에서 결과 데이터 형식을 보여 줍니다 두 비트 시프트 연산자에 대 한 `<<` 및 `>>`합니다. Visual Basic의 각 비트 시프트 연산자의 왼쪽된 피연산자 (이동 비트 패턴)에 단항 연산자 구조체로 처리 합니다.  
...
|`<<`, `>>`|Short|SByte|Byte|Short|UShort|정수|UInteger|Long|ULong|  
 ...
 왼쪽된 피연산자가 `Decimal`, `Single`, `Double`, 또는 `String`, Visual Basic로 변환 하려고 `Long` 작업과 데이터 형식이 결과 하기 전에 `Long`합니다. 오른쪽 피연산자 (이동할 비트 위치 수) `Integer` 또는로 확대 되는 형식의 `Integer`합니다.  
...
## <a name="relational-and-bitwise-comparisons"></a>관계 및 비트 비교  
 ...
 비트 논리 작업의 결과 데이터 형식 피연산자의 데이터 형식에 따라 달라 집니다. `AndAlso` 및 `OrElse` 에 대해서만 정의 `Boolean`, Visual Basic로 필요에 따라 각 피연산자를 변환 하 고 `Boolean` 작업을 수행 하기 전에.  
  
### <a name="-----and--operators"></a>=, <>, \<, >, \<= 및 > = 연산자  
 피연산자가 모두 경우 `Boolean`, Visual Basic은 간주 `True` 되도록 미만 `False`합니다. 숫자 형식을와 비교 하는 경우는 `String`, Visual Basic로 변환 하려고는 `String` 를 `Double` 작업 전에 합니다. A `Char` 또는 `Date` 피연산자는 동일한 데이터 형식이 다른 피연산자와만 비교할 수 있습니다. 결과 데이터 형식이 항상 `Boolean`합니다.  
...
|`Not`|부울|SByte|Byte|Short|UShort|정수|UInteger|Long|ULong|  
 ...
 피연산자가 `Decimal`, `Single`, `Double`, 또는 `String`, Visual Basic로 변환 하려고 `Long` 작업과 데이터 형식이 결과 하기 전에 `Long`합니다.  
...
|`ULong`|Long|Long|ULong|Long|ULong|Long|ULong|Long|ULong|  
 ...
 한 피연산자가 `Decimal`, `Single`, `Double`, 또는 `String`, Visual Basic로 변환 하려고 `Long` 하기 전에 작업을 하 고 결과 데이터 형식이 동일 해당 피연산자가 이미 마치 `Long`합니다.  
  
## <a name="miscellaneous-operators"></a>기타 연산자  
 `&` 의 연결에 대 한 연산자가 정의 `String` 피연산자. Visual Basic로 필요에 따라 각 피연산자를 변환 `String` 작업과 데이터 형식이 항상 결과 하기 전에 `String`합니다. 목적에 대 한는 `&` 연산자, 모든 변환을 `String` 확장 될 것으로 간주 됩니다 경우에 `Option Strict` 은 `On`합니다.  
 ...
 `Like` 의 패턴 일치에 대해서만 연산자가 정의 `String` 피연산자. Visual Basic에서 필요에 따라 각 피연산자를 변환 하려고 `String` 작업 전에 합니다. 결과 데이터 형식이 항상 `Boolean`합니다.  

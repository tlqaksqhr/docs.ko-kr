...
## <a name="example"></a>예제  
 이 예제에서는 이전 예제를 변경합니다. `AppendString` 메서드는 소스를 반복하기 전에 <xref:System.Linq.Enumerable.ToList%2A>를 호출합니다. 이로 인해 구체화가 수행됩니다.  
...
```  
 ...
 이 예제에서 <xref:System.Linq.Enumerable.ToList%2A>를 호출하면 `AppendString`이 첫 번째 항목을 생성하기 전에 전체 소스를 열거하는 것을 확인할 수 있습니다. 소스가 큰 배열인 경우에는 응용 프로그램의 메모리 프로필이 크게 변경됩니다.  

...
<xref:Microsoft.CodeAnalysis.Compilation>의 인스턴스는 컴파일러에서 보는 단일 프로젝트와 유사하고, Visual Basic 또는 C# 프로그램을 컴파일하는 데 필요한 모든 항목을 나타냅니다. **컴파일**에는 컴파일할 원본 파일, 어셈블리 참조, 컴파일러 옵션의 집합이 포함됩니다. 이 컨텍스트에서 다른 모든 정보를 사용하여 코드의 의미에 대해 추정할 수 있습니다. <xref:Microsoft.CodeAnalysis.Compilation>을 사용하면 이름 및 다른 식이 참조하는 형식, 네임스페이스, 멤버 및 변수 등의 엔터티인 **기호**를 찾을 수 있습니다. **기호**를 사용하여 이름 및 식을 연결하는 프로세스를 **바인딩**이라고 합니다.
...
[GitHub 샘플 리포지토리](https://github.com/dotnet/samples/csharp/roslyn-sdk/SemanticQuickStart)에서 이 샘플의 완성된 코드를 볼 수 있습니다.
...
<xref:Microsoft.CodeAnalysis.CSharp.CSharpCompilation.AddReferences%2A?displayProperty=nameWithType> 메서드는 컴파일에 참조를 추가합니다. <xref:Microsoft.CodeAnalysis.MetadataReference.CreateFromFile(System.String,Microsoft.CodeAnalysis.MetadataReferenceProperties,Microsoft.CodeAnalysis.DocumentationProvider)> 메서드는 어셈블리를 참조로 로드합니다. 

## <a name="querying-the-semantic-model"></a>의미 체계 모델 쿼리

<xref:Microsoft.CodeAnalysis.Compilation>이 있다면 <xref:Microsoft.CodeAnalysis.SemanticModel>에 대해 해당 <xref:Microsoft.CodeAnalysis.Compilation>에 포함된 <xref:Microsoft.CodeAnalysis.SyntaxTree>를 요청할 수 있습니다. 일반적으로 모든 정보의 원본을 IntelliSense에서 가져오는 경우 의미 체계 모델을 고려할 수 있습니다. <xref:Microsoft.CodeAnalysis.SemanticModel>은 "이 위치에서 범위에 있는 이름은 무엇인가요?"라는 질문에 대답할 수 있습니다. "이 메서드에서 액세스할 수 있는 멤버는 무엇인가요?" "이 텍스트 블록에 사용되는 변수는 무엇인가요?" 및 "이 이름/식이 참조하는 항목은 무엇인가요?" 의미 체계 모델을 만드는 이 문을 추가합니다.
...
위의 코드에서는 <xref:Microsoft.CodeAnalysis.SemanticModel> HelloWorld에 대한 <xref:Microsoft.CodeAnalysis.SyntaxTree> 개체를 가져오는 방법을 보여줍니다. 모델을 가져오면 첫 번째 `using` 지시문의 이름을 바인딩하여 `System` 네임스페이스에서 <xref:Microsoft.CodeAnalysis.SymbolInfo?displayProperty=nameWithType>을 검색합니다. 또한 위의 코드에서는 **구문 모델**을 사용하여 코드의 구조를 찾는 것을 설명합니다. **의미 체계 모델**을 사용하여 해당 의미를 이해합니다. **구문 모델**은 using 문에서 `System` 문자열을 찾습니다. **의미 체계 모델**에는 `System` 네임스페이스에서 정의된 형식에 대한 모든 정보가 있습니다.

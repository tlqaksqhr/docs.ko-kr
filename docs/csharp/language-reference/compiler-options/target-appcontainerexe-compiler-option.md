...
**\/target:appcontainerexe** 컴파일러 옵션을 사용하는 경우 컴파일러에서 응용 프로그램 컨테이너에서 실행해야 하는 Windows 실행 파일\(.exe\)을 만듭니다.  이 옵션은 [\/target:winexe](../../../csharp/language-reference/compiler-options/target-winexe-compiler-option.md)와 같지만 [!INCLUDE[win8_appname_long](../../../csharp/includes/win8_appname_long_md.md)] 응용 프로그램을 위해 설계되었습니다.  

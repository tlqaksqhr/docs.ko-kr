...
-   네임스페이스 이름에 대한 오류인 경우 네임스페이스를 포함하는 어셈블리에 대한 참조\([\/reference](../../../csharp/language-reference/compiler-options/reference-compiler-option.md)\)를 추가했나요? 예를 들어 코드에 `using Accessibility` 지시문이 포함되어 있을 수 있습니다. 그러나 프로젝트에서 Accessibility.dll 어셈블리를 참조하지 않을 경우 CS0246 오류가 보고됩니다. 자세한 내용은 [프로젝트의 참조 관리](/visual-studio/ide/managing-references-in-a-project)을 참조하세요.  
  
-   형식 이름에 대한 오류인 경우 적절한 [using](../../../csharp/language-reference/keywords/using.md) 지시문을 포함했거나 형식 이름을 정규화했나요? 다음 선언을 살펴보세요. `DataSet ds`.`DataSet` 형식을 사용하려면 두 가지가 필요합니다. 첫째, `DataSet` 형식의 정의가 포함된 어셈블리에 대한 참조가 필요합니다. 둘째, `DataSet`가 있는 네임스페이스에 대한 `using` 지시문이 필요합니다. 예를 들어 `DataSet`가 **System.Data** 네임스페이스에 있기 때문에 코드의 시작 부분에 다음 지시문이 있어야 합니다. `using System.Data`.  
  
     `using` 지시문은 필요하지 않습니다. 그러나 지시문을 생략할 경우 참조할 때 `DataSet` 형식을 정규화해야 합니다. 정규화는 코드에서 형식을 참조할 때마다 네임스페이스와 형식을 둘 다 지정하는 것을 의미합니다. 이전 예제에서 `using` 지시문을 생략할 경우 `DataSet ds` 대신 `ds`를 선언하도록 `System.Data.DataSet ds`를 작성해야 합니다.  
  
-   형식이 필요한 위치에 변수 또는 다른 언어 요소를 사용했나요? 예를 들어 **is** 문에서 실제 형식 대신 `Type` 개체를 사용하는 경우 CS0246 오류가 발생합니다.  
  
-   형식 이름을 정규화하지 않고 *using 별칭 지시문*을 사용했나요?`using` 별칭 지시문은 형식을 확인하기 위해 소스 코드 파일의 `using` 지시문을 사용하지 않습니다. 다음 예제에서는 `List<int>` 형식이 정규화되지 않았으므로 CS0246을 생성합니다.`System.Collections.Generic`에 대한 `using` 지시문으로 오류를 방지할 수 없습니다.  

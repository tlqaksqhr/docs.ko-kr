...
ICorDebugFrame 인스턴스, 현재 위치부터 지정한 수를 가져옵니다.  
...
#### <a name="parameters"></a>매개 변수  
 `celt`  
 [in] 수가 `ICorDebugFrame` 인스턴스를 검색할 수 있습니다.  
  
 `frames`  
 [out] 각각 가리키는 포인터의 배열은 `ICorDebugFrame` 개체입니다.  
  
 `pceltFetched`  
 [out] 수에 대 한 포인터 `ICorDebugFrame` 실제로 반환 된 인스턴스. 이 값은 null 일 수 있으면 `celt` 하나입니다.  

...
`pCtx`  
[in] 이 값은 일반적으로 `null`입니다. 그렇지 않을 경우에 대 한 포인터를 [IWbemContext](/windows/desktop/api/wbemcli/nn-wbemcli-iwbemcontext) 요청된 인스턴스를 제공 하는 공급자가 사용할 수 있는 인스턴스.
...
이 함수에 대 한 호출을 래핑하는 [IWbemServices::CreateClassEnum](/windows/desktop/api/wbemcli/nf-wbemcli-iwbemservices-createinstanceenum) 메서드.

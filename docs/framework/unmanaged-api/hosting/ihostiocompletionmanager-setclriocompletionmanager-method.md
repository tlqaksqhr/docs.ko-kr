...
호스트에 대 한 인터페이스 포인터에 제공 된 [ICLRIoCompletionManager](../../../../docs/framework/unmanaged-api/hosting/iclriocompletionmanager-interface.md) 공용 언어 런타임 (CLR)에 의해 구현 되는 인스턴스.  
...
#### <a name="parameters"></a>매개 변수  
 `pManager`  
 [in] 에 대 한 인터페이스 포인터는 `ICLRIoCompletionManager` 인스턴스 CLR에서 제공 합니다.  
...
## <a name="remarks"></a>설명  
 CLR가 호출 후 `SetCLRIoCompletionManager`, 호스트에서는 호출 [iclriocompletionmanager:: Oncomplete](../../../../docs/framework/unmanaged-api/hosting/iclriocompletionmanager-oncomplete-method.md) I/O 요청이 완료 될 때 CLR에 알리기 위해 합니다.  

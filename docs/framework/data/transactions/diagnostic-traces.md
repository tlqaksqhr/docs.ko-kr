...
## 추적 사용  
 트랜잭션 처리 중에 추적을 사용하려면 응용 프로그램의 구성 파일을 편집해야 합니다.예를 들면 다음과 같습니다.  
...
|추적 수준|설명|  
|-----------|--------|  
|위험|다음과 같은 심각한 오류가 발생했습니다.<br /><br /> -   사용자 기능이 즉시 손실될 수 있는 오류<br />-   관리자가 기능 손실을 방지하기 위해 조치를 취해야 하는 이벤트<br />-   코드 중단<br />-   이 추적 수준에서는 다른 중요한 추적을 해석하는 데 충분한 컨텍스트도 제공합니다.이를 통해 심각한 오류를 발생시키는 작업 시퀀스를 식별할 수 있습니다.|  
...
## 추적 코드  
 다음 표에서는 <xref:System.Transactions> 인프라에서 생성되는 추적 코드를 보여 줍니다.표에는 추적 코드 식별자, 추적에 대한 <xref:System.Diagnostics.EventTypeFilter.EventType%2A> 열거 수준, 추적의 **TraceRecord**에 포함된 추가 데이터가 들어 있습니다.또한 추적의 해당 추적 수준도 **TraceRecord**에 저장됩니다.  
...
|TransactionScopeCreated|Info|TransactionScopeResult. 다음 중 하나일 수 있습니다.<br /><br /> -   새 트랜잭션<br />-   트랜잭션 성공<br />-   종속 트랜잭션 성공<br />-   현재 트랜잭션 사용<br />-   트랜잭션 없음<br /><br /> 새 현재 TransactionTraceId|  
|TransactionScopeDisposed|Info|범위의 "예상" 현재 트랜잭션의 TransactionTraceId|  
|TransactionScopeIncomplete|경고|범위의 "예상" 현재 트랜잭션의 TransactionTraceId|  
|TransactionScopeNestedIncorrectly|경고|범위의 "예상" 현재 트랜잭션의 TransactionTraceId|  
|TransactionScopeCurrentTransactionChanged|경고|이전의 현재 TransactionTraceId, 기타 TransactionTraceId|  
|TransactionScopeTimeout|경고|범위의 "예상" 현재 트랜잭션의 TransactionTraceId|  

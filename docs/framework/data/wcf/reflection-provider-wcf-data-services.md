---
title: "리플렉션 공급자(WCF Data Services) | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
> [!NOTE]
>  <xref:System.Collections.Generic.IEnumerable%601> 인터페이스를 구현하는 모든 클래스에서 <xref:System.Linq.Queryable.AsQueryable%2A> 메서드를 사용하여 <xref:System.Linq.IQueryable%601> 인터페이스를 반환할 수 있습니다.  이렇게 하면 가장 일반적인 컬렉션 형식을 데이터 서비스의 데이터 소스로 사용할 수 있습니다.  
  
 리플렉션 공급자는 형식 계층 구조를 지원합니다.  자세한 내용은 [방법: 리플렉션 공급자를 사용하여 데이터 서비스 만들기](../../../../docs/framework/data/wcf/create-a-data-service-using-rp-wcf-data-services.md)을 참조하세요.  
...
-   엔터티 형식 \- 엔터티 집합이 반환하는 <xref:System.Linq.IQueryable%601>의 `T` 형식입니다.  상속 계층 구조에 속하는 클래스는 리플렉션 공급자에 의해 동등한 엔터티 형식 계층 구조로 변환됩니다.  
...
## 데이터 모델에서 업데이트 사용  
 이러한 종류의 데이터 모델을 통해 노출되는 데이터를 업데이트하려면 리플렉션 공급자는 <xref:System.Data.Services.IUpdatable> 인터페이스를 정의합니다.  이 인터페이스는 노출된 형식의 업데이트를 저장하는 방법을 데이터 서비스에 지시합니다.  데이터 모델에 의해 정의된 리소스를 업데이트하려면 엔터티 컨테이너 클래스에서 <xref:System.Data.Services.IUpdatable> 인터페이스를 구현해야 합니다.  <xref:System.Data.Services.IUpdatable> 인터페이스의 구현 예제는 [방법: LINQ to SQL 데이터 소스를 사용하여 데이터 서비스 만들기](../../../../docs/framework/data/wcf/create-a-data-service-using-linq-to-sql-wcf.md)를 참조하세요.  
...
## 리플렉션 공급자와 함께 LINQ to SQL 사용  
 Entity Framework는 기본적으로 지원되므로 [!INCLUDE[ssAstoria](../../../../includes/ssastoria-md.md)]에 관계형 데이터를 사용하는 경우 이 데이터 공급자를 사용하는 것이 좋습니다.  그러나 리플렉션 공급자를 통해 데이터 서비스에 LINQ to SQL 클래스를 사용할 수 있습니다.  LINQ to SQL O\/R Designer\(Object Relational Designer\)가 생성한 <xref:System.Data.Linq.DataContext>의 메서드에서 반환된 <xref:System.Data.Linq.Table%601> 결과 집합은 <xref:System.Linq.IQueryable%601> 인터페이스를 구현합니다.  이렇게 하면 리플렉션 공급자가 이러한 메서드에 액세스하고 생성된 LINQ to SQL 클래스를 사용하여 SQL Server에서 엔터티 데이터를 반환할 수 있습니다.  그러나 LINQ to SQL은 <xref:System.Data.Services.IUpdatable> 인터페이스를 구현하지 않으므로 기존 <xref:System.Data.Linq.DataContext> partial 클래스를 확장하여 <xref:System.Data.Services.IUpdatable> 구현을 추가하는 partial 클래스를 추가해야 합니다.  자세한 내용은 [방법: LINQ to SQL 데이터 소스를 사용하여 데이터 서비스 만들기](../../../../docs/framework/data/wcf/create-a-data-service-using-linq-to-sql-wcf.md)을 참조하세요.  

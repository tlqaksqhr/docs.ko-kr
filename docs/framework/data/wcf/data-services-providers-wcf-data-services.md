---
title: "데이터 서비스 공급자(WCF Data Services) | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
[!INCLUDE[ssAstoria](../../../../includes/ssastoria-md.md)]에서는 데이터를 [!INCLUDE[ssODataFull](../../../../includes/ssodatafull-md.md)] 피드로 노출하기 위한 여러 공급자 모델을 지원합니다.  이 항목에서 제공하는 정보를 통해 데이터 소스에 가장 적합한 [!INCLUDE[ssAstoria](../../../../includes/ssastoria-md.md)] 공급자를 선택할 수 있습니다.  
  
## 데이터 소스 공급자  
 [!INCLUDE[ssAstoria](../../../../includes/ssastoria-md.md)]에서는 데이터 서비스의 데이터 모델을 정의하기 위해 다음 공급자를 지원합니다.  
...
|사용자 지정 데이터 서비스 공급자|[!INCLUDE[ssAstoria](../../../../includes/ssastoria-md.md)]에 포함된 공급자 집합을 사용하면 런타임에 바인딩된 데이터 형식을 기반으로 데이터 모델을 동적으로 정의할 수 있습니다.  응용 프로그램을 디자인할 때 노출되는 데이터를 알 수 없거나 Entity Framework 또는 리플렉션 공급자가 충분하지 않은 경우 이러한 인터페이스를 구현해야 합니다.  자세한 내용은 [사용자 지정 데이터 서비스 공급자](../../../../docs/framework/data/wcf/custom-data-service-providers-wcf-data-services.md)을 참조하세요.|  
  
## 기타 데이터 서비스 공급자  
 [!INCLUDE[ssAstoria](../../../../includes/ssastoria-md.md)]에는 기타 공급자 중 하나를 사용하여 정의된 데이터 소스의 성능을 높이는 다음과 같은 추가 데이터 서비스 공급자가 있습니다.  

...
다음은 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 응용 프로그램에서 발생할 수 있는 문제와 이러한 문제를 방지하거나 문제의 영향을 줄일 수 있는 방법에 대한 설명입니다.  
 ...
 다른 문제는 [질문과 대답](../../../../../../docs/framework/data/adonet/sql/linq/frequently-asked-questions.md)에 설명되어 있습니다.  
  
## 지원되지 않는 표준 쿼리 연산자  
 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)]에서는 모든 표준 쿼리 연산자 메서드\(예: <xref:System.Linq.Enumerable.ElementAt%2A>\)를 지원하지 않습니다.  그 결과 프로젝트를 컴파일할 때 런타임 오류가 발생할 수 있습니다.  자세한 내용은 [표준 쿼리 연산자 변환](../../../../../../docs/framework/data/adonet/sql/linq/standard-query-operator-translation.md)을 참조하세요.  
...
## 파일 이름과 SQLMetal  
 입력 파일 이름을 지정하려면 이름을 명령줄에 입력 파일로 추가합니다.  **\/conn** 옵션 사용하여 연결 문자열에 파일 이름을 포함할 수는 없습니다.  자세한 내용은 [SqlMetal.exe\(코드 생성 도구\)](../../../../../../docs/framework/tools/sqlmetal-exe-code-generation-tool.md)을 참조하세요.  
...
-   사용자 고유의 코드를 사용하여 부모 개체를 삭제하는 데 방해가 되는 자식 개체를 먼저 삭제합니다.  
 ...
 자세한 내용은 [방법: 데이터베이스에서 행 삭제](../../../../../../docs/framework/data/adonet/sql/linq/how-to-delete-rows-from-the-database.md)을 참조하세요.  
  
## 쿼리할 수 없는 식  
 "Expression \[expression\] 형식의 식은 쿼리할 수 없습니다. 어셈블리 참조가 있는지 확인하세요." 오류가 나타나면 다음 사항을 확인하세요.  
...
-   <xref:System.Linq> 및 <xref:System.Data.Linq>에 사용할 `Imports`\([!INCLUDE[vbprvb](../../../../../../includes/vbprvb-md.md)]\) 또는 `using`\(C\#\) 지시문이 있는지 여부  
...
## 문자열 연결 예외  
 `[n]text`와 다른 `[n][var]char`에 매핑된 피연산자는 연결할 수 없습니다.  서로 다른 두 형식 집합에 매핑된 문자열을 연결하면 예외가 throw됩니다.  자세한 내용은 [System.String 메서드](../../../../../../docs/framework/data/adonet/sql/linq/system-string-methods.md)을 참조하세요.  
  
## SQL Server 2000의 Skip 및 Take 예외  
 SQL Server 2000 데이터베이스에 대해 <xref:System.Linq.Queryable.Take%2A> 또는 <xref:System.Linq.Queryable.Skip%2A>을 사용할 때는 ID 멤버\(<xref:System.Data.Linq.Mapping.ColumnAttribute.IsPrimaryKey%2A>\)를 사용해야 합니다.  조인이 아니라 단일 테이블에 대해 쿼리를 실행하거나, 쿼리가 <xref:System.Linq.Queryable.Distinct%2A>, <xref:System.Linq.Queryable.Except%2A>, <xref:System.Linq.Queryable.Intersect%2A> 또는 <xref:System.Linq.Queryable.Union%2A> 작업이어야 하며 쿼리에 <xref:System.Linq.Queryable.Concat%2A> 작업이 포함되면 안 됩니다.  자세한 내용은 [표준 쿼리 연산자 변환](../../../../../../docs/framework/data/adonet/sql/linq/standard-query-operator-translation.md)에서 "SQL Server 2000 지원" 단원을 참조하세요.  
  
 이 요구 사항은 [!INCLUDE[sqprsqlong](../../../../../../includes/sqprsqlong-md.md)]에는 적용되지 않습니다.  
  
## GroupBy InvalidOperationException  
 `group x by (Phone==@phone)`처럼 `boolean` 식을 사용하여 그룹화하는 <xref:System.Linq.Enumerable.GroupBy%2A> 쿼리에서 열 값이 null이면 이 예외가 throw됩니다.  식이 `boolean`이기 때문에 키가 `nullable` `boolean`이 아니라 `boolean`로 유추됩니다.  변환된 비교 연산의 결과가 null이면 `boolean`에 `nullable` `boolean`을 할당하려고 시도되고 예외가 throw됩니다.  

---
title: "SQL-CLR 형식 불일치 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# SQL-CLR 형식 불일치
[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)]에서는 개체 모델과 SQL Server 간의 변환을 대부분 자동화합니다.  하지만 정확한 변환이 불가능한 경우도 있습니다.  다음 단원에서는 이와 같이 CLR\(공용 언어 런타임\) 형식과 SQL Server 데이터베이스 형식 간에 불일치하는 부분에 대해 핵심적인 사항만 간략히 설명합니다.  [SQL\-CLR 형식 매핑](../../../../../../docs/framework/data/adonet/sql/linq/sql-clr-type-mapping.md) 및 [데이터 형식 및 함수](../../../../../../docs/framework/data/adonet/sql/linq/data-types-and-functions.md)에서 특정 형식 매핑 및 함수 변환에 대한 자세한 내용을 확인할 수 있습니다.  
...
```  
 ...
 SQL Server에서 쿼리를 실행하기 전에 Transact\-SQL 매개 변수 값을 지정해야 합니다.  이 예제에서는 먼저 `id` 매개 변수 값을 CLR의 <xref:System.Int32?displayProperty=fullName> 형식에서 SQL Server의 `INT` 형식으로 변환하여 데이터베이스에서 값을 인식할 수 있도록 해야 합니다.  그런 다음 결과를 검색하기 위해 SQL Server의 `DateOfBirth` 열을 SQL Server의 `DATETIME` 형식에서 개체 모델에 사용할 수 있는 CLR의 <xref:System.DateTime?displayProperty=fullName> 형식으로 변환해야 합니다.  이 예제에서 CLR 개체 모델의 형식과 SQL Server 데이터베이스의 형식은 자연적으로 매핑됩니다.  그러나 이러한 방식으로 매핑되지 않는 경우도 있습니다.  
...
-   CLR <xref:System> 네임스페이스의 불일치  
    ...
    -   **부호 없는 정수**.  일반적으로 이 형식은 오버플로를 방지하기 위해 크기가 더 큰 부호 있는 대응 항목에 매핑됩니다.  리터럴은 값에 따라 크기가 같거나 더 작은 부호 있는 숫자로 변환할 수 있습니다.  
  
    -   **부울**.  이 형식은 1비트 또는 더 큰 숫자나 문자열로 매핑될 수 있습니다.  리터럴은 동일한 값으로 계산되는 식\(예: SQL의 `1=1`과 CLS의 `True`\)에 매핑할 수 있습니다.  
  
    -   **TimeSpan**.  이 형식은 두 `DateTime` 값의 차이를 나타내며 SQL Server의 `timestamp`에 대응되지 않습니다.  일부 경우에는 CLR의 <xref:System.Timespan?displayProperty=fullName>이 SQL Server의 `TIME` 형식에 매핑될 수 있습니다.  SQL Server의 `TIME` 형식은 24시간 미만의 양수 값을 나타낼 때만 사용되지만  CLR의 <xref:System.Timespan>은 나타낼 수 있는 범위가 넓습니다.  
  
    > [!NOTE]
    >  <xref:System.Data.SqlTypes>의 SQL Server 관련 [!INCLUDE[dnprdnshort](../../../../../../includes/dnprdnshort-md.md)] 형식은 이러한 비교에 포함되지 않습니다.  
  
-   SQL Server의 불일치  
  
    -   **고정 길이 문자 형식**.  Transact\-SQL은 유니코드와 비유니코드 범주를 구분합니다. 각 범주에는 고정 길이 `nchar`\/`char`, 가변 길이 `nvarchar`\/`varchar` 및 크기가 더 큰 `ntext`\/`text`라는 세 가지 고유한 형식이 있습니다.  고정 길이 문자 형식은 문자를 검색하기 위해 CLR의 <xref:System.Char?displayProperty=fullName> 형식에 매핑될 수 있지만 실제 변환 및 동작은 동일한 형식과 일치하지 않습니다.  
  
    -   **Bit**.  `bit` 도메인에는 `Nullable<Boolean>`과 동일한 수의 값이 있지만 형식이 서로 다릅니다.  `Bit`는 `true`\/`false` 대신 값 `1`과 `0`을 사용하며 부울 식에 대응되는 항목으로 사용될 수 없습니다.  
  
    -   **Timestamp**.  CLR의 <xref:System.Timespan?displayProperty=fullName> 형식과 달리 SQL Server의 `TIMESTAMP` 형식은 각 업데이트에 고유하고 <xref:System.DateTime> 값 간의 차이를 기반으로 하지 않는 데이터베이스에서 생성된 8바이트 숫자를 나타냅니다.  
  
    -   **Money** 및 **SmallMoney**.  이 형식은 <xref:System.Decimal>에 매핑될 수 있지만 기본적으로 다른 형식이며 서버 기반 함수와 변환에서 다른 형식으로 취급됩니다.  
  
### 다중 매핑  
 여러 SQL Server 데이터 형식을 하나 이상의 CLR 데이터 형식에 매핑할 수 있습니다.  또한 여러 CLR 형식을 하나 이상의 SQL Server 형식에 매핑할 수도 있습니다.  LINQ to SQL에서 매핑을 지원하지만 그렇다고 해서 CLR 및 SQL Server 간에 매핑되는 두 형식의 정밀도, 범위 및 의미 체계가 완벽하게 일치하는 것은 아닙니다.  일부 매핑에는 이러한 차원의 일부 또는 전체에 있어 차이가 있을 수 있습니다.  [SQL\-CLR 형식 매핑](../../../../../../docs/framework/data/adonet/sql/linq/sql-clr-type-mapping.md)에서는 다양하게 연결될 수 있는 매핑 간에 어떤 차이점이 있는지 확인할 수 있습니다.  
...
-   C\#은 논리 연산자 `&&` 및 `||`에 대한 연산자의 사전순에 따라 단락 의미 체계를 지정합니다.  반면 SQL은 세트 기반 쿼리를 대상으로 하므로 최적화 프로그램에서 보다 자유롭게 실행 순서를 결정할 수 있습니다.  몇 가지 함축된 의미에는 다음이 포함됩니다.  
    ...
    -   의미 체계상 동등한 변환에서 피연산자 실행 순서를 다시 매기지 않으려면 SQL에 &quot;`CASE` … `WHEN` … `THEN`&quot; 구문이 필요합니다.  
  
    -   C\# 식에서 첫 번째 피연산자의 계산 결과에 따라 두 번째 피연산자가 계산되는 경우 `AND`\/`OR` 연산자로 느슨하게 변환하면 예기치 않은 오류가 발생할 수 있습니다.  
...
-   기본적으로 일반 로캘의 경우 SQL에서 문자열 비교는 대\/소문자를 구분하지 않습니다.  Visual Basic 및 C\#에서는 대\/소문자를 구분합니다.  예를 들어 `s`가 `food`인 경우 `s == "Food"`\([!INCLUDE[vbprvb](../../../../../../includes/vbprvb-md.md)]의 `s = "Food"`\) 및 `s == "Food"`가 다른 결과를 생성할 수 있습니다.  
...
-   SQL의 고정 길이 문자 형식 인수에 적용된 연산자\/함수는 CLR의 <xref:System.String?displayProperty=fullName>에 적용된 동일한 연산자\/함수와 의미 체계가 완전히 다릅니다.  이 문제를 형식에 대한 단원에서 설명한 대응 항목 누락 문제의 확장으로 간주할 수도 있습니다.  

...
>  큰 UDT에 대한 지원이 향상된 SqlClient를 활용하려면 .NET Framework 3.5 SP1 이상을 설치해야 합니다.  
 ...
 이전에는 UDT의 최대 크기가 8KB로 제한되었습니다.  그러나 SQL Server 2008에서는 <xref:Microsoft.SqlServer.Server.Format> 형식인 UDT에 대해 이 제한이 제거되었습니다.  
...
## GetSchema를 사용하여 UDT 스키마 검색  
 <xref:System.Data.SqlClient.SqlConnection>의 <xref:System.Data.SqlClient.SqlConnection.GetSchema%2A> 메서드는 데이터베이스 스키마 정보를 <xref:System.Data.DataTable>에 반환합니다.  자세한 내용은 [SQL Server 스키마 컬렉션](../../../../../docs/framework/data/adonet/sql-server-schema-collections.md)을 참조하세요.  
  
### UDT의 GetSchemaTable 열 값  
 <xref:System.Data.SqlClient.SqlDataReader>의 <xref:System.Data.SqlClient.SqlDataReader.GetSchemaTable%2A> 메서드는 열 메타데이터를 설명하는 <xref:System.Data.DataTable>을 반환합니다.  다음 표에서는 SQL Server 2005와 SQL Server 2008 간의 큰 UDT 열 메타데이터 차이점에 대해 설명합니다.  
...
## SqlDataReader 고려 사항  
 SQL Server 2008부터 <xref:System.Data.SqlClient.SqlDataReader>는 큰 UDT 값 검색을 지원하도록 확장되었습니다.  [SqlDataReader](assetId:///SqlDataReader?qualifyHint=False&amp;autoUpgrade=True)에서는 현재 사용하고 있는 SQL Server 버전뿐만 아니라 연결 문자열에 지정된 `Type System Version`에 따라 큰 UDT를 다르게 처리합니다.  자세한 내용은 <xref:System.Data.SqlClient.SqlConnection.ConnectionString%2A>을 참조하세요.  
  
 <xref:System.Data.SqlClient.SqlDataReader>의 다음 메서드는 `Type System Version`이 SQL Server 2005로 설정되어 있는 경우 UDT 대신 <xref:System.Data.SqlTypes.SqlBinary>를 반환합니다.  
...
-   <xref:System.Data.SqlClient.SqlDataReader.GetSqlValues%2A>  
 ...
 다음 메서드는 `Type System Version`이 SQL Server 2005로 설정되어 있는 경우 UDT 대신 `Byte[]`의 배열을 반환합니다.  

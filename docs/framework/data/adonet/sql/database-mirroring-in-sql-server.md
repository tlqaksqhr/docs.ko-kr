...
## <a name="retrieving-the-current-server-name"></a>현재 서버 이름 검색  
 ...
 장애 조치 이벤트가 발생 하 고 연결이 미러 서버로 전환 되 고 **DataSource** 속성이 미러 이름을 반영 하도록 업데이트 됩니다.  
...
## <a name="sqlclient-mirroring-behavior"></a>SqlClient 미러링 동작  
 클라이언트는 항상 현재의 주 서버에 대해 연결을 시도합니다. 연결에 실패하면 장애 조치 파트너에 대해 연결을 시도합니다. 미러 데이터베이스가 파트너 서버의 주 역할로 전환된 경우 연결이 성공하며 새 주-미러 매핑이 클라이언트로 전송되고 호출하는 <xref:System.AppDomain>의 수명 동안 캐시됩니다. 영구 저장소에 저장 되지 않은 하 고 다른의 후속 연결에 사용할 수 없으면 **AppDomain** 또는 프로세스입니다. 그러나 동일한 내의 후속 연결에 사용할 수는 **AppDomain**합니다. 참고는 다른 **AppDomain** 항상 동일한 또는 다른 컴퓨터에서 실행 중인 프로세스의 연결 풀이 있으며 또는 이러한 연결은 다시 설정 되지 않습니다. 이 경우 주 데이터베이스가 다운 되 면 각 프로세스 또는 **AppDomain** 실패 하 고 풀 자동으로 지워집니다.  
...
|리소스|설명|  
|--------------|-----------------|  
|[데이터베이스 미러링](http://msdn.microsoft.com/library/bb934127.aspx) SQL Server 온라인 설명서의|SQL Server에서 미러링을 설정 및 구성하는 방법을 설명합니다.|  

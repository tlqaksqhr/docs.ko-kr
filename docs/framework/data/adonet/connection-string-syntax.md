...
|아니요|N/A|예|예|암호화는 항상가 수행 되지만 자체 서명 된 서버 인증서를 사용할 수 있습니다.|  
|예|아니요|무시됨|무시됨|확인 가능한 서버 인증서, 필요한 경우에 암호화가 설정 되며 그렇지 않으면 연결 시도가 실패합니다.|  
|예|예|아니요(기본값)|무시됨|암호화는 항상가 수행 되지만 자체 서명 된 서버 인증서를 사용할 수 있습니다.|  
|예|예|예|아니요(기본값)|확인 가능한 서버 인증서, 필요한 경우에 암호화가 설정 되며 그렇지 않으면 연결 시도가 실패합니다.|  
|예|예|예|예|암호화는 항상가 수행 되지만 자체 서명 된 서버 인증서를 사용할 수 있습니다.|  

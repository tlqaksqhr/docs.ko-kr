...
|특성 이름|설명|  
|--------------------|-----------------|  
|**msdata:ConstraintOnly**|경우 **ConstraintOnly = "true"** 에 지정 된 **keyref** 스키마의 요소를 한 제약 조건을 만들어지지만 관계는 만들어지지입니다. 이 특성이 지정 되지 않은 경우 (또는로 설정 되어 **False**)에 제약 조건과 관계가 모두 만들어집니다는 **데이터 집합**합니다.|  

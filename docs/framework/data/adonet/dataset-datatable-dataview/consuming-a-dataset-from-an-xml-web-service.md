...
3.  XML Web services 클라이언트를 만듭니다.  
     ...
     이 예제에서는 전달은 **데이터 집합** 반환한 **DataSet.GetChanges** 를 **UpdateCustomers** 를 전달 하도록 수정 된 행만 필요 하기 때문에  **UpdateCustomers**합니다. **UpdateCustomers** 는 적용 된 반환 **데이터 집합**, 다음 수 있는 **병합** 기존 **DataSet** 확인 된 변경 내용을 통합할 수 및 업데이트 작업에서 모든 행 오류 정보입니다. 웹 참조를 만들려면 Visual Studio를 사용 하 고 웹 참조의 이름을 dssample로 바꾼 것으로 가정 하는 다음 코드는 **웹 참조 추가** 대화 상자.  

---
title: "수정 SQL 생성 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
이 단원에서는 SQL:1999 규격 데이터베이스 공급자에 대한 수정 SQL 생성 모듈을 개발하는 방법에 대해 설명합니다.  이 모듈은 수정 명령 트리를 적절한 SQL INSERT, UPDATE 또는 DELETE 문으로 변환하는 작업을 담당합니다.  
 ...
 SELECT 문의 SQL 생성에 대한 자세한 내용은 [SQL 생성](../../../../../docs/framework/data/adonet/ef/sql-generation.md)을 참조하세요.  
...
-   DbDeleteCommandTree  
 ...
 DbModificationCommandTree 및 [!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]에서 생성되는 해당 구현은 항상 단일 행 작업을 나타냅니다.  이 단원에서는 .NET Framework 버전 3.5에서 이러한 형식과 관련 제약 조건에 대해 설명합니다.  
  
 ![다이어그램](../../../../../docs/framework/data/adonet/ef/media/558ba7b3-dd19-48d0-b91e-30a76415bf5f.gif "558ba7b3\-dd19\-48d0\-b91e\-30a76415bf5f")  
  
 DbModificationCommandTree에는 수정 작업의 대상 집합을 나타내는 Target 속성이 있습니다.  Target의 Expression 속성은 입력 집합을 정의하며 항상 DbScanExpression입니다.  DbScanExpression은 테이블 또는 뷰를 나타내거나, Target의 메타데이터 속성 “정의 쿼리”가 null이 아닌 경우 쿼리를 사용하여 정의된 데이터 집합을 나타낼 수 있습니다.  
...
### 수정 명령 트리 속성에 대한 제한 사항  
 다음 정보와 제한 사항은 수정 명령 트리 속성에 적용됩니다.  
  
#### DbInsertCommandTree 및 DbUpdateCommandTree의 Returning  
...
#### DbInsertCommandTree 및 DbUpdateCommandTree의 SetClauses  
...
#### DbUpdateCommandTree 및 DbDeleteCommandTree의 Predicate  
...
## 샘플 공급자의 수정 SQL 생성  
 [Entity Framework 샘플 공급자](http://go.microsoft.com/fwlink/?LinkId=180616)\(영문\)에는 [!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]를 지원하는 ADO.NET 데이터 공급자 구성 요소가 나와 있습니다.  샘플 공급자는 SQL Server 2005 데이터베이스를 대상으로 하며 System.Data.SqlClient ADO.NET 2.0 데이터 공급자 위에 래퍼로 구현됩니다.  
...
## INSERT SQL 명령 생성  
 ...
 첫 번째 템플릿에는 SetClauses 목록의 값에 따라 삽입을 수행하는 명령과 Returning 속성이 null이 아닌 경우 삽입된 행의 Returning 속성에 지정된 속성을 반환하는 SELECT 문이 있습니다.  행이 삽입된 경우 조건자 요소 "@@ROWCOUNT \> 0"은 true입니다.  조건자 요소 "keyMemberI \=  keyValueI &#124; scope\_identity\(\)"는 scope\_identity\(\)가 ID\(저장소 생성\) 열에 마지막으로 삽입된 ID 값을 반환하기 때문에 keyMemeberI이 저장소 생성 키인 경우에만 "keyMemberI \=  scope\_identity\(\)" 모양입니다.  
...
```  
 ...
 두 번째 템플릿은 기본 키가 저장소 생성 키이지만 정수 형식이 아니라서 scope\_identity\(\)와 함께 사용할 수 없는 행의 삽입을 지정하는 경우에 필요합니다.  또한 복합 저장소 생성 키가 있는 경우에도 사용됩니다.  
...
```  
 ...
 SET 절이 지정되지 않은 경우에만 SET 절에 모조 SET 절\("@i \= 0"\)이 있습니다.  이는 모든 저장소 계산 열이 다시 계산되도록 하기 위한 것입니다.  

...
## 설명  
 `expression IS NOT OF (type)` 및 `expression IS NOT OF (ONLY type)` 식은 구문상 각각 `NOT (expression IS OF (type))` 및 `NOT (expression IS OF (ONLY type))`와 동등합니다.  
...
## 예제  
 다음 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 쿼리에서는 IS OF 연산자를 사용하여 쿼리 식의 형식을 결정한 다음 TREAT 연산자를 사용하여 Course 형식의 개체를 OnsiteCourse 형식의 개체 컬렉션으로 변환합니다. 쿼리는 [School 모델](http://msdn.microsoft.com/ko-kr/859a9587-81ea-4a45-9bc0-f8d330e1adac)을 기반으로 합니다.  

---
title: "리터럴(Entity SQL) | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## Null  
 ...
 형식화된 null은 null 리터럴에 대한 캐스트를 통해 만들 수 있습니다.  자세한 내용은 [CAST](../../../../../../docs/framework/data/adonet/ef/language-reference/cast-entity-sql.md)을 참조하세요.  
  
 자유 부동 null 리터럴을 사용할 수 있는 위치에 대한 규칙은 [Null 리터럴 및 형식 유추](../../../../../../docs/framework/data/adonet/ef/language-reference/null-literals-and-type-inference-entity-sql.md)를 참조하세요.  
...
## DateTime  
 ...
 날짜 부분의 형식은 `YYYY`\-`MM`\-`DD`이어야 하며, 여기서 `YYYY`는 0001에서 9999 사이의 4자리 연도 값이고, `MM`은 1에서 12 사이의 월이며, `DD`는 지정된 월 `MM`에 유효한 일 값입니다.  
...
## Guid  
 `GUID` 리터럴은 GUID\(Globally Unique Identifier\)를 나타냅니다.  이 리터럴은 키워드 `GUID` 뒤에 *레지스트리* 형식\(예: 작은따옴표로 묶인 8\-4\-4\-4\-12\)의 16진수로 구성된 시퀀스입니다.  16진수는 대\/소문자를 구분하지 않습니다.  

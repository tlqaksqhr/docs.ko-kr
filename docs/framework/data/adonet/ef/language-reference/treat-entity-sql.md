...
> [!NOTE]
>  지정된 식```Employee```가 지정된 데이터 형식 `Person`의 하위 형식이어야 하거나, 데이터 형식이 식의 하위 형식이어야 합니다. 그렇지 않으면 식에서 컴파일 시간 오류가 발생합니다.  
...
## 예제  
 다음 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 쿼리에서는 TREAT 연산자를 사용하여 Course 형식의 개체를 OnsiteCourse 형식의 개체 컬렉션으로 변환합니다. 쿼리는 [School 모델](http://msdn.microsoft.com/ko-kr/859a9587-81ea-4a45-9bc0-f8d330e1adac)을 기반으로 합니다.  

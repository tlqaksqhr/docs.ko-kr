---
title: "명령 트리의 모양 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   공급자에 제공되고 백엔드에 대한 쿼리를 설명하는 출력 쿼리를 표현하기 위해  
 ...
 DbExpression 개체는 특정 계산을 나타냅니다.  몇 가지 종류의 식이 상수, 변수, 함수, 생성자 및 표준 관계형 연산자\(필터, 조인 등\)를 포함하는 쿼리 식을 작성할 수 있도록 [!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]에서 제공됩니다.  모든 DbExpression 개체에는 해당 식에서 생성하는 결과의 형식을 나타내는 ResultType 속성이 있습니다.  이 형식은 TypeUsage로 표현됩니다.  
...
-   사용자 정의 함수  
 ...
 정식 함수\(자세한 내용은 [정식 함수](../../../../../docs/framework/data/adonet/ef/language-reference/canonical-functions.md) 참조\)는 [!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)]의 일부로 지정되며, 공급자는 해당 지정 사항에 따라 정식 함수의 구현을 제공해야 합니다.  저장소 함수는 해당 공급자 매니페스트에 지정된 사항을 기반으로 합니다.  사용자 정의 함수는 SSDL에서 지정된 사항을 기반으로 합니다.  
  
 또한 NiladicFunction 특성이 있는 함수에는 인수가 없으며 끝에 괄호 없이 변환되어야 합니다.  즉, *\<functionName\>\(\)*이 아니라 *\<functionName\>*으로 변환되어야 합니다.  
...
#### DbScanExpression  
 ...
 대상의 메타데이터 속성 “정의 쿼리”가 null이 아니면 대상은 쿼리를 나타냅니다. 쿼리의 텍스트는 저장소 스키마 정의에 지정된 공급자의 특정 언어로 해당 메타데이터 속성에서 제공됩니다.  
  
 대상의 메타데이터 속성 “정의 쿼리”가 null이면 대상은 테이블 또는 뷰를 나타냅니다.  스키마 접두사는 "스키마" 메타데이터 속성이 null이 아니면 해당 속성에 있으며, 그렇지 않으면 엔터티 컨테이너 이름입니다.  테이블 또는 뷰 이름은 "테이블" 메타데이터 속성이 null이 아니면 해당 속성에 있으며, 그렇지 않으면 엔터티 집합 기반의 Name 속성입니다.  

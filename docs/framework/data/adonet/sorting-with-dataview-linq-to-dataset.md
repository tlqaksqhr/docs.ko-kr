---
title: "DataView로 정렬(LINQ to DataSet) | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
특정 조건을 기준으로 데이터를 정렬한 다음 UI 컨트롤을 통해 클라이언트에 데이터를 제공하는 기능은 데이터 바인딩의 중요한 기능입니다.  <xref:System.Data.DataView>에서는 데이터를 정렬하여 특정 정렬 기준에 따라 정렬된 데이터 행을 반환하는 여러 방법을 제공합니다.  <xref:System.Data.DataView>에는 문자열 기반 정렬 기능뿐만 아니라 [!INCLUDE[vbteclinqext](../../../../includes/vbteclinqext-md.md)] 식을 정렬 기준으로 사용할 수 있는 기능도 있습니다. [!INCLUDE[vbteclinq](../../../../includes/vbteclinq-md.md)] 식을 사용하면 문자열 기반 정렬에 비해 더 복잡하고 강력한 정렬 작업을 수행할 수 있습니다.  이 항목에서는 <xref:System.Data.DataView>를 사용하여 정렬하는 두 가지 방법을 모두 설명합니다.  
...
## 정렬 정보가 있는 쿼리에서 DataView 만들기  
 ...
 식 기반 정렬은 간단한 문자열 기반 정렬에 비해 강력하고 복잡한 정렬 기능을 제공합니다.  문자열 기반 정렬 및 식 기반 정렬은 함께 사용할 수 없습니다.  쿼리에서 <xref:System.Data.DataView>를 만든 후에 문자열 기반 <xref:System.Data.DataView.Sort%2A>를 설정하면 쿼리에서 유추된 식 기반 필터가 지워지며, 이 필터는 재설정할 수 없습니다.  
  
 <xref:System.Data.DataView>의 인덱스는 <xref:System.Data.DataView>가 만들어지거나 정렬 또는 필터링 정보가 수정될 때 작성됩니다.  최적의 성능을 얻으려면 <xref:System.Data.DataView>가 만들어진 [!INCLUDE[linq_dataset](../../../../includes/linq-dataset-md.md)] 쿼리에 정렬 기준을 제공한 다음 이후에 정렬 정보를 수정하지 않아야 합니다.  자세한 내용은 [DataView 성능](../../../../docs/framework/data/adonet/dataview-performance.md)을 참조하세요.  
...
## 문자열 기반 정렬 속성 사용  
 <xref:System.Data.DataView>의 문자열 기반 정렬 기능은 [!INCLUDE[linq_dataset](../../../../includes/linq-dataset-md.md)]에서 계속 사용할 수 있습니다.  [!INCLUDE[linq_dataset](../../../../includes/linq-dataset-md.md)] 쿼리에서 <xref:System.Data.DataView>를 만든 후에는 <xref:System.Data.DataView.Sort%2A> 속성을 사용하여 <xref:System.Data.DataView>에 대한 정렬을 설정할 수 있습니다.  
 ...
 문자열 기반 <xref:System.Data.DataView.Sort%2A> 필터링에 대한 자세한 내용은 [데이터 정렬 및 필터링](../../../../docs/framework/data/adonet/dataset-datatable-dataview/sorting-and-filtering-data.md)\(을\)를 참조하세요.  

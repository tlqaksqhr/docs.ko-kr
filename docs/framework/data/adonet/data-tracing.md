...
## <a name="accessing-diagnostic-information-in-the-extended-events-log"></a>확장 이벤트 로그에서 진단 정보에 액세스  
 에 [!INCLUDE[dnprdnshort](../../../../includes/dnprdnshort-md.md)] Data Provider for SQL Server, 데이터 액세스 추적 ([데이터 액세스 추적](http://msdn.microsoft.com/library/hh880086.aspx)) 쉽게 수에서 클라이언트 이벤트와 연결 실패 등의 진단 정보 상관 관계를 보다 쉽게 업데이트는 서버 연결 링 버퍼 및 응용 프로그램 성능 정보 확장된 이벤트 로그에 있습니다. 확장된 이벤트 로그 읽기에 대 한 정보를 참조 하십시오. [이벤트 세션 데이터 보기](http://msdn.microsoft.com/library/hh710068\(SQL.110\).aspx)합니다.  

---
title: "&lt;ImpliesType&gt; 요소(.NET 네이티브) | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|값|설명|  
|-----------|-----------------|  
|*type_name*|형식 이름입니다. 이 나타내는 형식이 경우 `<ImpliesType>` 요소를 포함 하는 동일한 네임 스페이스에서 찾을 `<Type>` 요소인 *type_name* 네임 스페이스가 없는 형식 이름을 포함할 수 있습니다. 그렇지 않으면 *type_name* 정규화 된 형식 이름을 포함 해야 합니다.|  
...
|[<>\>](../../../docs/framework/net-native/method-element-net-native.md)|메서드에 리플렉션 정책을 적용합니다.|  
...
```  
 ...
 다음은 하나 이상의 serializer에 적용되는 실제 예제입니다. 지시문은 캡처 작업에 대 한 리플렉션이로 형식화 된 요구 사항을 `IList<` *무언가* `>` 해당 위해서는 `List<` *무언가* `>` 응용 프로그램별 주석이 없어도 유형입니다.  
...
```  
 ...
 `<ImpliesType>` 요소는 `<Method>` 내에서도 나타날 수 있습니다. 경우에 따라 제네릭 메서드를 인스턴스화하면 형식 인스턴스화에 대한 리플렉션이 수행될 수 있기 때문입니다. 예를 들어, 제네릭 메서드를 imagine `IEnumerable<T> MakeEnumerable<T>(string` `spelling``, T` `defaultValue``)` 함께 연결 된 지정한 라이브러리에 동적으로 액세스 하는 <xref:System.Collections.Generic.List%601> 및 <xref:System.Array> 형식입니다. 이 예는 다음 코드와 같이 표시될 수 있습니다.  

...
### <a name="typeof-used-in-the-constructor"></a>생성자 내부에서 사용되는 typeof  
 이러한 serialization 클래스의 생성자를 호출하고 C# [typeof](~/docs/csharp/language-reference/keywords/typeof.md) 키워드를 메서드 호출에 포함하는 경우에는 **추가 작업을 수행할 필요가 없습니다**. 예를 들어 serialization 클래스 생성자에 대한 다음의 각 호출에서 `typeof` 키워드는 생성자에 전달되는 식의 일부로 사용됩니다.  

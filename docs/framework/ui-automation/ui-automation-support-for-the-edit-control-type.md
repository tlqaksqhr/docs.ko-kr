...
|<xref:System.Windows.Automation.Provider.IRangeValueProvider.Value%2A>|메모를 참조하세요.|이 속성은 편집 컨트롤의 숫자 콘텐츠를 나타냅니다.`Minimum` 및 `Maximum` 속성에 지정된 범위 내에서 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 클라이언트가 보다 정확한 값을 설정하면, Value 속성은 허용되는 가장 근접한 값으로 자동으로 반올림됩니다.|  

---
title: "XML Serialization 예제 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# XML Serialization 예제
XML serialization은 간단한 것부터 복잡한 것까지 여러 형태를 가집니다.예를 들어 [XML Serialization 소개](../../../docs/framework/serialization/introducing-xml-serialization.md)에서처럼 public 필드와 속성으로 간단하게 구성된 클래스를 serialize할 수 있습니다.다음 코드 예제에서는 XML serialization을 사용하여 특정 XML 스키마\(XSD\) 문서를 따르는 XML 스트림을 생성하는 방법을 포함한 여러 가지 고급 시나리오를 보여 줍니다.  
...
## XmlElement 및 XmlNode serialize  
...
## 복잡한 개체를 반환하는 필드가 포함된 클래스 serialize  
 속성 또는 필드가 복잡한 개체\(예: 배열 또는 클래스 인스턴스\)를 반환하는 경우 [XmlSerializer](https://msdn.microsoft.com/en-us/library/system.xml.serialization.xmlserializer.aspx)는 그 개체를 주 XML 문서 내의 중첩된 요소로 변환합니다.예를 들어, 다음 코드 예제의 첫째 클래스는 둘째 클래스의 인스턴스를 반환합니다.  
...
```  
 ...
 serialize된 XML 출력은 다음과 같을 수 있습니다.  
...
## 개체 배열 serialize  
 다음 코드 예제에서처럼 개체 배열을 반환하는 필드를 serialize할 수도 있습니다.  
...
## ICollection 인터페이스를 구현하는 클래스 serialize  
 <xref:System.Collections.ICollection> 인터페이스를 구현하여 컬렉션 클래스를 직접 만들고 <xref:System.Xml.Serialization.XmlSerializer>를 사용하여 이러한 클래스의 인스턴스를 serialize할 수 있습니다.클래스가 <xref:System.Collections.ICollection> 인터페이스를 구현할 때 클래스에 의해 포함된 컬렉션만 serialize됩니다.클래스에 추가된 모든 공용 속성이나 필드는 serialize되지 않습니다.클래스에는 serialize될 **Add** 메서드 및 **Item** 속성\(C\# 인덱서\)이 포함되어야 합니다.  
...
## 구매 주문 예제  
 다음 예제 코드를 잘라내어 파일 이름 확장명이 .cs 또는 .vb 텍스트 파일에 붙여넣을 수 있습니다.C\# 또는 Visual Basic 컴파일러를 사용하여 파일을 컴파일합니다.그런 다음 실행 파일의 이름을 입력하여 실행합니다.  
 ...
 이 예제에서는 `PurchaseOrder`라는 클래스를 serialize한 다음 deserialize합니다.`ShipTo`라는 public 필드를 `Address`로 설정해야 하기 때문에 `Address`라는 두 번째 클래스도 포함해야 합니다.마찬가지로 `OrderedItem` 개체의 배열을 `OrderedItems` 필드로 설정해야 하기 때문에 `OrderedItem` 클래스도 포함했습니다.마지막으로 `Test`라는 클래스에는 클래스를 serialize하고 deserialize하는 코드가 포함됩니다.  
  
 `CreatePO` 메서드는 `PurchaseOrder`, `Address` 및 `OrderedItem` 클래스 개체를 만들고 public 필드 값을 설정합니다.메서드는 `PurchaseOrder`를 serialize하고 deserialize하는 데 사용되는 <xref:System.Xml.Serialization.XmlSerializer> 클래스의 인스턴스도 생성합니다.코드는 serialize될 클래스 형식을 생성자에 전달합니다.또한 코드는 XML 스트림을 XML 문서에 쓰는 데 사용되는 [FileStream](frlrfSystemIOFileStreamClassTopic)도 만듭니다.  
  
 `ReadPo` 메서드는 조금 더 간단합니다.deserialize할 개체를 만들고 그 값을 읽습니다.`CreatePo` 메서드와 마찬가지로 먼저 <xref:System.Xml.Serialization.XmlSerializer>를 만들어 deserialize될 클래스의 형식을 생성자에 전달합니다.또한 XML 문서를 읽기 위해 <xref:System.IO.FileStream>이 필요합니다.개체를 deserialize하기 위해 <xref:System.IO.FileStream>을 인수로 사용하여 <xref:System.Xml.Serialization.XmlSerializer.Deserialize%2A> 메서드를 호출합니다.deserialize된 개체는 `PurchaseOrder` 형식의 개체 변수로 캐스팅되어야 합니다.그런 다음 코드는 deserialize된 `PurchaseOrder`의 값을 읽습니다.작성된 PO.xml 파일을 읽어 실제 XML 출력을 볼 수 있습니다.  

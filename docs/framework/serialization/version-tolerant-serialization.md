---
title: "버전 독립적 Serialization | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
dev_langs: 
  ...
  - "jsharp"
helpviewer_keywords: 
  - "BinaryFormatter 클래스, 샘플"
  - "serialization, 특성"
  - "serialization, 제어"
  - "serialization, 사용자 지정 serialization"
  - "serialization, 버전 독립적"
  - "버전 독립적 serialization"
  - "버전 및 serialization"
...
-   데이터가 누락된 형식의 이전 버전을 deserialize할 때 응용 프로그램의 새 버전이 예외를 throw합니다.  
 ...
 VTS\(버전 독립적 Serialization\)는 .NET Framework 2.0에 추가된 기능 집합으로, serialize 가능한 형식을 시간이 지남에 따라 더 쉽게 수정할 수 있게 해 줍니다.특히 VTS 기능은 제네릭 형식을 비롯하여 <xref:System.SerializableAttribute> 특성이 적용된 클래스에 사용할 수 있습니다.VTS를 사용하면 형식의 다른 버전과 호환성을 유지하면서 해당 클래스에 새 필드를 추가할 수 있습니다.작동하는 샘플 응용 프로그램을 보려면 [Version Tolerant Serialization 기술 샘플](../../../docs/framework/serialization/version-tolerant-serialization-technology-sample.md)을 참조하십시오.  
  
 VTS 기능은 <xref:System.Runtime.Serialization.Formatters.Binary.BinaryFormatter>를 사용할 때 활성화됩니다.또한 <xref:System.Runtime.Serialization.Formatters.Soap.SoapFormatter>를 사용할 때 잘못 사용된 데이터 허용 범위를 제외한 모든 기능도 활성화됩니다. serialization에 이러한 클래스를 사용하는 방법에 대한 자세한 내용은 [이진 Serialization](../../../docs/framework/serialization/binary-serialization.md)을 참조하십시오.  
...
-   콜백 serialization.이를 통해 데이터가 누락되었을 때 지능적으로 기본값을 설정할 수 있습니다.  
 ...
 또한 새 선택적 필드가 추가되었을 때 선언하기 위한 기능이 있습니다.이것은 <xref:System.Runtime.Serialization.OptionalFieldAttribute> 특성의 <xref:System.Runtime.Serialization.OptionalFieldAttribute.VersionAdded%2A> 속성  
  
 이 기능은 아래에서 자세히 설명합니다.  
  
## 잘못 사용된 데이터 또는 예기치 않은 데이터의 허용 범위  
 ...
 다음 예제에서는 이전 응용 프로그램이 새 버전을 deserialize할 때 `Address` 클래스 버전 2.0의 `CountryField`에 포함된 추가 데이터가 무시됩니다.  
...
## 누락된 데이터의 허용 오차  
 ...
 다음 예제에서는 `CountryField` 필드가 선택 사항으로 표시된 `Address` 클래스의 버전 2.0을 보여 줍니다.이전 응용 프로그램이 버전 2.0이 필요한 새 응용 프로그램에 버전 1을 전송하면 데이터의 부재가 무시됩니다.  
...
## SerializationBinder  
 서버와 클라이언트에서 서로 다른 버전의 클래스를 필요로 하므로 일부 사용자는 serialize 및 deserialize할 클래스를 제어해야 할 수 있습니다.<xref:System.RuntimeSerialization.SerializationBinder>는 serialization 및 deserialization 중에 사용되는 실제 형식을 제어하는 데 사용되는 추상 클래스입니다.이 클래스를 사용하려면 <xref:System.RuntimeSerialization.SerializationBinder>에서 클래스를 파생시키고 <xref:System.Runtime.Serialization.SerializationBinder.BindToName%2A> System.String, System.String)?qualifyHint=False&autoUpgrade=True 및 <xref:System.Runtime.Serialization.SerializationBinder.BindToType%2A> System.String)?qualifyHint=False&autoUpgrade=True 메서드를 재정의합니다.[!INCLUDE[crdefault](../../../includes/crdefault-md.md)][SerializationBinder를 사용하여 serialization 및 deserialization 제어](../../../docs/framework/wcf/feature-details/controlling-serialization-and-deserialization-with-serializationbinder.md).  
...
-   새 serialize된 필드를 추가할 때 **OptionalFieldAttribute** 특성을 적용합니다.  
  
-   **NonSerializedAttribute** 특성을 필드에서 제거할 때\(이전 버전에서는 serialize할 수 없었음\) **OptionalFieldAttribute** 특성을 적용합니다.  
...
-   **OptionalFieldAttribute** 특성에 **VersionAdded** 속성을 올바르게 설정합니다.  

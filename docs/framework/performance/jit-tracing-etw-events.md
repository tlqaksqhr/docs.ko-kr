---
title: "JIT 추적 ETW 이벤트 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|InlinerName|win:UnicodeString|컴파일러가 코드를 생성할 메서드의 이름입니다. 컴파일러가 `InlinerName`에 대한 호출을 생성하는 대신 코드를 `MethodBeingCompiledName`으로 인라인 처리하려고 하면 이름이 `MethodBeingCompiledName`과 같지 않을 수 있습니다.|  
...
|FailReason|win:UnicodeString|INLINE\_NEVER는 어떤 다른 이유로 인라인 처리가 실패할 것으로 확인된 이전 인라인 처리 시도를 의미합니다. 이외의 경우는 자유 형식 텍스트입니다.|  

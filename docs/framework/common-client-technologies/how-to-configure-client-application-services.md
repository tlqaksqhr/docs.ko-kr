...
3.  SQL Server Compact 데이터베이스를 생성 및 사용하려면 기본 연결 문자열 값을 유지합니다. [!INCLUDE[vsprvs](../../../includes/vsprvs-md.md)]에서 데이터베이스 파일을 생성하고 <xref:System.Windows.Forms.Application.UserAppDataPath%2A?displayProperty=fullName> 속성에 지정된 디렉터리에 넣습니다.  

---
title: "보안 및 Serialization | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
helpviewer_keywords: 
  - "코드 보안, serialization"
  - "보안 코딩, serialization"
  - "보안[.NET Framework], serialization"
  - "serialization, 보안"
...
---
# 보안 및 Serialization
다른 방법으로는 액세스할 수 없는 개체 인스턴스 데이터를 직렬화를 통해 다른 코드에서 보거나 수정할 수 있으므로 <xref:System.Security.Permissions.SecurityPermissionFlag> 플래그가 지정된 <xref:System.Security.Permissions.SecurityPermission>의 직렬화를 수행하려면 코드에 특수 권한이 필요합니다. 기본 정책에 따라 이 권한은 인터넷에서 다운로드한 코드나 인트라넷 코드에는 부여되지 않고 로컬 컴퓨터에 있는 코드에만 부여됩니다.  
...
-   **SerializationFormatter** 권한을 지정하여 **SecurityPermission**을 요청하거나 메서드 출력에 중요 정보가 노출되지 않도록 하여 <xref:System.Runtime.Serialization.ISerializable.GetObjectData%2A> 메서드의 보안을 명시적으로 유지해야 합니다. 예:  

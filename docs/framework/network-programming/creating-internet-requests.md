...
응용 프로그램은 <xref:System.Net.WebRequest.Create%2A?displayProperty=fullName> 메서드를 통해 <xref:System.Net.WebRequest> 인스턴스를 만듭니다. 이것은 전달된 URI 구성표에 따라 **WebRequest**에서 파생된 클래스를 만드는 정적 메서드입니다.  

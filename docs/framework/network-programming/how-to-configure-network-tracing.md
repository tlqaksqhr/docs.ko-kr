---
title: "방법: 네트워크 추적 구성 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
dev_langs: 
  ...
  - "jsharp"
helpviewer_keywords: 
  - "app.config 파일, 네트워크 추적"
  - "응용 프로그램 구성 파일, 네트워크 추적"
  - "구성 파일[.NET Framework], 네트워크 추적"
  - "서식 지정[.NET Framework], 네트워크 추적"
  - "수준 속성"
  - "네트워크 추적, 구성"
  - "프로토콜 수준 추적 출력"
  - "소켓, 추적 출력"
...
---
# 방법: 네트워크 추적 구성
응용 프로그램 또는 컴퓨터 구성 파일은 형식과 네트워크 추적의 내용을 결정하는 설정을 유지합니다. 이 절차를 수행하기 전에 추적이 활성화되어야 합니다. 추적 사용에 대한 자세한 내용은 [네트워크 추적 사용](../../../docs/framework/network-programming/enabling-network-tracing.md)을 참조하십시오.  
...
|특성 이름|특성 값|  
|-----------|----------|  
|`Value`|필수 <xref:System.String> 특성입니다. 출력의 자세한 정도를 설정합니다. 올바른 값은 `Critical`, `Error`, `Verbose`, `Warning`, `Information`입니다.<br /><br /> 예제에 나타낸 것처럼, \<스위치\> 요소의 \<이름 추가\> 요소에 대해 이 특성을 설정해야 합니다. 이 특성이 \<소스\> 요소에 대해 설정되어 있는 경우 예외가 throw됩니다.|  
|`maxdatasize`|선택적 <xref:System.Int32> 특성입니다. 각 줄 추적에 포함된 네트워크 데이터의 최대 바이트 수를 설정합니다. 기본값은 1024입니다.<br /><br /> 예제에 나타낸 것처럼, \<소스\> 요소에 대해 이 특성을 설정해야 합니다. 이 특성이 \<스위치\> 요소 아래의 요소에 대해 설정되어 있는 경우 예외가 throw됩니다.|  
|`Tracemode`|선택적 <xref:System.String> 특성입니다. 16진수 및 텍스트 형식으로 프로토콜 추적을 표시하려면 `includehex`로 설정합니다. 텍스트만 표시하려면 `protocolonly`로 설정합니다. 기본값은 `includehex`입니다.<br /><br /> 예제에 나타낸 것처럼, \<스위치\> 요소에 대해 이 특성을 설정해야 합니다. 이 특성이 \<소스\> 요소 아래의 요소에 대해 설정되어 있는 경우 예외가 throw됩니다.|  
...
## 참고 항목  
 ...
 [계측 및 추적 소개](http://msdn.microsoft.com/ko-kr/e924e57c-33cf-4b0e-9e7f-a45d13e38f2c)
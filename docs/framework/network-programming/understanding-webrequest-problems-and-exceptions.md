...
|상태|세부 정보|솔루션|  
|------------|-------------|--------------|  
|<xref:System.Net.WebExceptionStatus.SendFailure><br /><br /> 또는<br /><br /> <xref:System.Net.WebExceptionStatus.ReceiveFailure>|기본 소켓에 문제가 있습니다. 연결이 재설정되었을 수 있습니다.|다시 연결하여 요청을 다시 보내세요.<br /><br /> 최신 서비스 팩이 설치되어 있는지 확인합니다.<br /><br /> <xref:System.Net.ServicePointManager.MaxServicePointIdleTime%2A?displayProperty=fullName> 속성 값을 늘립니다.<br /><br /> <xref:System.Net.HttpWebRequest.KeepAlive%2A?displayProperty=fullName>를 `false`로 설정합니다.<br /><br /> <xref:System.Net.ServicePointManager.DefaultConnectionLimit%2A> 속성을 사용하여 최대 연결 수를 늘립니다.<br /><br /> 프록시 구성을 확인합니다.<br /><br /> SSL을 사용하는 경우 서버 프로세스에 인증서 저장소에 액세스할 수 있는 권한이 있는지 확인합니다.<br /><br /> 대량 데이터를 보내는 경우 <xref:System.Net.HttpWebRequest.AllowWriteStreamBuffering%2A>을 `false`로 설정합니다.|  

...
응용 프로그램 도메인 사용을 마쳤으면 <xref:System.AppDomain.Unload%2A?displayProperty=fullName> 메서드를 사용하여 언로드합니다. **Unload** 메서드는 지정된 응용 프로그램 도메인을 정상적으로 종료합니다. 언로드 프로세스 중에는 새 스레드가 응용 프로그램 도메인에 액세스할 수 없으며 모든 응용 프로그램 도메인 특정 데이터 구조가 비워집니다.  

---
title: "메시지 로깅에 대한 보안 고려 사항 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
### 중요한 정보 로깅  
 [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)]에서는 응용 프로그램별 헤더 및 본문의 데이터를 수정하지 않으며,[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서 응용 프로그램별 헤더 또는 본문 데이터에서 개인 정보를 추적하지도 않습니다.  
...
-   로그 파일 위치의 절대 경로를 지정합니다. 절대 경로는 웹 브라우저를 사용하여 외부 사용자가 해당 파일에 액세스하지 못하도록 웹 호스트 vroot 공용 디렉터리의 외부에 있어야 합니다.  
 ...
 기본적으로 사용자 이름 및 암호와 같은 PII\(개인적으로 식별할 수 있는 정보\)와 키는 추적 및 기록된 메시지에 기록되지 않습니다.그러나 컴퓨터 관리자는 Machine.config 파일의 `machineSettings` 요소에 있는 `enableLoggingKnownPII` 특성을 사용하여 컴퓨터에서 실행되는 응용 프로그램이 알려진 PII\(개인적으로 식별할 수 있는 정보\)를 기록하도록 허용할 수 있습니다.다음 구성 파일에서 이 작업을 수행하는 방법을 보여 줍니다.  
...
```  
 ...
 컴퓨터 관리자와 응용 프로그램 배포자는 이러한 두 개의 스위치를 사용할 때 특별히 주의를 기울여야 합니다.PII 로깅을 사용하도록 설정하면 보안 키와 PII가 기록됩니다.PII 로깅을 사용하지 않도록 설정해도 중요한 데이터와 응용 프로그램별 데이터는 메시지 헤더 및 본문에 기록됩니다.개인 정보 및 PII의 노출을 방지하는 방법은 [User Privacy](http://go.microsoft.com/fwlink/?LinkID=94647)를 참조하십시오.  
...
-   Log Known PII On: 이 이벤트는 알려진 PII의 로깅을 사용하도록 설정하면 내보내집니다.이는 Machine.config 파일의 `machineSettings` 요소에 있는 `enableLoggingKnownPii` 특성이 `true`로 설정되고, App.config 또는 Web.config 파일 중 하나의 `source` 요소에 있는 `logKnownPii` 특성도 `true`로 설정된 경우에 발생합니다.  
  
-   Log Known PII Not Allowed: 이 이벤트는 알려진 PII의 로깅이 허용되지 않을 때 내보내집니다.이는 App.config 또는 Web.config 파일 중 하나의 `source` 요소에 있는 `logKnownPii` 특성은 `true`로 설정되어 있지만 Machine.config 파일의 `machineSettings` 요소에 있는 `enableLoggingKnownPii` 특성은 `false`로 설정된 경우에 발생합니다.예외가 throw되지 않습니다.  
  
 이러한 이벤트는 Windows에 포함된 이벤트 뷰어 도구에서 볼 수 있습니다.이에 대한 자세한 내용은 [이벤트 로깅](../../../../docs/framework/wcf/diagnostics/event-logging/index.md)을 참조하십시오.  

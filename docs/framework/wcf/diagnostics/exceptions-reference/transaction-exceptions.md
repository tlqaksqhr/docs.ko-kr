...
|리소스 코드|리소스 문자열|  
|------------|-------------|  
|SFxCannotHaveDifferentTransactionProtocolsInOneBinding|메타데이터에서 가져오는 정책 정보는 각 작업의 TransactionProtocol에 여러 값을 지정하지만각 끝점에는 하나의 TransactionProtocol만 지원됩니다.|  

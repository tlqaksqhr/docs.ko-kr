...
이 항목에서는 추적을 사용하고, 추적을 내보내도록 추적 소스를 구성하고, 추적 수준을 설정하고, 종단 간 추적 상관 관계를 지원하도록 동작 추적 및 전파를 설정하고, 추적에 액세스하도록 추적 수신기를 설정하는 방법에 대해 설명합니다.  
 ...
 프로덕션 또는 디버깅 환경에서 권장되는 추적 설정에 대한 자세한 내용은 [추적 및 메시지 로깅에 권장되는 설정](../../../../../docs/framework/wcf/diagnostics/tracing/recommended-settings-for-tracing-and-message-logging.md)을 참조하십시오.  
  
> [!IMPORTANT]
>  Windows 8에서는 응용 프로그램에서 추적 로그를 만들려면 높은 권한\(관리자 권한으로 실행\)으로 응용 프로그램을 실행해야 합니다.  
...
-   추적 기능 오작동 시 Windows 오류 이벤트.[이벤트 로깅](../../../../../docs/framework/wcf/diagnostics/event-logging/index.md)을 참조하십시오.  
 ...
 응용 프로그램의 구성 파일\(웹에 호스팅된 응용 프로그램의 경우 Web.config, 자체 호스팅 응용 프로그램의 경우 Appname.exe.config\)을 편집하여 추적을 구성할 수 있습니다.다음은 이러한 편집의 예제입니다.이러한 설정에 대한 자세한 내용은 “추적을 사용하도록 추적 수신기 구성” 단원을 참조하십시오.  
...
> [!NOTE]
>  [!INCLUDE[vs_current_short](../../../../../includes/vs-current-short-md.md)]에서 [!INCLUDE[indigo2](../../../../../includes/indigo2-md.md)] 서비스 프로젝트의 구성 파일을 편집하려면 **솔루션 탐색기**에서 응용 프로그램의 구성 파일\(웹에 호스팅된 응용 프로그램의 경우 Web.config, 자체 호스팅 응용 프로그램의 경우 Appname.exe.config\)을 마우스 오른쪽 단추로 클릭합니다.그런 다음 상황에 맞는 메뉴에서 **WCF 구성 편집** 항목을 선택합니다.그러면 그래픽 사용자 인터페이스를 통해 [!INCLUDE[indigo2](../../../../../includes/indigo2-md.md)] 서비스의 구성 설정을 수정할 수 있는 [Configuration Editor 도구\(SvcConfigEditor.exe\)](../../../../../docs/framework/wcf/configuration-editor-tool-svcconfigeditor-exe.md)가 시작됩니다.  
...
```  
 ...
 사용자 정의 추적 소스 생성[!INCLUDE[crabout](../../../../../includes/crabout-md.md)][추적 확장](../../../../../docs/framework/wcf/samples/extending-tracing.md)을 참조하십시오.  
...
## 추적을 사용하도록 추적 수신기 구성  
 ...
 추적 수신기를 프로그래밍 방식으로 구성할 수도 있습니다.[!INCLUDE[crdefault](../../../../../includes/crdefault-md.md)][방법: 추적 수신기 만들기 및 초기화](http://go.microsoft.com/fwlink/?LinkId=94648) 및 [Creating a Custom TraceListener](http://go.microsoft.com/fwlink/?LinkId=96239)를 참조하십시오.  
...
|-----------|----------------|----------------|------------|------------|  
|Off|해당 없음|N\/A|내보낸 추적이 없습니다.|N\/A|  
|Critical|"Negative" 이벤트: 예기치 않은 처리 또는 오류 조건을 나타내는 이벤트||다음을 포함하여 처리되지 않은 예외를 기록합니다.<br /><br /> -   OutOfMemoryException<br />-   ThreadAbortException\(CLR이 ThreadAbortExceptionHandler를 호출함\)<br />-   StackOverflowException\(catch할 수 없음\)<br />-   ConfigurationErrorsException<br />-   SEHException<br />-   응용 프로그램 시작 오류<br />-   Failfast 이벤트<br />-   시스템 중단<br />-   포이즌 메시지: 응용 프로그램 오류를 일으킨 메시지 추적입니다.|관리자<br /><br /> 응용 프로그램 개발자|  
|Error|"Negative" 이벤트: 예기치 않은 처리 또는 오류 조건을 나타내는 이벤트|예기치 않은 처리가 발생했습니다.응용 프로그램이 예상한 대로 작업을 수행하지 못했습니다.그러나 응용 프로그램이 여전히 실행 중입니다.|모든 예외가 기록됩니다.|관리자<br /><br /> 응용 프로그램 개발자|  
|Warning|"Negative" 이벤트: 예기치 않은 처리 또는 오류 조건을 나타내는 이벤트|문제가 발생했거나 발생할 수도 있지만 응용 프로그램이 여전히 올바르게 작동합니다.그러나 계속 올바르게 작동하지 않을 수도 있습니다.|-   응용 프로그램이 스로틀 설정이 허용하는 것보다 많은 요청을 받고 있습니다.<br />-   수신 큐가 구성된 최대 용량에 근접합니다.<br />-   시간 제한이 초과되었습니다.<br />-   자격 증명이 거부되었습니다.|관리자<br /><br /> 응용 프로그램 개발자|  
|Information|"Positive" 이벤트: 성공적인 중요 시점을 표시하는 이벤트|응용 프로그램이 올바르게 작동하는지 여부에 관계 없이 응용 프로그램 실행의 중요한 중요 시점과 성공적인 중요 시점입니다.|일반적으로 시스템 상태 모니터링 및 진단, 성능 측정 또는 프로파일링에 유용한 메시지가 생성됩니다.용량 계획 및 성능 관리에 이러한 정보를 사용할 수 있습니다.<br /><br /> -   채널이 만들어집니다.<br />-   끝점 수신기가 만들어집니다.<br />-   메시지 전송을 시작\/종료합니다.<br />-   보안 토큰이 검색되었습니다.<br />-   구성 설정을 읽습니다.|관리자<br /><br /> 응용 프로그램 개발자<br /><br /> 제품 개발자|  
|Verbose|"Positive" 이벤트: 성공적인 중요 시점을 표시하는 이벤트|사용자 코드와 서비스에 대한 하위 수준 이벤트를 내보냅니다.|일반적으로 디버깅 또는 응용 프로그램 최적화에 이 수준을 사용할 수 있습니다.<br /><br /> -   메시지 헤더를 인식했습니다.|관리자<br /><br /> 응용 프로그램 개발자<br /><br /> 제품 개발자|  
|ActivityTracing||처리 동작과 구성 요소 간의 흐름 이벤트입니다.|이 수준에서 관리자와 개발자가 같은 응용 프로그램 도메인의 응용 프로그램을 서로 연결할 수 있습니다.<br /><br /> -   시작\/중지와 같은 동작 경계 추적<br />-   전송 추적|모두|  
|All||응용 프로그램이 올바르게 작동할 수 있습니다.모든 이벤트를 내보냅니다.|앞의 이벤트 모두|모두|  
...
## 상관 관계를 위한 동작 추적 및 전파 구성  
 `switchValue` 특성에 대한 지정된 `activityTracing` 값은 끝점 내에서 동작 경계와 전송에 대한 추적을 내보내는 동작 추적을 활성화하는 데 사용됩니다.  
...
>  [!INCLUDE[indigo2](../../../../../includes/indigo2-md.md)]에서 특정 확장성 기능을 사용하는 경우 동작 추적이 활성화될 때 <xref:System.NullReferenceException>을 가져올 수 있습니다.이 문제를 해결하려면 응용 프로그램의 구성 파일을 확인하고 추적 소스의 `switchValue` 특성이 `activityTracing`으로 설정되지 않았는지 확인합니다.  
 ...
 동작 추적 및 전파[!INCLUDE[crabout](../../../../../includes/crabout-md.md)][전파](../../../../../docs/framework/wcf/diagnostics/tracing/propagation.md)를 참조하십시오.  
  
 `propagateActivity` 및 `ActivityTracing`부울 값은 모두 System.ServiceModel TraceSource에 적용됩니다.`ActivityTracing`값도 [!INCLUDE[indigo2](../../../../../includes/indigo2-md.md)]또는 사용자 정의 추적 소스를 포함한 모든 추적 소스에 적용됩니다.  

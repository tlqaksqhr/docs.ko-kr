---
title: "사용자 지정 스트림 업그레이드 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
1.  업그레이드 스트림 *개시자*는 프로세스를 시작합니다. 런타임에 연결 다른 쪽 끝에 대한 요청을 시작하여 채널 전송 계층을 업그레이드할 수 있습니다.  
  
2.  업그레이드 스트림 *수락자*는 업그레이드를 실행합니다. 런타임에 다른 쪽 컴퓨터에서 업그레이드 요청을 받고 가능한 경우 업그레이드를 수락합니다.  
  
3.  업그레이드 *공급자*는 클라이언트에 *개시자*를 만들고 서버에 *수락자*를 만듭니다.  
  
4.  스트림 업그레이드 *바인딩 요소*는 서비스와 클라이언트의 바인딩에 추가되고 런타임에 공급자를 만듭니다.  
...
2.  압축 스트림과 보안 스트림의 콘텐츠 형식을 순서대로 반환하도록 <xref:System.ServiceModel.Channels.StreamUpgradeInitiator.GetNextUpgrade%2A> 메서드를 재정의하여 <xref:System.ServiceModel.Channels.StreamSecurityUpgradeInitiator>를 서브클래싱합니다.  
  
3.  <xref:System.ServiceModel.Channels.StreamUpgradeAcceptor.CanUpgrade%2A> 메서드의 사용자 지정 콘텐츠 형식을 이해하는 <xref:System.ServiceModel.Channels.StreamSecurityUpgradeAcceptor>를 서브클래싱합니다.  

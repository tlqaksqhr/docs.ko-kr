---
title: "메시지 교환 패턴 선택 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# 메시지 교환 패턴 선택
사용자 지정 전송을 작성하는 첫 번째 단계는 개발 중인 채널에 필요한 *메시지 교환 패턴*\(즉, MEP\)을 결정하는 것입니다.이 항목에서는 사용 가능한 옵션과 여러 가지 요구 사항에 대해 설명합니다.이 작업은 [채널 개발](../../../../docs/framework/wcf/extending/developing-channels.md)에 설명된 채널 개발 작업 목록의 첫 번째 작업입니다.  
...
-   이중\(<xref:System.ServiceModel.Channels.IDuplexChannel>\)  
 ...
 ![메시지 교환 패턴 선택](../../../../docs/framework/wcf/extending/media/wcfc-basicthreemepsc.gif "wcfc\_BasicThreeMEPsc")  
다음은 세 가지 기본 메시지 교환 패턴으로위에서부터 차례로 데이터그램, 요청\-응답 및 이중 교환 패턴입니다.  
  
 또한 이러한 MEP는 각각 *세션*을 지원합니다.세션\(및 <xref:System.ServiceModel.Channels.ISession?displayProperty=fullName> 형식의 <xref:System.ServiceModel.Channels.ISessionChannel%601?displayProperty=fullName> 구현\)은 채널에서 주고 받는 모든 메시지와 상호 관련됩니다.요청\-응답 패턴은 요청과 회신이 상호 관련되어 있지만 독립 실행형 두 메시지 세션입니다.반면, 세션을 지원하는 요청\-응답 패턴은 해당 채널의 모든 요청\/응답 쌍이 상호 관련되어 있음을 의미합니다.이런 식으로 다음과 같이 총 여섯 개의 MEP가 제공됩니다.  
...
## 세션 및 세션 채널  
 ...
 ![메시지 교환 패턴 선택](../../../../docs/framework/wcf/extending/media/wcfc-sessionandsessionlesschannelsc.gif "wcfc\_SessionAndSessionlessChannelsc")  
...
## 세션 시작 및 종료  
 ...
 일반적으로 세션은 들어오는 쪽이 아닌 나가는 쪽에서 닫힙니다.즉, 세션 출력 채널을 닫아 세션을 완전히 종료할 수 있습니다.세션 출력 채널을 닫으면 해당 세션 입력 채널에서는 <xref:System.ServiceModel.Channels.IDuplexSessionChannel>의 <xref:System.ServiceModel.Channels.IInputChannel.Receive%2A?displayProperty=fullName>를 호출하는 응용 프로그램에 null을 반환합니다.  
  
 하지만 세션 입력 채널은 <xref:System.ServiceModel.Channels.IDuplexSessionChannel>의 <xref:System.ServiceModel.Channels.IInputChannel.Receive%2A?displayProperty=fullName>가세션이 이미 닫혀 있음을 나타내는 null을 반환하지 않는 이상 닫히지 않습니다.<xref:System.ServiceModel.Channels.IDuplexSessionChannel>의 <xref:System.ServiceModel.Channels.IInputChannel.Receive%2A?displayProperty=fullName>가null을 반환하지 않은 상태에서 세션 입력 채널을 닫으면 채널을 닫는 동안 예기치 않은 메시지를 받을 수 있으므로 예외가 throw됩니다.발신자가 세션을 종료하기 전에 수신자가 세션을 종료하려면 갑자기 세션이 종료되는 <xref:System.ServiceModel.ICommunicationObject.Abort%2A>를 입력 채널에서 호출해야 합니다.  
...
-   채널에서 <xref:System.ServiceModel.ICommunicationObject.Close%2A?displayProperty=fullName>가 호출되면 지정된 시간 제한이나 기본 시간 제한을 사용하여 세션을 닫는 데 필요한 작업을 수행합니다.이 작업은 세션을 하위 채널에서 얻은 경우 해당 채널에서 <xref:System.ServiceModel.ICommunicationObject.Close%2A>를 호출하거나 특별한 SOAP 메시지를 보내거나 전송 연결을 닫는 작업만큼이나 간단합니다.  
...
-   들어오는 각 메시지에 대해, 채널 수신기에서 해당 메시지가 속한 세션을 감지해야 합니다.세션의 첫 번째 메시지에 대해서는 채널 수신기가 새 채널을 만들고 <xref:System.ServiceModel.Channels.IChannelListener%601.AcceptChannel%2A?displayProperty=fullName>에 대한 호출에서 이를 반환해야 합니다.그렇지 않으면 채널 수신기가 세션에 해당하는 기존 채널을 찾아 해당 채널을 통해 메시지를 전달해야 합니다.  

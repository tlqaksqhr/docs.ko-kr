---
title: "연습: 사용자 지정 클라이언트 및 서비스 자격 증명 만들기 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## 자격 증명 확장성 클래스  
 <xref:System.ServiceModel.Description.ClientCredentials> 및 <xref:System.ServiceModel.Description.ServiceCredentials> 클래스는 [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 보안 확장성에 대한 주요 진입점입니다.이러한 자격 증명 클래스는 API를 제공합니다. 이 API를 통해 응용 프로그램 코드에서는 자격 증명 정보를 설정하고, 자격 증명 형식을 보안 토큰으로 변환할 수 있습니다.\(*보안 토큰*은 SOAP 메시지 내부에서 자격 증명 정보를 전송하는 데 사용하는 형식입니다.\) 이러한 자격 증명 클래스의 책임은 다음과 같은 두 가지 영역으로 나눠 볼 수 있습니다.  
  
-   응용 프로그램에서 자격 증명 정보를 설정하도록 API 제공  
  
-   <xref:System.IdentityModel.Selectors.SecurityTokenManager> 구현을 위한 팩터리 역할 수행  
  
 <xref:System.ServiceModel.Description.ClientCredentials> 및 <xref:System.ServiceModel.Description.ServiceCredentials> 클래스는 모두 <xref:System.IdentityModel.Selectors.SecurityTokenManager> 반환에 대한 계약을 정의하는 추상 <xref:System.ServiceModel.Security.SecurityCredentialsManager> 클래스로부터 상속됩니다.  
  
 자격 증명 클래스 및 이 클래스가 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 보안 아키텍처에 속하는 방법[!INCLUDE[crabout](../../../../includes/crabout-md.md)][Security Architecture](http://msdn.microsoft.com/ko-kr/16593476-d36a-408d-808c-ae6fd483e28f)를 참조하십시오.  
...
2.  선택적 요소로서,새 자격 증명 형식에 대해 새 메서드 또는 속성을 추가합니다.새 자격 증명 형식을 추가하지 않으려면 이 단계를 건너뜁니다.다음 예제에서는 `CreditCardNumber` 속성을 추가합니다.  
...
2.  선택적 요소로서,사용자 지정 <xref:System.IdentityModel.Selectors.SecurityTokenProvider> 구현을 만들어야 하는 경우 <xref:System.IdentityModel.Selectors.SecurityTokenManager.CreateSecurityTokenProvider%28System.IdentityModel.Selectors.SecurityTokenRequirement%29> 메서드를 재정의합니다.사용자 지정 보안 토큰 공급자에 대한 [!INCLUDE[crabout](../../../../includes/crabout-md.md)]는 [방법: 사용자 지정 보안 토큰 공급자 만들기](../../../../docs/framework/wcf/extending/how-to-create-a-custom-security-token-provider.md)를 참조하십시오.  
  
3.  선택적 요소로서,사용자 지정 <xref:System.IdentityModel.Selectors.SecurityTokenAuthenticator> 구현을 만들어야 하는 경우 <xref:System.IdentityModel.Selectors.SecurityTokenManager.CreateSecurityTokenAuthenticator%28System.IdentityModel.Selectors.SecurityTokenRequirement%2CSystem.IdentityModel.Selectors.SecurityTokenResolver%40%29> 메서드를 재정의합니다.사용자 지정 보안 토큰 인증자에 대한 [!INCLUDE[crabout](../../../../includes/crabout-md.md)]는 [방법: 사용자 지정 보안 토큰 인증자 만들기](../../../../docs/framework/wcf/extending/how-to-create-a-custom-security-token-authenticator.md)을 참조하십시오.  
  
4.  선택적 요소로서,사용자 지정 <xref:System.IdentityModel.Selectors.SecurityTokenSerializer>를 만들어야 하는 경우 <xref:System.IdentityModel.Selectors.SecurityTokenManager.CreateSecurityTokenSerializer%2A> 메서드를 재정의합니다.사용자 지정 보안 토큰 및 사용자 지정 보안 토큰 serializer에 대한 [!INCLUDE[crabout](../../../../includes/crabout-md.md)]는 [방법: 사용자 지정 토큰 만들기](../../../../docs/framework/wcf/extending/how-to-create-a-custom-token.md)를 참조하십시오.  
...
2.  <xref:System.ServiceModel.ChannelFactory.Endpoint%2A> 속성을 통해 액세스할 수 있는 <xref:System.ServiceModel.Description.ServiceEndpoint.Behaviors%2A> 컬렉션으로부터 시스템 제공 클라이언트 자격 증명 동작을 제거합니다.  
  
3.  사용자 지정 클라이언트 자격 증명 클래스의 새 인스턴스를 만들고 이 인스턴스를 <xref:System.ServiceModel.ChannelFactory.Endpoint%2A> 속성을 통해 액세스할 수 있는 <xref:System.ServiceModel.Description.ServiceEndpoint.Behaviors%2A> 컬렉션에 추가합니다.  
...
2.  선택적 요소로서,응용 프로그램 구성을 통해 노출하려고 하는 모든 추가 구성 매개 변수에 대한 속성을 추가합니다.아래 예제에서는 이름이 `CreditCardNumber`인 속성 하나를 추가합니다.  
...
5.  선택적 요소입니다.2단계에서 속성을 추가한 경우 구성 프레임워크에 대한 추가 구성 설정을 등록하여 이를 인식할 수 있도록 하려면 <xref:System.Configuration.ConfigurationElement.Properties%2A> 속성을 재정의해야 합니다.사용자의 속성과 기본 클래스 속성을 조합하면 이 사용자 지정 클라이언트 자격 증명 구성 요소를 통해 시스템 제공 설정을 구성할 수 있습니다.  
...
1.  \<`extensions`\> 요소 및 \<`behaviorExtensions`\> 요소를 구성 파일에 추가합니다.  
...
4.  구성 처리기를 등록한 후에 사용자 지정 자격 증명 요소를 시스템 제공 \<`clientCredentials`\> 요소 대신에 동일한 구성 파일 내에서 사용할 수 있습니다.시스템 제공 속성 및 구성 처리기 구현에 추가한 새 속성을 모두 사용할 수 있습니다.다음 예제에서는 `creditCardNumber` 특성을 사용하여 사용자 지정 속성의 값을 설정합니다.  
...
2.  선택적 요소로서,추가 중인 새 자격 증명 값에 대한 API를 제공하려면 새 속성을 추가합니다.새 자격 증명 값을 추가하지 않으려면 이 단계를 건너뜁니다.다음 예제에서는 `AdditionalCertificate` 속성을 추가합니다.  
...
4.  선택적 요소로서,<xref:System.ServiceModel.Description.ServiceCredentials.CloneCore%2A> 메서드를 재정의합니다.이는 새 속성 또는 내부 필드를 사용자 지정 클라이언트 자격 증명 구현에 추가하는 경우에만 필요합니다.  
...
2.  선택적 요소로서,사용자 지정 <xref:System.IdentityModel.Selectors.SecurityTokenProvider> 구현을 만들어야 하는 경우 <xref:System.IdentityModel.Selectors.SecurityTokenManager.CreateSecurityTokenProvider%2A> 메서드를 재정의합니다.사용자 지정 보안 토큰 공급자에 대한 [!INCLUDE[crabout](../../../../includes/crabout-md.md)]는 [방법: 사용자 지정 보안 토큰 공급자 만들기](../../../../docs/framework/wcf/extending/how-to-create-a-custom-security-token-provider.md)를 참조하십시오.  
  
3.  선택적 요소로서,사용자 지정 <xref:System.IdentityModel.Selectors.SecurityTokenAuthenticator> 구현을 만들어야 하는 경우 <xref:System.IdentityModel.Selectors.SecurityTokenManager.CreateSecurityTokenAuthenticator%2A> 메서드를 재정의합니다.사용자 지정 보안 토큰 인증자에 대한 [!INCLUDE[crabout](../../../../includes/crabout-md.md)]는 [방법: 사용자 지정 보안 토큰 인증자 만들기](../../../../docs/framework/wcf/extending/how-to-create-a-custom-security-token-authenticator.md) 항목을 참조하십시오.  
  
4.  선택적 요소로서,사용자 지정 <xref:System.IdentityModel.Selectors.SecurityTokenSerializer>를 만들어야 하는 경우 <xref:System.IdentityModel.Selectors.SecurityTokenManager.CreateSecurityTokenSerializer%28System.IdentityModel.Selectors.SecurityTokenVersion%29> 메서드를 재정의합니다.사용자 지정 보안 토큰 및 사용자 지정 보안 토큰 serializer에 대한 [!INCLUDE[crabout](../../../../includes/crabout-md.md)]는 [방법: 사용자 지정 토큰 만들기](../../../../docs/framework/wcf/extending/how-to-create-a-custom-token.md)를 참조하십시오.  

...
-   <xref:System.IdentityModel.Selectors.SecurityTokenManager> 구현을 위한 팩터리 역할 수행  
 ...
 자격 증명 클래스와 WCF 보안 아키텍처에 적합 하 게 하는 방법에 대 한 자세한 내용은 참조 [보안 아키텍처](http://msdn.microsoft.com/library/16593476-d36a-408d-808c-ae6fd483e28f)합니다.  
  
 WCF에서 제공 하는 기본 구현을 시스템 제공 자격 증명 형식을 지원 하 고 보안 토큰 관리자는 이러한 자격 증명 형식을 처리할 수 있는 만듭니다.  
  
## <a name="reasons-to-customize"></a>사용자 지정해야 하는 이유  
 클라이언트 또는 서비스 자격 증명 클래스를 사용자 지정해야 하는 이유는 여러 가지가 있습니다. 무엇 보다도 특히 다음과 같은 이유로 시스템 제공 자격 증명 형식을 처리와 관련 하 여 기본 WCF 보안 동작을 변경 하는 요구 사항을입니다.  
...
## <a name="first-in-a-series"></a>시리즈의 첫 번째 단계  
 자격 증명 구축, 보안 토큰 serialization 또는 인증에 대 한 WCF 동작을 변경 하는 자격 증명을 사용자 지정 하기 위한 이유 때문에 첫 번째 단계는 사용자 지정 자격 증명 클래스를 만드는 합니다. 이 단원의 다른 항목에서는 사용자 지정 serializer 및 인증자를 만드는 방법에 대해 설명합니다. 이와 관련하여 사용자 지정 자격 증명 클래스를 만드는 것이 이 시리즈의 첫 번째 항목입니다. 후속 작업(사용자 지정 serializer 및 인증자 만들기)은 사용자 지정 자격 증명을 만든 이후에야 수행할 수 있습니다. 이 항목을 기초로 한 추가 항목은 다음과 같습니다.  
...
3.  <xref:System.ServiceModel.Security.SecurityCredentialsManager.CreateSecurityTokenManager%2A> 메서드를 재정의합니다. 이 메서드는 사용자 지정 클라이언트 자격 증명을 사용할 때 WCF 보안 인프라에서 자동으로 호출 됩니다. 이 메서드는 <xref:System.IdentityModel.Selectors.SecurityTokenManager> 클래스 구현의 인스턴스를 만들고 반환합니다.  
...
5.  선택 사항입니다. 2단계에서 속성을 추가한 경우 구성 프레임워크에 대한 추가 구성 설정을 등록하여 이를 인식할 수 있도록 하려면 <xref:System.Configuration.ConfigurationElement.Properties%2A> 속성을 재정의해야 합니다. 사용자의 속성과 기본 클래스 속성을 조합하면 이 사용자 지정 클라이언트 자격 증명 구성 요소를 통해 시스템 제공 설정을 구성할 수 있습니다.  
 ...
 구성 처리기 클래스를 만든 후에 WCF 구성 프레임 워크에 통합할 수 있습니다. 이를 통해 다음 절차에서처럼 사용자 지정 클라이언트 자격 증명을 클라이언트 끝점 동작 요소에서 사용할 수 있습니다.  
...
3.  <xref:System.ServiceModel.Security.SecurityCredentialsManager.CreateSecurityTokenManager%2A> 메서드를 재정의합니다. 이 메서드는 사용자 지정 클라이언트 자격 증명을 사용할 때 WCF 인프라에서 자동으로 호출 됩니다. 이 메서드는 <xref:System.IdentityModel.Selectors.SecurityTokenManager> 클래스 구현의 인스턴스를 만들고 반환하며, 이에 대해서는 다음 절차에서 설명합니다.  

---
title: "사용자 정의 바인딩 만들기 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   바인딩 요소로 채워지는 컨테이너인 <xref:System.ServiceModel.Channels.CustomBinding> 클래스에 따라 사용자 지정 바인딩을 만듭니다.  그런 다음 사용자 지정 바인딩을 서비스 끝점에 추가합니다.  프로그램 방식이나 응용 프로그램 구성 파일을 통해 사용자 지정 바인딩을 만들 수 있습니다.  응용 프로그램 구성 파일에서 바인딩 요소를 사용하려면 바인딩 요소가 <xref:System.ServiceModel.Configuration.BindingElementExtensionElement>를 확장해야 합니다.  사용자 지정 바인딩[!INCLUDE[crabout](../../../../includes/crabout-md.md)] [사용자 지정 바인딩](../../../../docs/framework/wcf/extending/custom-bindings.md) 및 <xref:System.ServiceModel.Channels.CustomBinding>을 참조하세요.  
...
|계층|옵션|필수|  
|--------|--------|--------|  
|트랜잭션 흐름|<xref:System.ServiceModel.Channels.TransactionFlowBindingElement?displayProperty=fullName>|아니요|  
|안정성|<xref:System.ServiceModel.Channels.ReliableSessionBindingElement?displayProperty=fullName>|아니요|  
|보안|<xref:System.ServiceModel.Channels.SecurityBindingElement?displayProperty=fullName>|아니요|  
|복합 이중|<xref:System.ServiceModel.Channels.CompositeDuplexBindingElement?displayProperty=fullName>|아니요|  
...
## 새 바인딩 요소 만들기  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]가 제공하는 <xref:System.ServiceModel.Channels.BindingElement>에서 파생되는 형식 이외에 바인딩 요소를 만들 수 있습니다.  따라서 바인딩 스택을 만드는 방법 및 스택에서 기타 시스템 제공 형식으로 구성할 수 있는 <xref:System.ServiceModel.Channels.BindingElement>를 만들어 이 스택에 삽입되는 구성 요소를 사용자 지정할 수 있습니다.  
  
 예를 들어, 메시지를 데이터베이스에 기록하는 기능을 제공하는 `LoggingBindingElement`를 구현하는 경우 채널 스택의 전송 채널 위에 요소를 배치해야 합니다.  이 경우 응용 프로그램은 다음 예제에서처럼 `TcpTransportBindingElement`로 `LoggingBindingElement`를 구성한 사용자 지정 바인딩을 만듭니다.  
...
```  
 ...
 새 바인딩 요소를 작성하는 방법은 정확한 기능에 따라 달라집니다.  샘플 중 하나인 [전송: UDP](../../../../docs/framework/wcf/samples/transport-udp.md)에서는 한 종류의 바인딩 요소를 구현하는 방법에 대해 자세히 설명합니다.  
...
## 새 바인딩 만들기  
 ...
 둘 이상의 응용 프로그램에서 바인딩을 사용하는 경우 직접 바인딩을 만들어 <xref:System.ServiceModel.Channels.Binding>을 확장합니다.  이렇게 하면 사용할 때마다 사용자 지정 바인딩을 수동으로 만들지 않아도 됩니다.  사용자 정의 바인딩을 사용하여 바인딩의 동작을 정의하고 사용자 정의 바인딩 요소를 포함시킬 수 있습니다.  이는 *사전 패키지*로 구성되어 있으므로 사용할 때마다 바인딩을 다시 빌드할 필요가 없습니다.  
 ...
 <xref:System.ServiceModel.Channels.Binding.CreateBindingElements%2A> 메서드는 바인딩의 바인딩 요소가 포함된 새 <xref:System.ServiceModel.Channels.BindingElementCollection>을 반환합니다.  컬렉션에는 먼저 프로토콜 바인딩 요소 그리고 인코딩 바인딩 요소와 전송 바인딩 요소가 순서대로 포함되어 있어야 합니다.  [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 시스템 제공 바인딩 요소를 사용하는 경우 [사용자 지정 바인딩](../../../../docs/framework/wcf/extending/custom-bindings.md)에 지정된 바인딩 요소 순서 지정 규칙을 따라야 합니다.  이 컬렉션은 사용자 정의 바인딩 클래스 내에서 참조된 개체를 참조하지 않아야 합니다. 따라서 바인딩 작성자는 <xref:System.ServiceModel.Channels.Binding.CreateBindingElements%2A>를 호출할 때마다 <xref:System.ServiceModel.Channels.BindingElementCollection>의 `Clone()`을 반환해야 합니다.  
  
 <xref:System.ServiceModel.Channels.Binding.Scheme%2A> 속성은 바인딩에서 사용 중인 전송 프로토콜의 URI 스키마를 나타냅니다.  예를 들어 *WSHttpBinding* 및 *NetTcpBinding*은 “http” 및 “net.tcp”를 각각의 <xref:System.ServiceModel.Channels.Binding.Scheme%2A> 속성에서 반환합니다.  
  
 사용자 정의 인코딩 바인딩에 대한 선택적 메서드 및 속성의 전체 목록은 <xref:System.ServiceModel.Channels.Binding>을 참조하세요.  
  
### 예제  
 이 예제에서는 <xref:System.ServiceModel.Channels.Binding>에서 파생되는 `SampleProfileUdpBinding`의 프로필 바인딩을 구현합니다.  `SampleProfileUdpBinding`에는 최대 4개의 바인딩 요소가 포함됩니다. 하나는 사용자가 만든 `UdpTransportBindingElement`이고 나머지 3개는 시스템 제공 `TextMessageEncodingBindingElement`, `CompositeDuplexBindingElement` 및 `ReliableSessionBindingElement`입니다.  
...
### 원샷 보안  
 \<message\> 구성 요소의 `negotiateServiceCredential` 특성을 `false`로 설정하여 필요한 모든 보안 자격 증명이 하나의 메시지로 전송되는 "원샷" 보안을 구현할 수 있습니다.  

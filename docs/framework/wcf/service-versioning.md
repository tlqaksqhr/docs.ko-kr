---
title: "서비스 버전 관리 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   구현 변경: 예를 들어 내부 메서드 구현이 변경됩니다.  
 ...
 이러한 변경 중 일부는 "주요" 변경 내용이고, 나머지는 "주요하지 않은" 변경 내용입니다. 이전 버전에서 처리된 모든 메시지가 새 버전에서도 처리된다면 이 변경 내용은 *주요하지 않은* 변경 내용입니다.해당 조건을 충족하지 않는 모든 변경 내용은 *주요* 변경 내용입니다.  
...
### Lax 버전 관리  
 ...
 [!INCLUDE[indigo2](../../../includes/indigo2-md.md)], ASP.NET 웹 서비스 및 많은 기타 웹 서비스 스택은 *lax 버전 관리*를 지원합니다. 즉, 수신된 데이터에서 알려지지 않은 새 데이터 멤버에 대해 예외를 throw하지 않습니다.  
 ...
 데이터 계약의 lax 버전 관리 및 엄격한 버전 관리에 대한 자세한 지침은 [최선의 방법: 데이터 계약 버전 관리](../../../docs/framework/wcf/best-practices-data-contract-versioning.md)를 참조하십시오.  
...
### 이름, 네임스페이스 및 동작 지정  
 기본적으로 서비스 계약의 이름은 인터페이스의 이름입니다.이 기본 네임스페이스는 "http:\/\/tempuri.org"이며, 각 작업의 동작은 "http:\/\/tempuri.org\/contractname\/methodname"입니다.서비스 계약에 대한 이름 및 네임스페이스, 각 작업에 대한 동작을 명시적으로 지정하여 "http:\/\/tempuri.org"의 사용을 피하고, 인터페이스와 메서드 이름이 서비스 계약에서 노출되지 않도록 하는 것이 좋습니다.  
...
## 라우팅 서비스 및 버전 관리  
 서비스 변경이 주요한 변경이고 서로 다른 버전의 서비스를 두 개 이상 동시에 실행해야 하면 WCF 라우팅 서비스를 사용하여 적절한 서비스 인스턴스에 메시지를 라우팅할 수 있습니다.WCF 라우팅 서비스는 콘텐츠 기반 라우팅을 사용합니다. 즉, 메시지 내의 정보를 사용하여 메시지를 라우팅할 위치를 확인합니다.WCF 라우팅 서비스에 대한 [!INCLUDE[crabout](../../../includes/crabout-md.md)]는 [라우팅 서비스](../../../docs/framework/wcf/feature-details/routing-service.md)를 참조하십시오.서비스 버전 관리에 WCF 라우팅 서비스를 사용하는 방법에 대한 예제는 [방법: 서비스 버전 관리](../../../docs/framework/wcf/feature-details/how-to-service-versioning.md)를 참조하십시오.  

...
*자격 증명* 요구 된 id 또는 기능을 설정 하기 위해 Windows Communication Foundation (WCF) 사용 하는 데이터입니다. 예를 들어 여권은 정부에서 국가나 지역의 시민권을 입증하기 위해 발급하는 자격 증명입니다. WCF, 자격 증명 사용자 이름 토큰 및 X.509 인증서 같은 다양 한 형태를 취할 수 있습니다. 이 항목에서는 자격 증명, WCF에서 어떻게 사용 되는지 및 응용 프로그램에 대 한 적합 한 자격 증명을 선택 하는 방법에 설명 합니다.  
 ...
 자격 증명을 제공할 때는 데이터와 데이터 소유 증명을 모두 제공해야 합니다. WCF에는 다양 한 전송 및 메시지 보안 수준에서 자격 증명 형식 지원합니다. 예를 들어 두 가지 유형의 WCF에서 지원 되는 자격 증명: 사용자 이름 및 (X.509) 인증서 자격 증명입니다.  
...
|Ntlm|NTLM(NT LAN Manager) 인증을 지정합니다. 이 인증은 어떤 이유로 Kerberos 인증을 사용할 수 없는 경우에 사용됩니다. 설정 하 여 한 대체 방식으로 용도 비활성화할 수도 있습니다는 <xref:System.ServiceModel.Security.WindowsClientCredential.AllowNtlm%2A> 속성을 `false`, 있는 경우 WCF NTLM을 사용 하는 경우 예외를 throw 하는 최상의 노력을 확인 합니다. 이 속성을 `false`로 설정하면 유선을 통해 NTLM 자격 증명을 보낼 수 있습니다.|  
...
|Username|서비스에서 사용자 이름 자격 증명을 사용하여 클라이언트를 인증하도록 요구할 수 있습니다. 참고 WCF 사용자 이름, 서명 생성, 데이터 암호화 등과 같은 암호화 작업을 허용 하지 않습니다. WCF는 사용자 이름 자격 증명을 사용 하는 경우에 전송 보안을 보장 합니다.|  
 ...
 *협상* 자격 증명을 교환 하 여 클라이언트와 서비스 간의 신뢰를 설정 하는 프로세스입니다. 이 프로세스는 협상 프로세스의 다음 단계에 필요한 정보만 공개하도록 클라이언트와 서비스 간에 반복적으로 수행됩니다. 실제로 최종 결과는 후속 작업에 사용할 서비스의 자격 증명이 클라이언트에 전달됩니다.  
 ...
 단, 기본적으로 시스템 제공 바인딩은 wcf에서 서비스 자격 증명을 협상 자동으로 메시지 수준 보안을 사용 하는 경우. 예외는 기본적으로 보안을 사용하지 않는 <xref:System.ServiceModel.BasicHttpBinding>입니다. 이 동작을 사용하지 않도록 설정하려면 <xref:System.ServiceModel.MessageSecurityOverHttp.NegotiateServiceCredential%2A> 및 <xref:System.ServiceModel.FederatedMessageSecurityOverHttp.NegotiateServiceCredential%2A> 속성을 참조하세요.  
  
> [!NOTE]
>  .NET Framework 3.5 이상 SSL 보안이 사용 되는 경우 WCF 클라이언트를 사용 하 여 SSL 협상 중에 받은 중간 인증서와 인증서 저장소에서의 중간 인증서를 모두 서비스의 인증서 체인 유효성 검사를 수행 합니다. 인증서입니다. .NET Framework 3.0은 로컬 인증서 저장소에 설치된 중간 인증서만 사용합니다.  
...
### <a name="setting-client-credentials"></a>클라이언트 자격 증명 설정  
 WCF, 클라이언트 응용 프로그램 서비스에 연결 하는 WCF 클라이언트를 사용 합니다. 모든 클라이언트는 <xref:System.ServiceModel.ClientBase%601> 클래스에서 파생되고, 클라이언트의 <xref:System.ServiceModel.ClientBase%601.ClientCredentials%2A> 속성은 클라이언트 자격 증명의 다양한 값을 지정하는 데 사용됩니다.  
...
-   클라이언트 자격 증명은 WCF 클라이언트 인스턴스를 사용 하 여 보안 컨텍스트를 설정 하는 첫 번째 메시지를 보내기 전에 한 번만 사용 됩니다. 그런 다음 모든 응용 프로그램 메시지는 보안 컨텍스트를 통해 보안이 유지됩니다.  
...
> [!IMPORTANT]
>  ID를 전환할 수 없을 때(보안 컨텍스트 설정이 켜진 경우의 기본 동작) 주의해야 할 경우가 있습니다. 두 번째 서비스와 통신 하는 서비스를 만드는 경우에 WCF 클라이언트를 두 번째 서비스에 여는 데 사용 된 id는 변경할 수 없습니다. 여러 클라이언트가 첫 번째 서비스를 사용할 수 있고 두 번째 서비스에 액세스할 때 서비스가 클라이언트를 가장하는 경우 이것이 문제가 됩니다. 서비스가 모든 호출자에 대해 동일한 클라이언트를 다시 사용하는 경우 두 번째 서비스에 대한 모든 호출은 클라이언트를 두 번째 서비스에 여는 데 사용된 첫 번째 호출자의 ID로 수행됩니다. 즉, 서비스는 모든 클라이언트에 대해 첫 번째 클라이언트의 ID를 사용하여 두 번째 서비스와 통신합니다. 이 경우 권한 상승이 발생할 수 있습니다. 원하는 서비스 동작이 아닌 경우 각 호출자를 추적하여 각 호출자와 관련해서 두 번째 서비스에 대해 새 클라이언트를 만들고 서비스가 올바른 호출자에 대해 올바른 클라이언트만 사용하여 두 번째 서비스와 통신하도록 해야 합니다.  

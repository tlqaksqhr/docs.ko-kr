---
title: "사용자 지정 필터 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
> [!NOTE]
>  실제 구현에서는 Match 메서드에 이 메시지 필터가 **true**를 반환하는지 아니면 **false**를 반환하는지를 확인하기 위해 메시지를 검사하는 논리가 포함됩니다.  
...
## 사용법  
 라우팅 서비스에서 사용자 지정 필터를 사용하려면 “사용자 지정” 형식의 새 필터 항목, 메시지 필터의 정규화된 이름 및 어셈블리 이름을 지정하여 필터 테이블에 사용자 지정 필터를 추가해야 합니다.다른 MessageFilter와 마찬가지로 사용자 지정 필터의 생성자에게 전달될 filterData 문자열을 지정할 수 있습니다.  

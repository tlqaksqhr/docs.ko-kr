...
1.  실행의 [ServiceModel Metadata 유틸리티 도구 (Svcutil.exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) 둘 다와 함께 도구는 `/async` 및 `/tcv:Version35` 다음 명령에 표시 된 대로 옵션을 함께 명령입니다.  
    ...
    ```  
     ...
     이 생성 하면 동기 작업 및 표준 대리자 기반 비동기 작업 외에도 포함 하는 WCF 클라이언트 클래스:  
  
    -   두 개의 <`operationName` > `Async` 이벤트 기반 비동기 호출 접근 방식으로 사용 하기 위한 작업입니다. 예를 들어:  
  
         [!code-csharp[EventAsync#1](../../../../samples/snippets/csharp/VS_Snippets_CFX/eventasync/cs/generatedclient.cs#1)]
         [!code-vb[EventAsync#1](../../../../samples/snippets/visualbasic/VS_Snippets_CFX/eventasync/vb/generatedclient.vb#1)]  
  
    -   폼의 र ् ण 이벤트가 <`operationName` > `Completed` 이벤트 기반 비동기 호출 접근 방식으로 사용 합니다. 예를 들어:  
  
         [!code-csharp[EventAsync#2](../../../../samples/snippets/csharp/VS_Snippets_CFX/eventasync/cs/generatedclient.cs#2)]
         [!code-vb[EventAsync#2](../../../../samples/snippets/visualbasic/VS_Snippets_CFX/eventasync/vb/generatedclient.vb#2)]  
  
    -   <xref:System.EventArgs?displayProperty=nameWithType> 각 작업에 대 한 형식 (형식의 <`operationName`>`CompletedEventArgs`) 이벤트 기반 비동기 호출 접근 방식으로 사용 합니다. 예를 들어:  
...
3.  작업을 호출 하기 전에 새 제네릭을 사용 하 여 <xref:System.EventHandler%601?displayProperty=nameWithType> 형식의 <`operationName` > `EventArgs` (이전 단계에서 만든) 처리기 메서드를 추가 하는 <`operationName` > `Completed` 이벤트입니다. 그런 다음 호출에서 <`operationName` > `Async` 메서드. 예를 들어:  

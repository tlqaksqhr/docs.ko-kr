...
---
# 보안 프로토콜
웹 서비스 보안 프로토콜은 모든 기존 엔터프라이즈 메시징 보안 요구 사항을 포함하는 웹 서비스 보안 메커니즘을 제공합니다.이 단원에서는 다음 웹 서비스 보안 프로토콜에 대한 [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 세부 정보\(<xref:System.ServiceModel.Channels.SecurityBindingElement>에서 구현\)를 제공합니다.  
...
|WS\-SecurityPolicy 1.2\(2007\/04\)|http:\/\/www.oasis\-open.org\/committees\/download.php\/23821\/ws\-securitypolicy\-1.2\-spec\-cs.pdf|  
 ...
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 버전 1에서는 웹 서비스 보안 구성에 기본으로 사용할 수 있는 17가지 인증 모드를 제공합니다.각 모드는 다음과 같은 공통 배포 요구 사항에 대해 최적화된 것입니다.  
...
|-----------|--------------|-----------|--------|  
|UserNameOverTransport|사용자 이름\/암호|X509|Transport|  
|CertificateOverTransport|X509|X509|Transport|  
|KerberosOverTransport|Windows|X509|Transport|  
|IssuedTokenOverTransport|페더레이션|X509|Transport|  
|SspiNegotiatedOverTransport|Windows Sspi 협상|Windows Sspi 협상|Transport|  
|AnonymousForCertificate|없음|X509|Message|  
|UserNameForCertificate|사용자 이름\/암호|X509|Message|  
|MutualCertificate|X509|X509|Message|  
|MutualCertificateDuplex|X509|X509|Message|  
|IssuedTokenForCertificate|페더레이션|X509|Message|  
|Kerberos|Windows|Windows|Message|  
|IssuedToken|페더레이션|페더레이션|Message|  
|SspiNegotiated|Windows SSPI 협상|Windows SSPI 협상|Message|  
|AnonymousForSslNegotiated|없음|X509, TLS 협상|Message|  
|UserNameForSslNegotiated|사용자 이름\/암호|X509, TLS 협상|Message|  
|MutualSslNegotiated|X509|X509, TLS 협상|Message|  
|IssuedTokenForSslNegotiated|페더레이션|X509, TLS 협상|Message|  
 ...
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 WS\-SecureConversation을 사용하여 응용 프로그램 간의 다중 메시지 교환 보호를 위한 보안 세션을 지원합니다.구현에 대한 자세한 내용은 아래의 "보안 세션"을 참조하십시오.  
...
## 1.토큰 프로필  
 웹 서비스 보안 사양에서는 자격 증명을 보안 토큰으로서 나타냅니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 다음과 같은 토큰 유형을 지원합니다.  
...
### 1.3 Kerberos 토큰  
 ...
 R1301 Kerberos Token은 GSS\_API 및 Kerberos 사양에 정의된 GSS 래핑된 Kerberos v4 AP\_REQ 값을 사용하고, 값이 \#GSS\_Kerberosv5\_AP\_REQ인 ValueType 특성이 있어야 합니다.  
...
### 2.1 타임스탬프  
 타임스탬프 표시 여부는 <xref:System.ServiceModel.Channels.SecurityBindingElement> 클래스의 <xref:System.ServiceModel.Channels.SecurityBindingElement.IncludeTimestamp%2A> 속성을 사용하여 제어합니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 wsse:Created 및 wsse:Expires 필드를 사용하여 wsse:TimeStamp를 항상 serialize합니다.wsse:TimeStamp는 서명이 사용될 경우 항상 서명됩니다.  
  
### 2.2 보호 순서  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 "암호화 전 서명" 및 "서명 전 암호화"\(Security Policy 1.2\)의 두 가지 메시지 보호 순서를 지원합니다. "WS\-Security 1.1 서명 확인 메커니즘을 사용하지 않을 경우 서명 전 암호화로 보호된 메시지는 서명 대체 공격에 취약하고, 암호화된 내용에 대한 서명으로 인해 감사를 수행하기 더 어려워지므로 "암호화 전 서명"을 사용하는 것이 좋습니다.  
  
### 2.3 서명 보호  
 서명 전 암호화를 사용할 경우, 특히 사용자 지정 토큰을 weak 키 자료와 함께 사용하는 경우에는 암호화된 내용이나 서명 키를 추측하기 위한 무차별 키 대입 공격을 방지해 서명을 보호하는 것이 좋습니다.  
  
### 2.4 알고리즘 모음  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 Security Policy 1.2에 나열된 모든 알고리즘 모음을 지원합니다.  
  
### 2.5 키 파생  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 WS\-SecureConversation에 설명된 대로 "대칭 키에 대한 키 파생"을 사용합니다.  
...
## 3.일반 메시지 보안 매개 변수  
 이 단원에서는 클라이언트와 서비스 간에 교환되는 메시지의 보안 헤더 구조를 보여 주는 예제와 함께 각 인증 모드에 대한 정책 예제를 제공합니다.  
...
### 보안 헤더 예: SignBeforeEncrypt, EncryptSignature  
 요청  
...
#### 3.2.3 X.509 서비스 인증에서 SymmetricBinding 사용  
 "WSS10"에서는 X509 토큰 사용 시나리오를 제한적으로 지원했습니다.예를 들어, 서비스 X509 토큰만 사용하여 메시지에 대한 서명 및 암호화 보호를 제공할 수는 없습니다. ""WSS11"에서는 EncryptedKey를 대칭 토큰으로 도입했습니다.이제 요청 메시지 보호와 응답 메시지 보호에 서비스의 X.509 인증서에 대해 암호화된 임시 키를 사용할 수 있습니다.아래 3.4 단원에 설명된 인증 모드에서는 이 패턴을 사용합니다.  
...
### 3.5 SslNegotiated를 사용하여 서비스 인증  
 이 단원에서는 보호 토큰이 WS\-T\(WS\-Trust\) RST\/RSTR 메시지를 통해 TLS 프로토콜을 실행하여 그 키 값이 협상되는 WS\-SC\(WS\-SecureConversation\)별 보안 컨텍스트 토큰인 대칭 바인딩을 사용하는 인증 모드 그룹에 대해 설명합니다.WS\-Trust를 사용하는 TLS 핸드셰이크 구현에 대한 자세한 내용은 TLSNEGO를 참조하십시오.여기의 메시지 예제에서는 연결된 보안 컨텍스트가 있는 SCT가 핸드셰이크를 통해 이미 설정되어 있다고 가정합니다.  

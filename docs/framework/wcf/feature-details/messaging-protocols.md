...
[!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 채널 스택에서는 인코딩 및 전송 채널을 통해 내부 메시지 표현을 통신 형식으로 변환한 후 특정 전송을 사용하여 보냅니다.웹 서비스 상호 운용성을 위해 사용되는 가장 일반적인 전송은 HTTP이고, 웹 서비스에 사용되는 가장 일반적인 인코딩은 XML 기반 SOAP 1.1, SOAP 1.2 및 MTOM\(Message Transmission Optimization Mechanism\)입니다.  
 ...
 이 항목에서는 <xref:System.ServiceModel.Channels.HttpTransportBindingElement>에 사용되는 다음 프로토콜에 대한 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 구현 정보에 대해 설명합니다.  
...
|SOAP 1.2 HTTP 바인딩|http:\/\/www.w3.org\/TR\/soap12\-part2\/, 단원 7|  
 ...
 이 항목에서는 <xref:System.ServiceModel.Channels.TextMessageEncodingBindingElement> 및 <xref:System.ServiceModel.Channels.MtomMessageEncodingBindingElement>에 사용되는 다음 프로토콜에 대한 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 구현 정보에 대해 설명합니다.  
...
|WSDL SOAP1.2 바인딩|http:\/\/www.w3.org\/Submission\/wsdl11soap12\/|  
 ...
 이 항목에서는 <xref:System.ServiceModel.Channels.MtomMessageEncodingBindingElement>에 사용되는 다음 프로토콜에 대한 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 구현 정보에 대해 설명합니다.  
...
-   B1112: [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 SOAP 1.1과 SOAP 1.2 버전 SOAP 봉투 모두에 대해 `mustUnderstand` 값 0과 1을 내보냅니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 `mustUnderstand` 헤더에 `xs:boolean`의 전체 값 공간\(0, 1, `false`, `true`\)을 사용합니다.  
  
#### SOAP 오류  
 다음은 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 관련 SOAP 오류 구현 목록입니다.  
  
-   B2121: [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 `s11:mustUnderstand`, `s11:Client` 및 `s11:Server`와 같은 SOAP 1.1 오류 코드를 반환합니다.  
...
-   R2221: `application/soap+xml` 작업 매개 변수\(SOAP 1.2 요청에 있는 경우\)는 해당 WSDL 바인딩 내 `wsoap12:operation` 요소의 `soapAction` 특성과 일치해야 합니다.  
...
#### 끝점 참조 및 WS\-Addressing 버전  
 ...
 R3111에서 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 끝점과 교환된 메시지에 사용된 `EndpointReference` 요소 또는 형식의 네임스페이스는 이 끝점에서 구현된 WS\-Addressing 버전과 일치해야 합니다.  
  
 예를 들어, [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 끝점에서 WS\-ReliableMessaging을 구현하는 경우 `CreateSequenceResponse`에서 이러한 끝점에 의해 반환되는 `AcksTo` 헤더는 `EncodingBinding` 요소가 이 끝점에 대해 지정하는 WS\-Addressing 버전을 사용합니다.  
...
#### 메시지 헤더  
 두 WS\-Addressing 버전에 대해 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 사양에 정의된 대로 `wsa:To`, `wsa:ReplyTo`, `wsa:Action`, `wsa:MessageID`,및 `wsa:RelatesTo`와 같은 메시지 헤더를 사용합니다.  
...
### 메시지 교환 패턴  
 웹 서비스 작업 호출에 관련된 메시지의 시퀀스를 *메시지 교환 패턴*이라고 합니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 단방향, 요청\-회신 및 이중 메시지 교환 패턴을 지원합니다.이 단원에서는 사용 중인 메시지 교환 패턴에 따른 메시지 처리에 대한 WS\-Addressing 요구 사항에 대해 자세히 설명합니다.  
...
-   R3321: 요청자는 `wsa:To`, `wsa:Action`, `wsa:MessageID`를 비롯하여 끝점 참조에 지정된 모든 \[reference parameters\], \[reference properties\] 또는 둘 모두에 대한 헤더를 요청에 포함해야 합니다.  
...
-   R3324: 요청자는 `wsa:To`, `wsa:Action` 및 `wsa:RelatesTo` 헤더를 회신 메시지에 포함하고, `ReplyTo` 끝점 참조에 지정된 모든 \[reference parameters\], \[reference properties\] 또는 둘 모두에 대한 헤더를 요청에 포함해야 합니다.  
...
|코드|원인|  
|--------|--------|  
|wsa10:InvalidAddressingHeader|`wsa:To`, `wsa:ReplyTo`, `wsa:From` 또는 `wsa:MessageID`가 중복되었습니다.동일한 `RelationshipType`을 가진 `wsa:RelatesTo`가 중복되었습니다.|  
...
-   R3514: 끝점이 WSDL 1.1 SOAP 1.x HTTP 바인딩을 사용하고 `wsap10:UsingAddressing` 또는 `wsap:UsingAddressing` 어설션이 `cdp:CompositeDuplex`와 함께 연결된 정책 대안이 있는 경우 끝점에 보낸 요청 메시지에 `[address]` 속성이 "http:\/\/www.w3.org\/2005\/08\/addressing\/anonymous"가 아닌 `ReplyTo` 헤더가 있어야 합니다.  
  
-   R3515: 끝점이 WSDL 1.1 SOAP 1.x HTTP 바인딩을 사용하고 `wsap10:UsingAddressing` 어설션은 연결되어 있지만 `cdp:CompositeDuplex` 어설션은 연결되지 않은 정책 대안이 있는 경우 끝점에 보낸 요청 메시지에 `[address]` 속성이 "http:\/\/www.w3.org\/2005\/08\/addressing\/anonymous"인 `ReplyTo` 헤더가 있거나 `ReplyTo` 헤더가 전혀 없어야 합니다.  
  
-   R3516: 끝점이 WSDL 1.1 SOAP 1.x HTTP 바인딩을 사용하고 `wsap:UsingAddressing` 어설션은 연결되어 있지만 `cdp:CompositeDuplex` 어설션은 연결되지 않은 정책 대안이 있는 경우 끝점에 보낸 요청 메시지에 `[address]` 속성이 "http:\/\/www.w3.org\/2005\/08\/addressing\/anonymous"인 `ReplyTo` 헤더가 있어야 합니다.  
  
 WS\-addressing WSDL 사양에서는 세 개의 텍스트 값\(required, optional, prohibited\)을 가진 `<wsaw:Anonymous/>``wsa:ReplyTo 요소를 추가하여`  헤더\(단원 3.2\)에 대한 요구 사항을 나타냄으로써 비슷한 프로토콜 바인딩을 설명하려고 합니다.그러나 해당 요소를 어설션으로 사용하여 대안 교차를 지원하려면 도메인별 확장이 필요하므로, 이러한 요소 정의는 WS\-Policy 컨텍스트에서 어설션으로 사용할 수 없습니다.또한 이러한 요소 정의는 통신 중인 끝점 동작과 반대로 `ReplyTo` 헤더 값을 나타내므로 HTTP 전송에만 해당됩니다.  
  
#### 동작 정의  
 WS\-Addressing 2004\/08에서는 `wsdl:portType/wsdl:operation/[wsdl:input | wsdl:output | wsdl:fault]` 요소에 대한 `wsa:Action` 특성을 정의합니다.WS\-ADDR10\-WSDL\(WS\-Addressing 1.0 WSDL 바인딩\)에서는 비슷한 특성인 `wsaw10:Action`을 정의합니다.  
 ...
 B3521: [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 WS\-ADDR10\-WSDL에 정의된 대로 `wsdl:portType/wsdl:operation/[wsdl:input | wsdl:output | wsdl:fault]` 요소에서 `wsaw10:Action` 특성을 사용하여 끝점에 사용되는 WS\-Addressing 버전에 상관없이 해당 메시지에 대한 `Action` URI를 결정합니다.  
  
#### WSDL 포트에서 끝점 참조 사용  
 WS\-ADDR10\-WSDL 단원 4.1에서는 `<wsa10:EndpointReference…/>` 자식 요소를 포함하도록 `wsdl:port` 요소를 확장하여 WS\-Addressing 용어로 끝점을 설명합니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 WS\-Addressing 2004\/08에서 이 유틸리티를 확장하여 `<wsa:EndpointReference…/>`를 `wsdl:port`의 자식 요소로 나타낼 수 있게 합니다.  
  
-   R3531: 끝점에 `<wsaw10:UsingAddressing/>``wsdl:port 정책 어설션과 연결된 정책 대안이 있는 경우 해당` `<wsa10:EndpointReference  /> 요소는`  자식 요소를 포함할 수 있습니다.  
  
-   R3532: `wsdl:port`에 `<wsa10:EndpointReference  />``wsa10:EndpointReference/wsa10:Address` 자식 요소가 포함되어 있는 경우 `@address` 자식 요소 값이 형제 `wsdl:port`\/`wsdl:location` 요소의 특성 값과 일치해야 합니다.  
  
-   R3533: 끝점에 `<wsap:UsingAddressing/>``wsdl:port` 정책 어설션과 연결된 정책 대안이 있는 경우 해당 `<wsa:EndpointReference  />` 요소는 자식 요소를 포함할 수 있습니다.  
  
-   R3534: `wsdl:port`에 `<wsa:EndpointReference  />``wsa:EndpointReference/wsa:Address` 자식 요소가 포함되어 있는 경우 `@address` 자식 요소 값이 형제 `wsdl:port`\/`wsdl:location` 요소의 특성 값과 일치해야 합니다.  
...
3.  `xop:Include` 요소 정보 항목을 \[children\] 속성의 단독 멤버로 가진 구성된 SOAP 봉투의 각 요소 정보 항목의 경우 다음을 수행합니다.  
    ...
    1.  `cid:` 접두사를 제거하고 `xop:Include` 요소의 `@href` 특성 값에서 모든 URI 이스케이프 시퀀스\(RFC 2396\)를 이스케이프 해제합니다.결과 문자열을 “\<”, “\>”로 묶습니다.  
  
    2.  3a 단계에서 파생된 문자열과 일치하는 Content\-ID 헤더 값을 가진 MIME 부분을 찾습니다.  
  
    3.  각 항목의 `children` 속성에 나타나는 `xop:Include` 요소 정보 항목을 3b 단계에서 식별된 MIME 부분의 엔터티 본문의 정규 base64 인코딩\(XSD\-2, 3.2.16 base64Binary 참조\)을 나타내는 문자 정보 항목으로 대체합니다. 즉, `xop:Include` 요소 정보 항목을 패키지 부분에서 재구성된 데이터로 효과적으로 대체합니다.  
...
```  
 ...
 사실상 “\<” 및 “\>”로 묶인 전자 메일 주소입니다.`[CFWS]` 접두사와 접미사는 설명을 전달하기 위해 RFC 2822에 추가되었지만 상호 운용성을 유지하려면 사용하지 않아야 합니다.  
  
 R4143: Infoset MIME 부분의 Content\-ID 헤더 값은 `[CFWS]` 접두사 및 접미사 부분을 생략한 상태의 RFC 2822의 `msg-id` 생성을 따라야 합니다.  
  
 많은 MIME 구현에서 “\<” 및 “\>”로 묶은 값을 전자 메일 주소로 사용하도록 하는 요구 사항을 완화하여 전자 메일 주소 이외에 `absoluteURI`도 “\<” , “\>”로 묶어 사용하고 있습니다.이 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 버전에서는 다음과 같은 형식의 Content\-ID MIME 헤더 값을 사용합니다.  
...
-   R4149: SOAP 1.2 Infoset 부분은 미디어 형식이 `application/xop+xml`이고 매개 변수가 type\=”`application/soap+xml`” 및 `charset`인 Content\-Type 헤더를 포함해야 합니다.  
    ...
    ```  
     ...
     XOP에서는 `application/xop+xml`의 `charset` 매개 변수를 선택적 요소로 정의하고 있지만, `text/xml` 미디어 형식의 `charset` 매개 변수에 대한 BP 1.1 요구 사항과 마찬가지로 상호 운용성을 위해 필요합니다.  
...
#### MTOM을 사용하여 인코딩한 WCF Secure SOAP 1.2 메시지  
 이 예제에서 메시지는 WS\-Security를 사용하여 보호된 SOAP 1.2 및 MTOM을 사용하여 인코딩됩니다.인코딩을 위해 식별되는 이진 부분은 `BinarySecurityToken`의 콘텐츠로서, 암호화된 서명 및 암호화된 본문에 해당하는 `EncryptedData`의 `CipherValue`입니다.`EncryptedKey`의 `CipherValue`는 길이가 1024바이트 미만이므로 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서 최적화하도록 식별되지 않습니다.  

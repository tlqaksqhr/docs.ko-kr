...
### <a name="example"></a>예제  
 ...
 WCF 인프라에서 원래 데이터 계약의 일부가 되지 않은 데이터를 발견 하면 데이터 속성에 저장 되 고 유지 합니다. 임시 저장소를 제외한 다른 방법으로는 처리되지 않습니다. 개체가 최초 위치로 다시 돌아오는 경우, 원래 데이터(알 수 없는 데이터)도 반환됩니다. 따라서 데이터가 손실 없이 원래 끝점 간을 라운드트립합니다. 그러나 원래 끝점에서 데이터를 처리해야 하는 경우 예상했던 작업은 수행되지 않고 해당 끝점은 변경 사항을 감지하고 적용해야 합니다.  

...
```  
 ...
 그러면 라우팅에 필요한 모듈과 처리기가 로드됩니다. [!INCLUDE[crdefault](../../../../includes/crdefault-md.md)][라우팅](../../../../docs/framework/wcf/feature-details/routing.md)합니다. 또한 다음 예제와 같이 `aspNetCompatibilityEnabled` 요소의 `true` 특성도 `<serviceHostingEnvironment>`로 설정해야 합니다.  

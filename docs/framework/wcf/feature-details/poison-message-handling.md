---
title: "포이즌 메시지 처리 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   Move. 이 옵션은 [!INCLUDE[wv](../../../../includes/wv-md.md)]에서만 사용할 수 있습니다. 이 옵션은 포이즌 메시지 처리 응용 프로그램이 나중에 처리할 수 있도록 포이즌 메시지를 포이즌 메시지 큐로 이동합니다. 포이즌 메시지 큐는 응용 프로그램 큐의 하위 큐입니다. 포이즌 메시지 처리 응용 프로그램은 포이즌 큐 외부의 메시지를 읽는 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 서비스일 수 있습니다. 포이즌 큐는 응용 프로그램 큐의 하위 큐 및 net.msmq://로 주소가 지정 될 수*컴퓨터 이름*>/*applicationQueue*; poison, 여기서 *컴퓨터 이름* 큐가 있는 컴퓨터의 이름 및 *applicationQueue* 응용 프로그램별 큐의 이름입니다.  
...
-   <xref:System.ServiceModel.NetMsmqBinding>합니다. [!INCLUDE[dnprdnshort](../../../../includes/dnprdnshort-md.md)] 바인딩은 다른 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 끝점과 큐 기반 통신을 수행하는 데 적합합니다.  
  
-   <xref:System.ServiceModel.MsmqIntegration.MsmqIntegrationBinding>합니다. 기존 메시지 큐 응용 프로그램과 통신하는 데 적합한 바인딩입니다.  
...
## <a name="best-practice-handling-msmqpoisonmessageexception"></a>최선의 방법: MsmqPoisonMessageException 처리  
 서비스는 메시지가 포이즌 임을 확인, 대기 중인된 전송 throw는 <xref:System.ServiceModel.MsmqPoisonMessageException> 를 포함 하는 `LookupId` 포이즌 메시지의 합니다.  
  
 수신 응용 프로그램을 구현할 수는 <xref:System.ServiceModel.Dispatcher.IErrorHandler> 응용 프로그램에 필요한 모든 오류를 처리 하는 인터페이스입니다. [!INCLUDE[crdefault](../../../../includes/crdefault-md.md)][오류 처리 및 보고에 대 한 제어 확장](../../../../docs/framework/wcf/samples/extending-control-over-error-handling-and-reporting.md)합니다.  
  
 응용 프로그램에서 서비스가 큐의 남은 메시지에 액세스할 수 있도록 포이즌 메시지를 포이즌 메시지 큐로 이동하는 몇 가지 포이즌 메시지 자동 처리 작업이 필요할 수 있습니다. 오류 처리 메커니즘을 사용 하 여 포이즌 메시지 예외를 수신 하는 유일한 시나리오는 경우는 <xref:System.ServiceModel.Configuration.MsmqBindingElementBase.ReceiveErrorHandling%2A> 로 설정 되어 <xref:System.ServiceModel.ReceiveErrorHandling>합니다. 메시지 큐 3.0의 포이즌 메시지 샘플은 이 동작을 보여 줍니다. 다음에서는 최선의 방법을 포함하여 포이즌 메시지를 처리하는 단계를 간략히 보여 줍니다.  
...
## <a name="batching-and-poison-messages"></a>일괄 처리 및 포이즌 메시지  
 메시지가 포이즌 메시지가 되고 일괄 처리에 포함되면 전체 일괄 처리가 롤백되고 채널이 한 번에 하나의 메시지를 반환합니다. [!INCLUDE[crabout](../../../../includes/crabout-md.md)]다음을 참조 하십시오 [트랜잭션에서 메시지 일괄 처리](../../../../docs/framework/wcf/feature-details/batching-messages-in-a-transaction.md)  

...
## <a name="configuring-endpoints"></a>끝점 구성  
 클라이언트 구성은 클라이언트가 하나를 지정할 수 있도록 설계 되었습니다 또는 각각 고유한 이름, 더 많은 끝점 주소 및 계약을 참조는 [ \<바인딩 >](../../../../docs/framework/configure-apps/file-schema/wcf/bindings.md) 및 [ \< 동작 >](../../../../docs/framework/configure-apps/file-schema/wcf/behaviors.md) 해당 끝점을 구성 하는 데 사용할 클라이언트 구성 요소입니다. 클라이언트 구성 파일은 명명할 수 "App.config" WCF 런타임에 예상 하는 이름입니다. 다음 예제에서는 클라이언트 구성 파일을 보여 줍니다.  

---
title: "UriTemplate 및 UriTemplateTable | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|활동 예보|weather\/{state}\/{city}\/{activity}|  
 ...
 이 표에서는 구조적으로 비슷한 URI 집합을 보여 줍니다.각 항목은 URI 템플릿에 해당합니다.중괄호 안에 있는 세그먼트는 변수를 설명하고중괄호 밖에 있는 세그먼트는 리터럴 문자열을 설명합니다.개발자는 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 템플릿 클래스를 사용하여 수신 URI\(예: "\/weather\/wa\/seattle\/cycling"\)를 가져와서 해당 URI를 설명하는 템플릿\("\/weather\/{state}\/{city}\/{activity}"\)과 일치시킬 수 있습니다.  
...
>  <xref:System.UriTemplate> 클래스는 후보 URI를 템플릿에 일치시킬 때 구성표 및 포트 번호를 무시합니다.  
 ...
 템플릿에서 URI를 생성할 수 있게 해주는 두 메서드\(<xref:System.UriTemplate.BindByName%28System.Uri%2CSystem.Collections.Specialized.NameValueCollection%29> 및 [BindByPosition\(Uri, String\<xref:System.UriTemplate.BindByPosition%28System.Uri%2CSystem.String%5B%5D%29>\)가 있습니다.<xref:System.UriTemplate.BindByName%28System.Uri%2CSystem.Collections.Specialized.NameValueCollection%29>은 기본 주소와 매개 변수의 이름\/값 컬렉션을 사용합니다.이러한 매개 변수는 템플릿이 바인딩될 때 변수를 대체합니다.[BindByPosition\(Uri, String\<xref:System.UriTemplate.BindByPosition%28System.Uri%2CSystem.String%5B%5D%29>은 이름\/값 쌍을 사용하고 왼쪽에서 오른쪽으로 대체합니다.  
...
```  
 ...
 선행\/후행 슬래시는 경로 식에서 선택적 요소입니다.쿼리 식과 조각 식 모두 완전히 생략할 수 있습니다.경로는 '\/'로 구분된 세그먼트로 구성되며, 각 세그먼트는 리터럴 값, 변수 이름\({중괄호}로 표시\) 또는 와일드카드\('\*'로 표시\)를 포함할 수 있습니다.이전 템플릿에서 "\\weather\\ 세그먼트는 리터럴 값이고 "{state}" 및 "{city}"는 변수입니다.변수는 중괄호 안의 내용에서 이름을 가져온 다음 나중에 구체적인 값으로 대체되어 *닫힌 URI*를 만들 수 있습니다.와일드카드는 선택적 요소이지만 URI의 끝에만 표시될 수 있고, "경로의 나머지 부분"과 논리적으로 일치합니다.  
  
 쿼리 식\(있는 경우\)은 순서가 지정되지 않은, '&'로 구분된 이름\/값 쌍을 지정합니다.쿼리 식의 요소는 리터럴 쌍\(x\=2\) 또는 변수 쌍\(x\={var}\)이 될 수 있습니다.쿼리의 오른쪽에만 변수 식을 포함할 수 있습니다.\({someName} \= {someValue}는 허용되지 않습니다.짝이 없는 값\(?x\)은 사용할 수 없습니다.빈 쿼리 식과 단일 '?' 만으로 구성된 쿼리 식은 차이가 없습니다. 모두 "any query"를 의미합니다.  
...
-   “{shoe}\/{SHOE}\/x\=2” \- 변수 이름이 중복됩니다.  
  
-   “{shoe}\/boat\/?bed\={shoe}” \- 변수 이름이 중복됩니다.  
  
-   “?x\=2&x\=3” – 쿼리 문자열의 이름\/값 쌍은 리터럴인 경우에도 고유해야 합니다.  
  
-   “?x\=2&” – 쿼리 문자열의 형식이 잘못되었습니다.  
  
-   “?2&x\={shoe}” – 쿼리 문자열은 이름\/값 쌍이어야 합니다.  
  
-   “?y\=2&&X\=3” – 쿼리 문자열은 이름\/값 쌍이어야 하며, 이름은 '&'로 시작할 수 없습니다.  
...
### 복합 및 일치하는 경로 세그먼트  
 복합 경로 세그먼트를 사용하면 단일 경로 세그먼트 내에 여러 변수가 포함된 UriTemplate을 정의할 수 있습니다.예를 들어, 템플릿 문자열 “Addresses\/{state}.{city}”에서는 두 변수\(state 및 city\)가 동일한 세그먼트 안에 정의되어 있습니다.이 템플릿은 “http:\/\/example.com\/Washington.Redmond”와 같은 URL과 일치하지만 “http:\/\/example.com\/Washington.Redmond.Microsoft”와 같은 URL과도 일치합니다.후자의 경우 state 변수에 “Washington”이 포함되고 city 변수에 “Redmond.Microsoft”가 포함됩니다.이 경우 모든 텍스트\(‘\/’ 제외\)가 {city} 변수와 일치합니다.“추가” 텍스트와 일치하지 않는 템플릿을 만들려면 변수를 별도의 템플릿 세그먼트에 두십시오\(예: “Addresses\/{state}\/{city}”\).  
...
```  
 ...
 이 템플릿은 기본값 `1`을 사용하여 이름이 `a`인 변수를 선언하고 기본값 `5`를 사용하여 이름이 `b`인 변수를 선언합니다.  
...
### 템플릿 동등성  
 템플릿의 모든 리터럴이 일치하고 동일한 세그먼트에 변수가 있는 경우 두 템플릿이 *구조적으로 동등*한 것으로 간주됩니다.예를 들어 다음 템플릿은 구조적으로 동등합니다.  
...
## UriTemplateTable  
 <xref:System.UriTemplateTable> 클래스는 개발자의 선택 개체에 바인딩된 <xref:System.UriTemplate> 개체의 연결 테이블을 나타냅니다.<xref:System.UriTemplateTable.MakeReadOnly%28System.Boolean%29>를 호출하기 전에 <xref:System.UriTemplateTable>에 하나 이상의 <xref:System.UriTemplate>이 있어야 합니다.<xref:System.UriTemplateTable.MakeReadOnly%28System.Boolean%29>가 호출되기 이전에는 <xref:System.UriTemplateTable>의 내용을 변경할 수 있습니다.<xref:System.UriTemplateTable.MakeReadOnly%28System.Boolean%29>가 호출되면 유효성 검사가 수행됩니다.수행되는 유효성 검사 형식은 <xref:System.UriTemplateTable.MakeReadOnly%28System.Boolean%29>에 대한 `allowMultiple` 매개 변수 값에 따라 다릅니다.  
  
 `false`를 전달하여 <xref:System.UriTemplateTable.MakeReadOnly%28System.Boolean%29>를 호출하면 <xref:System.UriTemplateTable>에서 테이블에 템플릿이 없는지 확인합니다.구조적으로 동등한 템플릿이 있으면 예외를 throw합니다.하나의 템플릿만 수신 URI와 일치하게 하려면 <xref:System.UriTemplateTable.MatchSingle%28System.Uri%29>과 함께 사용합니다.  
  
 `true`를 전달하여 <xref:System.UriTemplateTable.MakeReadOnly%28System.Boolean%29>를 호출하면 <xref:System.UriTemplateTable>에서 <xref:System.UriTemplateTable>에 구조적으로 동등한 여러 템플릿을 포함하도록 허용합니다.  
  
 <xref:System.UriTemplateTable>에 추가된 <xref:System.UriTemplate> 개체 집합에 쿼리 문자열이 있는 경우 해당 문자열은 모호하지 않아야 합니다.동일한 쿼리 문자열을 사용할 수 있습니다.  

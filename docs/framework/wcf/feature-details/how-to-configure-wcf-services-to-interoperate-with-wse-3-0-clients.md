...
1.  WCF 서비스에 대 한 사용자 지정 바인딩을 정의 합니다.  
    ...
    3.  인증 모드와 자식 추가 하 여 WSE 3.0과 호환 되는 메시지 보안을 유지 하는 데 사용 되는 Ws-security 사양 버전 지정 [ \<보안 >](../../../../docs/framework/configure-apps/file-schema/wcf/security-of-custombinding.md) 에 [ \<바인딩 >](../../../../docs/framework/misc/binding.md)합니다.  
         ...
         인증 모드를 설정 하려면 설정는 `authenicationMode` 특성에는 [ \<보안 >](../../../../docs/framework/configure-apps/file-schema/wcf/security-of-custombinding.md)합니다. 인증 모드는 대체로 WSE 3.0의 턴키 보안 어설션에 해당합니다. 다음 표에서 WSE 3.0에서 턴키 보안 어설션에 wcf에서 인증 모드를 매핑합니다.  
    ...
    4.  Ws-addressing 사양의 2004 년 8 월 버전을 추가 하 여 WCF에서 사용 하도록 지정는 [ \<textMessageEncoding >](../../../../docs/framework/configure-apps/file-schema/wcf/textmessageencoding.md) 설정 하 고는 `messageVersion` 하려면 해당 값으로 <xref:System.ServiceModel.Channels.MessageVersion.Soap11WSAddressingAugust2004%2A>합니다.  

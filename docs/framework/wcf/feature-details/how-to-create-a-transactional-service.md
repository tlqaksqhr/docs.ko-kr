---
title: "방법: 트랜잭션 서비스 만들기 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
3.  트랜잭션 컨텍스트를 이동하도록 지정하고 이 작업에 사용할 프로토콜을 지정하여 구성 파일에서 바인딩을 구성합니다.[!INCLUDE[crdefault](../../../../includes/crdefault-md.md)][ServiceModel 트랜잭션 구성](../../../../docs/framework/wcf/feature-details/servicemodel-transaction-configuration.md).특히 바인딩 형식은 끝점 요소의 `binding` 특성에서 지정합니다.[\<endpoint\>](http://msdn.microsoft.com/ko-kr/13aa23b7-2f08-4add-8dbf-a99f8127c017) 요소에는 다음 샘플 구성에서처럼 이름이 `transactionalOleTransactionsTcpBinding`인 바인딩 구성을 참조하는 `bindingConfiguration` 특성이 포함됩니다.  
...
1.  기본적으로 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 작업은 처리되지 않은 예외가 throw되지 않으면 트랜잭션을 자동으로 완료합니다.<xref:System.ServiceModel.OperationBehaviorAttribute.TransactionAutoComplete%2A> 속성 및 <xref:System.ServiceModel.OperationContext.SetTransactionComplete%2A> 메서드를 사용하면 이 동작을 수정할 수 있습니다.debit 및 credit 연산 등 다른 연산과 동일한 트랜잭션 내에서 연산을 수행해야 하는 경우 다음 `Debit` 연산 예제에서처럼 <xref:System.ServiceModel.OperationBehaviorAttribute.TransactionAutoComplete%2A> 속성을 `false`로 설정하여 autocomplete 동작을 비활성화할 수 있습니다.`Debit` 연산에서 사용하는 트랜잭션은 `Credit1` 연산에서처럼 `true`로 설정된 <xref:System.ServiceModel.OperationBehaviorAttribute.TransactionAutoComplete%2A> 속성을 가진 메서드가 호출될 때까지 또는 `Credit2` 연산에서처럼 트랜잭션 완료를 명시적으로 표시하기 위해 <xref:System.ServiceModel.OperationContext.SetTransactionComplete%2A> 메서드가 호출될 때까지 완료되지 않습니다.설명의 목적으로 두 개의 credit 연산을 예로 들었지만 하나의 credit 연산이 보다 일반적입니다.  
...
2.  트랜잭션의 상관 관계를 위해 <xref:System.ServiceModel.OperationBehaviorAttribute.TransactionAutoComplete%2A> 속성을 `false`로 설정하려면 세션 바인딩을 사용해야 합니다.이 요구 사항은 <xref:System.ServiceModel.ServiceContractAttribute>의 `SessionMode` 속성을 사용하여 지정합니다.  

---
title: "큰 데이터 및 스트리밍 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# 큰 데이터 및 스트리밍
[!INCLUDE[indigo1](../../../../includes/indigo1-md.md)]는 XML 기반 통신 인프라입니다.XML 데이터가 일반적으로 [XML 1.0 specification](http://go.microsoft.com/fwlink/?LinkId=94838)에 정의된 표준 텍스트 형식으로 인코딩되기 때문에 연결된 시스템 개발자와 설계자는 네트워크를 통해 전송되는 메시지의 통신 사용량\(또는 크기\)에 관심이 많으며, XML의 텍스트 기반 인코딩은 이진 데이터의 효율적인 전송에 특수한 과제를 제공합니다.  
...
### 이진 콘텐츠  
 ...
 Base64 인코딩된 문자열에서 각 문자는 원래 8비트 데이터 중 6비트를 나타내며, 따라서 대화에서 일반적으로 추가되는 추가 형식 문자\(캐리지 리턴\/줄 바꿈\)를 제외하고 Base64에서 인코딩 오버헤드 비율은 4:3이 됩니다.XML과 이진 인코딩 사이의 차이가 갖는 심각도는 일반적으로 시나리오에 따라 결정되지만, 500MB의 페이로드를 전송하면서 크기가 33% 이상 증가한다면 보통은 적절하지 못한 것으로 봅니다.  
...
### 큰 데이터 콘텐츠  
 와이어 크기를 고려하지 않더라도, 앞에서 언급한 500MB 페이로드는 서비스 및 클라이언트에 큰 로컬 과제를 남깁니다.기본적으로 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 *버퍼링 모드*로 메시지를 처리합니다.즉, 메시지의 전체 콘텐츠가 보내기 전과 받은 후에 메모리에 보관됩니다.대부분의 시나리오에서는 좋은 전략이며, 디지털 서명 및 안정적인 배달 등의 메시징 기능에 필수적인 방법이지만, 큰 메시지를 전달하는 경우에는 시스템 리소스가 빠른 속도로 소모될 수 있습니다.  
...
-   전송을 시작할 때 전체를 사용할 수 없는 데이터.  
 ...
 많은 양의 데이터를 전송할 때는 maxAllowedContentLength IIS 설정\(자세한 내용은 [IIS 요청 한도 구성](http://go.microsoft.com/fwlink/?LinkId=253165) 참조\) 및 maxReceivedMessageSize 바인딩 설정\(예: <xref:System.ServiceModel.BasicHttpBinding.MaxReceivedMessageSize%2A> 또는 <xref:System.ServiceModel.NetTcpBinding.MaxReceivedMessageSize%2A>\)을 설정해야 합니다.maxAllowedContentLength 속성의 기본값은 28.6M으로 설정되고 maxReceivedMessageSize 속성의 기본값은 64KB로 설정됩니다.  
  
## 인코딩  
 *인코딩*은 와이어에서 메시지를 표현하는 방법에 대한 일련의 규칙을 정의합니다.*인코더*는 그러한 인코딩을 구현하며 보내는 쪽에서 <xref:System.ServiceModel.Channels.Message> 메모리 내 메시지를 네트워크로 전달할 수 있는 바이트 스트림이나 바이트 버퍼로 변환합니다.받는 쪽에서는 인코더가 바이트 시퀀스를 메모리 내 메시지로 변환합니다.  
...
|<xref:System.ServiceModel.Channels.BinaryMessageEncodingBindingElement>|이진 메시지 인코더는 Net\* 바인딩의 기본 인코더이며 통신 당사자 양쪽 모두가 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]를 기반으로 하는 경우에 적절합니다.이진 메시지 인코더에서는 .NET 이진 XML 형식을 사용합니다. 이 형식은 일반적으로 동급 XML 1.0 표현보다 크기가 작으며 이진 데이터를 바이트 스트림으로 인코딩하는 XML Infoset을 나타내는 Microsoft만의 이진 표현입니다.|  
...
### MTOM 활성화  
 상호 운용성이 필요하며 큰 이진 데이터를 전송해야 하는 경우 MTOM 메시지 인코딩은 표준 <xref:System.ServiceModel.BasicHttpBinding> 또는 <xref:System.ServiceModel.WSHttpBinding> 바인딩에서 해당 `MessageEncoding` 속성을 <xref:System.ServiceModel.WSMessageEncoding>으로 설정하거나 <xref:System.ServiceModel.Channels.MtomMessageEncodingBindingElement>를 <xref:System.ServiceModel.Channels.CustomBinding>에 작성하여 활성화할 수 있는 대체 인코딩 전략입니다.[MTOM 인코딩](../../../../docs/framework/wcf/samples/mtom-encoding.md) 샘플에서 추출한 다음 코드 예에서는 구성에서 MTOM을 활성화하는 방법을 보여 줍니다.  
...
```  
 ...
 MTOM 인코더는 다른 모든 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 기능과 함께 사용할 수 있습니다.세션 지원이 필요한 경우와 같이 이 규칙을 따를 수 없는 경우도 있습니다.  
  
### 프로그래밍 모델  
 응용 프로그램에서 세 개의 기본 제공 인코더 중 어느 것을 사용해도, 이진 데이터 전송에 대한 프로그래밍 경험은 동일합니다.차이는 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서 데이터 형식에 따라 데이터를 처리하는 방법에 있습니다.  
...
> [!NOTE]
>  데이터 계약 내에서는 <xref:System.IO.Stream?displayProperty=fullName> 파생 형식을 사용하지 말아야 합니다.스트림 데이터는 다음 "스트리밍 데이터" 단원에 설명된 대로 스트리밍 모델을 사용하여 통신해야 합니다.  
  
## 스트리밍 데이터  
 전송할 데이터가 많은 경우 버퍼링을 수행하고 메모리 내에서 메시지 전체를 처리하는 기본 동작 대신 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]의 스트리밍 전송 모드를 사용할 수 있습니다.  
...
### 제한  
 스트리밍이 활성화되어 있으면 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 기능 중 상당수를 사용할 수 없습니다.  
...
-   스트리밍 모드에서 요청을 보내기 및 승인, 버퍼링 모드에서 응답 승인 및 반환\(<xref:System.ServiceModel.TransferMode>\).  
  
-   버퍼링 모드에서 요청을 보내기 및 승인, 스트리밍 모드에서 응답 승인 및 반환\(<xref:System.ServiceModel.TransferMode>\).  
  
-   스트리밍 모드에서 양방향으로 요청 및 응답을 보내고 받기.\(<xref:System.ServiceModel.TransferMode>\).  
  
 전송 모드를 모든 바인딩의 기본 설정인 <xref:System.ServiceModel.TransferMode>로 설정하면 스트리밍을 비활성화할 수 있습니다.다음 코드에서는 구성에 전송 모드를 설정하는 방법을 보여 줍니다.  
...
```  
 ...
 요청 및 응답의 스트리밍을 켜거나 양방향 모두에 대해 통신 참가자 중 어느 쪽에서나 기능에 영향을 주지 않고 독립적으로 스트리밍을 켤 수 있습니다.하지만 전송 데이터 크기가 너무 커서 통신 링크의 양 끝점 모두에서 스트리밍을 활성화하는 것이 적합하다고 항상 가정해야 합니다.끝점 중 하나가 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]로 구현되지 않은 교차 플랫폼 통신의 경우 스트리밍 사용 기능은 플랫폼의 스트리밍 기능에 따라 달라집니다.다른 흔치 않은 예외로는 클라이언트 또는 서비스에서 작업 집합을 최소화해야 하며 작은 버퍼 크기만 사용할 수 있는 메모리 소비 구동 시나리오가 있습니다.  
...
```  
 ...
 위 예의 `Echo` 작업에서는 스트림을 받아서 반환하기 때문에 <xref:System.ServiceModel.TransferMode>의 바인딩에서 사용해야 합니다.`RequestInfo` 작업의 경우 <xref:System.ServiceModel.TransferMode>가 <xref:System.IO.Stream>만 반환하기 때문에 가장 적절합니다.단방향 작업은 <xref:System.ServiceModel.TransferMode>에 가장 적절합니다.  
...
```  
 ...
 스트림이 EOF\(파일의 끝\)에 도달하면 스트리밍 전송이 끝나고 메시지가 닫힙니다.메시지를 보낼 때\(값 반환 또는 작업 호출\) <xref:System.IO.FileStream>을 전달하면 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 인프라에서는 그 후로 스트림을 완전히 읽고 EOF에 도달할 때까지 모든 데이터를 가져옵니다.그러한 기본 제공되는 <xref:System.IO.Stream> 파생 클래스가 없는 소스에 스트리밍 데이터를 전달하려면 그러한 클래스를 생성하고, 스트림 소스 위에 해당 클래스를 오버레이하고, 그 결과를 인수 또는 반환 값으로 사용합니다.  
  
 메시지를 받을 때 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 Base64인코딩 메시지 본문 콘텐츠\(또는 MTOM을 사용하는 경우 해당 MIME 부분\)로 스트림을 생성하며 콘텐츠를 읽고 나면 스트림이 EOF에 도달합니다.  
...
### 큰 데이터의 특수 보안 고려 사항  
 모든 바인딩에서는 들어오는 메시지의 크기를 제한하여 서비스 거부 공격을 방지할 수 있습니다.예를 들어 <xref:System.ServiceModel.BasicHttpBinding>에서는 들어오는 메시지의 크기를 제한하여 메시지를 처리할 때 액세스되는 메모리의 최대 크기를 제한하는 <xref:System.ServiceModel.BasicHttpBinding.MaxReceivedMessageSize%2A> 속성을 노출합니다.이 단위는 바이트로 설정되며 기본값은 65,536바이트입니다.  
  
 큰 데이터 스트리밍 시나리오에 해당되는 보안 위험은 수신기에서 스트리밍을 예상할 때 데이터를 버퍼링하게 만들어 서비스 거부를 일으킵니다.예를 들어 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 항상 메시지의 SOAP 헤더를 버퍼링하기 때문에 공격자는 전체가 헤더로 구성된 큰 용량의 악의적인 메시지를 생성하여 데이터를 버퍼링시킬 수 있습니다.스트리밍이 활성화되면 수신기에서 전체 메시지가 한 번에 메시지에 버퍼링될 것이라고 예상하지 않기 때문에 `MaxReceivedMessageSize`를 매우 큰 값으로 설정할 수 있습니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서 메시지를 버퍼링하게 되면 메모리 오버플로가 발생합니다.  
  
 따라서 이 경우에는 들어오는 메시지 크기를 제한하는 것만으로는 부족합니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서 버퍼링하는 메모리를 제한하려면 `MaxBufferSize` 속성이 필요합니다.스트리밍을 수행할 때 이 값을 안전한 값으로 설정하거나 기본값으로 유지하는 것이 중요합니다.예를 들어, 서비스에서 크기 4GB까지의 파일을 받아 로컬 디스크에 저장해야 하는 경우를 가정할 수 있습니다.한 번에 64KB까지의 데이터만 버퍼링할 수 있는 방법으로 메모리가 제한된 경우도 가정할 수 있습니다.그러면 `MaxReceivedMessageSize`는 4GB로 설정하고 `MaxBufferSize`는 64KB로 설정합니다.또한 서비스 구현에서 64KB 청크의 들어오는 스트림에서만 읽으며 이전 부분이 디스크에 기록된 후 메모리에서 폐기되기 전에는 다음 청크를 읽지 않는지 확인해야 합니다.  
  
 이 할당량이 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서 수행하는 버퍼링만 제한하며 자체 서비스나 클라이언트 구현에서 수행하는 버퍼링으로부터는 보호할 수 없다는 것도 이해해야 합니다.추가 보안 고려 사항에 대한 [!INCLUDE[crabout](../../../../includes/crabout-md.md)]는 [데이터에 대한 보안 고려 사항](../../../../docs/framework/wcf/feature-details/security-considerations-for-data.md)을 참조하십시오.  

---
title: "세션의 대기 중인 메시지 그룹화 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
1.  세션이 필요한 서비스 계약을 정의합니다. 이 작업을 수행 된 <xref:System.ServiceModel.OperationContractAttribute> 특성을 지정 하 여:  
  
    ```  
    SessionMode=SessionMode.Required  
    ```  
  
2.  이러한 메서드는 아무것도 반환하지 않기 때문에 계약의 작업을 단방향으로 표시합니다. 이러한 용도로 <xref:System.ServiceModel.OperationContractAttribute> 특성을 지정 하 여:  
...
4.  서비스 작업마다 하나의 트랜잭션이 필요합니다. 이를 지정 된 <xref:System.ServiceModel.OperationBehaviorAttribute> 특성입니다. 트랜잭션을 완료하는 작업에서는 `TransactionAutoComplete`를 `true`로 설정해야 합니다.  
...
6.  사용 하 여 트랜잭션 큐 만들기 <xref:System.Messaging>합니다. 메시지 큐(MSMQ) 또는 MMC를 사용하여 큐를 만들 수도 있습니다. 그러면 트랜잭션 큐가 만들어집니다.  
  
7.  서비스에 대 한 서비스 호스트를 사용 하 여 만들 <xref:System.ServiceModel.ServiceHost>합니다.  

---
title: "Reliable Messaging 프로토콜 버전 1.0 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   B1101: [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 개시자가 `CreateSequence` 메시지에서 선택적 Expires 요소를 생성하지 않거나 `CreateSequence` 메시지에 `Offer` 요소가 포함된 경우 `Offer` 요소에서 선택적 `Expires` 요소를 생성하지 않습니다.  
...
-   R1103: `CreateSequence`에 `Offer` 요소가 있는 경우 신뢰할 수 있는 메시징 응답자는 `wsrm:Accept` 요소가 포함된 `CreateSequenceResponse`와 함께 시퀀스 및 응답을 수락하여 상호 관련된 두 개의 역방향 시퀀스를 구성하거나 `CreateSequence` 요청을 거부해야 합니다.  
  
-   R1104: 역방향 시퀀스로 이동하는 `SequenceAcknowledgement` 및 응용 프로그램 메시지는 `CreateSequence`의 `ReplyTo` 끝점 참조로 보내야 합니다.  
  
-   R1105: `CreateSequence`의 `AcksTo` 및 `ReplyTo` 끝점 참조에는 8비트 형식과 일치하는 주소 값이 있어야 합니다.  
  
     [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 응답자는 시퀀스를 만들기 전에 `AcksTo` 및 `ReplyTo` EPR\(끝점 참조\)의 URI 부분이 동일한지 확인합니다.  
  
-   R1106: `CreateSequence`의 `AcksTo` 및 `ReplyTo` 끝점 참조에는 동일한 참조 매개 변수 집합이 있어야 합니다.  
  
     [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]는 `CreateSequence`에 대한 `AcksTo` 및 `ReplyTo`의 \[참조 매개 변수\]를 동일한 것으로 지정하지 않지만 동일하다고 가정하며, 승인 및 역방향 시퀀스 메시지에 `ReplyTo` 끝점 참조의 \[참조 매개 변수\]를 사용합니다.  
  
-   R1107: `Offer` 메커니즘을 사용하여 두 개의 역방향 시퀀스가 설정된 경우 역방향 시퀀스로 이동하는 `SequenceAcknowledgement` 및 응용 프로그램 메시지를 `CreateSequence`의 `ReplyTo` 끝점 참조로 보내야 합니다.  
  
-   R1108: Offer 메커니즘을 사용하여 두 개의 역방향 시퀀스가 설정된 경우 `CreateSequenceResponse`의 `wsrm:Accept` 요소에 대한 `wsrm:AcksTo` 끝점 자식 요소의 `[address]` 속성은 `CreateSequence`의 바이트별 대상 URI와 일치해야 합니다.  
  
-   R1109: `Offer` 메커니즘을 사용하여 두 개의 역방향 시퀀스가 설정된 경우 개시자가 보낸 메시지 및 응답자의 메시지 승인을 같은 끝점 참조로 보내야 합니다.  
  
     [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]는 WS\-Reliable Messaging을 사용하여 개시자와 응답자 간에 신뢰할 수 있는 세션을 설정합니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] WS\-Reliable Messaging 구현에서는 단방향, 요청\-회신 및 전체 이중 메시징 패턴에 신뢰할 수 있는 세션을 제공합니다.`CreateSequence`\/`CreateSequenceResponse`에서 WS\-Reliable Messaging `Offer` 메커니즘을 사용하면 상호 관련된 두 개의 역방향 시퀀스를 설정하고 모든 메시지 끝점에 적합한 세션 프로토콜을 제공합니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]는 세션 무결성을 위한 종단 간 보호를 포함하여 세션에 대한 보안 보장을 제공하므로 같은 상대방에 보낼 메시지가 동일한 대상에 도달하는지 확인하는 것이 효율적입니다.이렇게 하면 응용 프로그램 메시지에서 피기백킹이라고 하는 시퀀스 승인을 사용할 수 있습니다.따라서 제약 조건 R1104, R1105, 및 R1108은 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에 적용됩니다.  
...
-   B1201:[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]는 `xs:long`의 최대 포함 값, 9223372036854775807보다 작은 시퀀스 번호를 생성하고 액세스합니다.  
  
-   B1202:[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]는 항상 http:\/\/schemas.xmlsoap.org\/ws\/2005\/02\/rm\/LastMessage의 동작 URI를 포함하는 본문이 비어 있는 마지막 메시지를 생성합니다.  
...
### AckRequested 헤더  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]는 `AckRequested` 헤더를 연결 유지 메커니즘으로 사용합니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]는 선택적 `MessageNumber` 요소를 생성하지 않습니다.`MessageNumber` 요소가 포함된 `AckRequested` 헤더가 있는 메시지를 수신하는 경우 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]는 다음 예제에서처럼 `MessageNumber` 요소의 값을 무시합니다.  
...
-   B1403: [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]는 `Nack` 요소가 포함된 `SequenceAcknowledgement` 헤더를 생성하지 않지만 `Nack` 요소를 지원합니다.  
...
-   B1502:[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 끝점은 사양에 설명된 대로 `CreateSequenceRefused` 오류를 생성할 수 있습니다.  
  
-   B1503:서비스 끝점이 해당 연결 제한에 도달하여 새 연결을 처리할 수 없는 경우 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]는 다음 예제에서처럼 추가 `CreateSequenceRefused` 오류 하위 코드,  `netrm:ConnectionLimitReached`를 생성합니다.  
...
-   B1601:[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]는 다음 중 하나가 true인 경우 "메시지 주소 지정 헤더 필요" 오류를 생성합니다.  
...
-   B1602:[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]는 `Sequence` 헤더가 없고 WS\-Reliable Messaging 사양에서 인식하지 않는 `Action` 헤더가 있는 메시지에 대한 응답으로 "지원되지 않는 작업" 오류를 생성합니다.  
  
-   B1603:[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]는 끝점이 `CreateSequence` 메시지의 주소 지정 헤더 검사에 따라 시퀀스를 처리하지 않음을 나타내기 위해 "끝점을 사용할 수 없음" 오류를 생성합니다.  
...
-   R2102:단일 버전의 WS\-Addressing은 `wsrm:Offer` 메커니즘을 사용하여 상호 관련된 한 쌍의 역방향 시퀀스 또는 제공된 WS\-Reliable Messaging 시퀀스 전체에서 사용할 수 있어야 합니다.  
...
-   R2301:개별 메시지의 무결성과 기밀성뿐만 아니라 WS\-Reliable Messaging 시퀀스의 무결성을 보호하려면 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서 WS\-Secure Conversation을 사용해야 합니다.  
...
-   B2304:WS\-Reliable Messaging 시퀀스 또는 한 쌍의 상호 관련된 역방향 시퀀스는 항상 단일 WS\-SecureConversation 세션에 바인딩됩니다.  
     ...
     [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 소스는 `CreateSequence` 메시지의 요소 확장성 섹션에 `wsse:SecurityTokenReference` 요소를 생성합니다.  
  
-   R2305:WS\-Secure Conversation을 사용하여 구성하는 경우 `CreateSequence` 메시지에 `wsse:SecurityTokenReference` 요소가 있어야 합니다.  
...
## 흐름 제어 WS\-Reliable Messaging 확장  
 ...
 `ReliableSessionBindingElement`의 `FlowControlEnabled``bool` 속성을 `true`로 설정하여 흐름 제어를 사용하도록 설정합니다.다음은 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에 적용되는 제약 조건의 목록입니다.  
  
-   B4001: 신뢰할 수 있는 메시징 흐름 제어를 사용하는 경우 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]는 `SequenceAcknowledgement` 헤더의 요소 확장성에 `netrm:BufferRemaining` 요소를 생성합니다.  
...
-   B4004:[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 신뢰할 수 있는 메시징 서비스는 신뢰할 수 있는 메시징 대상 응용 프로그램이 메시지를 신속하게 받을 수 없는 경우 전송된 메시지 수를 스로틀합니다.신뢰할 수 있는 메시징 대상은 메시지를 버퍼링하고 요소의 값을 0으로 줄입니다.  
...
#### CreateSequence 교환  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 개시자는 시퀀스를 제공하지 않는 `CreateSequence` 메시지를 생성합니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 응답자는 시퀀스를 만들기 전에 `CreateSequence`에서 제공하는 시퀀스가 없는지 확인합니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 응답자는 `CreateSequenceResponse` 메시지를 통해 `CreateSequence` 요청에 응답합니다.  
...
#### CreateSequence 교환  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 개시자는 시퀀스를 제공하지 않는 `CreateSequence` 메시지를 생성합니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 응답자는 시퀀스를 만들기 전에 `CreateSequence`에서 제공하는 시퀀스가 없는지 확인합니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 응답자는 `ReplyTo` 끝점 참조를 사용하여 주소가 지정된 HTTP 요청에 `CreateSequenceResponse` 메시지를 전송합니다.  
...
#### CreateSequence 교환  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 개시자는 시퀀스를 제공하는 `CreateSequence` 메시지를 생성합니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 응답자는 시퀀스를 만들기 전에 `CreateSequence`에서 제공하는 시퀀스가 있는지 확인합니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]는 `CreateSequence`의 `ReplyTo` 끝점 참조로 주소가 지정된 HTTP 요청에 `CreateSequenceResponse` 메시지를 전송합니다.  
...
#### CreateSequence 교환  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 개시자는 시퀀스를 제공하는 `CreateSequence` 메시지를 생성합니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 응답자는 시퀀스를 만들기 전에 `CreateSequence`에서 제공하는 시퀀스가 있는지 확인합니다.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 응답자는 `CreateSequenceResponse` 메시지를 통해 `CreateSequence` 요청에 응답합니다.  

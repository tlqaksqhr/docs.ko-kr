...
## JSONP 사용  
 JSONP는 자동으로 사용되도록 설정되지 않습니다.  JSONP를 사용하도록 설정하려면 다음 예제와 같이 HTTP 표준 끝점\(<xref:System.ServiceModel.Description.WebHttpEndpoint> 또는 <xref:System.ServiceModel.Description.WebScriptEndpoint>\) 중 하나에서 `javascriptCallbackEnabled` 특성을 `true`로 설정합니다.  

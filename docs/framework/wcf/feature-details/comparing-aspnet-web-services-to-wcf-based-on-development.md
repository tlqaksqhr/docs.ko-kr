---
title: "개발을 기반으로 ASP.NET 웹 서비스와 WCF 비교 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# 개발을 기반으로 ASP.NET 웹 서비스와 WCF 비교
...
-   <xref:System.Collections.Hashtable>과 같은 <xref:System.Collections.IDictionary> 인터페이스를 구현하는 클래스는 XML로 serialize할 수 없습니다.  
...
## 서비스 설명  
 ...
 ASP.NET 2.0을 통해 서비스가 WS\-I\(웹 서비스 상호 운용성\) 조직의 Basic Profile 1.1과 호환되는지 확인하고 서비스가 WSDL과 호환된다는 클레임을 넣을 수 있게 되었습니다.  다음과 같이 <xref:System.Web.Services.WebServiceBindingAttribute> 특성의 `ConformsTo` 및 `EmitConformanceClaims` 매개 변수를 사용하면 됩니다.  
...
```  
 ...
 여기서 `TradingSystem` 서비스 유형에는 같은 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 클라이언트 인스턴스의 모든 호출이 서비스 유형의 같은 인스턴스로 라우트되도록 지정하는 <xref:System.ServiceModel.ServiceBehaviorAttribute>가 있습니다.  
...
### 보안: 클레임 기반 권한 부여  
 ...
 클레임 기반 인증은 클레임 집합을 작업의 액세스 요구 사항과 비교하여 그 결과에 따라 작업에 대한 액세스 권한을 부여하거나 거부함으로써 수행됩니다.  [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 <xref:System.ServiceModel.Description.ServiceAuthorizationBehavior>의 `ServiceAuthorizationManager` 속성에 값을 다시 한 번 할당함으로써 클레임 기반 권한 부여를 실행하는 데 사용할 클래스를 지정할 수 있습니다.  

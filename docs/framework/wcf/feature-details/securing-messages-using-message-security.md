---
title: "메시지 보안을 사용하여 메시지에 보안 설정 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# 메시지 보안을 사용하여 메시지에 보안 설정
이 절에서는 <xref:System.ServiceModel.NetMsmqBinding>을 사용하는 경우의 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 메시지 보안에 대해 설명합니다  
...
### Certificate  
 ...
 Windows를 실행하는 컴퓨터에서 인증서는 다양한 종류의 저장소에 저장됩니다.여러 저장소에 대한 [!INCLUDE[crabout](../../../../includes/crabout-md.md)]는 [인증서 저장소](http://go.microsoft.com/fwlink/?LinkId=87787)\(영문 페이지일 수 있음\)를 참조하십시오.  
...
### Windows  
 ...
 대기 중인 통신에 Kerberos 프로토콜을 사용하는 경우의 문제는 KDC\(키 배포 센터\)에서 배포하는 클라이언트 ID가 포함된 티켓의 수명이 비교적 짧다는 것입니다.*수명*은 티켓의 유효성을 나타내는 Kerberos 티켓에 연결됩니다.따라서 대기 시간이 긴 경우에는 클라이언트를 인증하는 서비스에서 토큰이 여전히 유효한지 확신할 수 없습니다.  
  
 이 자격 증명 형식을 사용하는 경우 서비스는 SERVICE 계정에서 실행해야 합니다.  
  
 메시지 자격 증명을 선택할 때, 기본적으로 Kerberos 프로토콜이 사용됩니다.[!INCLUDE[crdefault](../../../../includes/crdefault-md.md)][Exploring Kerberos, the Protocol for Distributed Security in Windows 2000](http://go.microsoft.com/fwlink/?LinkId=87790)을 참조하십시오.  

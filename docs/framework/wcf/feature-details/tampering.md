...
---
# 변조
*변조*는 메시지 변경 또는 메시지 배달 작업 및 변경된 메시지를 원래 목적 이외의 목적으로 사용하는 동작입니다.  
...
## WS\-Addressing 활성화  
 ...
 이러한 문제를 줄이려면 <xref:System.ServiceModel.Channels.MessageVersion.Addressing%2A> 속성을 <xref:System.ServiceModel.Channels.AddressingVersion.None%2A>으로 설정하지 않고 <xref:System.ServiceModel.Channels.MessageVersion.Addressing%2A> 속성을 <xref:System.ServiceModel.Channels.AddressingVersion.None%2A>로 설정하는 정적 <xref:System.ServiceModel.Channels.MessageVersion.Soap12%2A> 속성과 같이 <xref:System.ServiceModel.Channels.MessageVersion>을 사용하지 않습니다.  

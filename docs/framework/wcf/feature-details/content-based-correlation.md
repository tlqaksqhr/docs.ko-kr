...
> [!NOTE]
>  내용 기반 상관 관계는 지원되는 컨텍스트 교환 바인딩이 없어 컨텍스트 상관 관계를 사용할 수 없는 경우에 유용합니다.  컨텍스트 상관 관계에 대한 [!INCLUDE[crabout](../../../../includes/crabout-md.md)]는 [컨텍스트 교환](../../../../docs/framework/wcf/feature-details/context-exchange-correlation.md)을 참조하세요.  
  
 이러한 통신에 사용되는 각 메시징 작업은 메시지에서 인스턴스를 고유하게 식별하는 데이터가 있는 위치를 지정해야 합니다.  이 작업은 <xref:System.ServiceModel.Activities.QueryCorrelationInitializer> 또는 <xref:System.ServiceModel.Activities.Receive.CorrelatesOn%2A>을 통해 메시지에서 인스턴스를 고유하게 식별하는 데이터를 쿼리하는 <xref:System.ServiceModel.MessageQuerySet>을 제공하여 수행됩니다.  
...
>  인스턴스를 식별하는 데 사용되는 데이터는 상관 관계 키로 해시됩니다.  상관 관계에는 고유한 데이터를 사용해야 합니다. 그렇지 않으면 해시된 키에서 충돌이 발생하여 메시지가 잘못 라우트될 수 있습니다.  예를 들어 고객 이름에만 의존하는 상관 관계는 동일한 이름을 가진 고객이 여러 명 있을 수 있으므로 충돌을 일으킬 수 있습니다.  메시지를 상호 연결하는 데 사용되는 데이터에 콜론\(`:`\)을 포함하면 안 됩니다. 이는 이후에 해시되는 문자열을 만들기 위해 메시지 쿼리의 키와 값을 구분하는 데 콜론이 이미 사용되기 때문입니다.  
 ...
 다음 예제에서는 워크플로 서비스의 초기 <xref:System.ServiceModel.Activities.Receive>\/<xref:System.ServiceModel.Activities.SendReply>가 `OrderId`를 반환합니다. 이 주문 ID는 나중에 <xref:System.ServiceModel.Activities.Receive> 작업이 호출되면 클라이언트에 의해 워크플로 서비스로 다시 전달됩니다.  
...
## Workflow Designer를 사용하여 XPath 쿼리 구성  
 ...
 Workflow Designer에서 메시징 작업의 XPath를 구성하려면 Workflow Designer에서 작업을 선택합니다.  이전 예제와 같이 작업에 의해 상관 관계가 초기화되면 **속성** 창에서 **CorrelationInitializers** 속성에 대한 줄임표 단추를 클릭합니다.  그러면 **상관 관계 이니셜라이저 추가** 대화 상자 창이 나타납니다.  이 대화 상자에서 상관 관계 형식을 지정하고 상관 관계에 사용할 내용을 선택할 수 있습니다.  <xref:System.ServiceModel.Activities.CorrelationHandle> 변수는 **이니셜라이저 추가** 상자에서 지정하고 상관 관계에 사용되는 상관 관계 형식 및 데이터는 이 대화 상자의 **XPath 쿼리** 섹션에서 선택합니다.  
 ...
 상관 관계를 초기화하지 않는 메시징 작업에 대해 XPath 쿼리를 구성하려면 Workflow Designer에서 해당 작업을 선택한 다음 **속성** 창에서 **CorrelatesOn** 속성에 대한 줄임표 단추를 클릭합니다.  그러면 **CorrelatesOn 정의** 대화 상자 창이 나타납니다.  
  
 ![CorrelatesOn 정의](../../../../docs/framework/wcf/feature-details/media/correlatesondialog.jpg "CorrelatesOnDialog")  
  
 이 대화 상자에서 <xref:System.ServiceModel.Activities.CorrelationHandle>을 지정하고 **XPath 쿼리** 목록에서 항목을 선택하여 XPath 쿼리를 빌드합니다.
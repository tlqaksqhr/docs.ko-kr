...
-   인터넷 영역 권한  
 ...
 더 제한적인 사용 권한으로 부분 신뢰 응용 프로그램에서 WCF를 사용 하려고 런타임에 보안 예외가 발생할 수 있습니다.  
...
-   `[DataMember]` 형식의 serialize할 수 있는 모든 `[DataContract]` 필드 또는 속성은 public이고 읽기/쓰기가 가능해야 합니다. Serialization 및 deserialization [readonly](http://go.microsoft.com/fwlink/?LinkID=98854) 필드에는 부분적으로 신뢰할 수 있는 응용 프로그램에서 WCF를 실행 하는 경우 지원 되지 않습니다.  
...
### <a name="collection-types"></a>컬렉션 형식  
 ...
 컬렉션 형식의 경우 WCF가 부분 신뢰 환경에서 실행 되는 `GetEnumerator()` 구현은 `public`, 또는 명시적 인터페이스 구현에는 그 중 한 다음 보안 예외가 throw 됩니다.  
...
## <a name="configuration"></a>구성  
 단, 부분적으로 신뢰할 수 있는 코드 로드할 수 있습니다 로컬에서 WCF 구성 섹션 `app.config` 파일입니다. Machine.config에서 또는 루트에서 WCF 섹션을 참조 하는 WCF 구성 섹션을 로드 하려면 web.config 파일에는 ConfigurationPermission(Unrestricted) 필요 합니다. 이 사용 권한이 없는 경우 구성을 로드할 때 WCF 구성 섹션 (동작, 바인딩) 외부 예외가 로컬 구성 파일 결과에서 참조 합니다.  
...
### <a name="message-logging"></a>메시지 로깅  
 부분 신뢰 환경에서 WCF를 실행 하는 메시지 로깅이 작동 하지 않습니다. 부분 신뢰에서 사용하도록 설정된 경우 서비스 활성화는 실패하지 않지만 메시지가 기록되지 않습니다.  
...
> [!NOTE]
>  추적 파일에 중복 오류가 폭주를 방지 하려면 WCF 추적 리소스 또는 첫 번째 보안 실패 후 작업을 비활성화 합니다. 처음으로 리소스에 액세스하거나 작업을 수행할 때 실패한 각 리소스 액세스에 대해 한 가지 예외 추적이 있습니다.  
  
## <a name="wcf-service-host"></a>WCF 서비스 호스트  
 WCF 서비스 호스트는 부분 신뢰를 지원 하지 않습니다. 부분 신뢰 환경에서 WCF 서비스를 사용 하려는 경우 사용 하지 마십시오 WCF 서비스 라이브러리 프로젝트 템플릿을 Visual Studio에서 서비스를 빌드하기 위해. 대신, WCF 부분 신뢰가 지원 되는 웹 서버에서 서비스를 호스팅할 수 있는 WCF 서비스 웹 사이트 템플릿을 선택 하 여 Visual Studio에서 새 웹 사이트를 만듭니다.  
  
## <a name="other-limitations"></a>기타 제한 사항  
 WCF는 일반적으로 호스팅 응용 프로그램에 의해 적용 된 보안 고려 사항으로 제한 합니다. 예를 들어 WCF가를 호스팅하는 경우에 응용 프로그램 XBAP (XAML 브라우저), XBAP 제한 사항이 적용에 설명 된 대로 [Windows Presentation Foundation 부분 신뢰 보안](http://go.microsoft.com/fwlink/?LinkId=89138)합니다.  
...
-   성능 카운터  
 ...
 부분 신뢰 환경에서 지원 되지 않는 WCF 기능을 사용 하 여 런타임 시 예외가 발생할 수 있습니다.  
  
## <a name="unlisted-features"></a>목록에 없는 기능  
 부분 신뢰 환경에서 실행할 때 사용할 수 없는 정보 또는 작업 부분을 검색하는 가장 좋은 방법은 리소스에 액세스하거나 `try` 블록 내의 작업을 수행한 다음 오류를 `catch` 하는 것입니다. 추적 파일에 중복 오류가 폭주를 방지 하려면 WCF 추적 리소스 또는 첫 번째 보안 실패 후 작업을 비활성화 합니다. 처음으로 리소스에 액세스하거나 작업을 수행할 때 실패한 각 리소스 액세스에 대해 한 가지 예외 추적이 있습니다.  

...
-   **호스트 수준**: 호스트 공유 수준에서에서 캐시는 워크플로 서비스 호스트에서 호스팅되는 워크플로 인스턴스에서만에 사용할 수 있습니다. 또한 프로세스 수준 캐시에서 워크플로 서비스 호스트 간에 캐시를 공유할 수 있습니다.  
  
-   **인스턴스 수준**: 인스턴스 공유 수준, 캐시를 전체 수명 동안 특정 워크플로 인스턴스에 사용할 수 있지만 캐시는 다른 워크플로 인스턴스에서 사용할 수 없습니다.  
  
-   **캐시 없음**: 캐시는 기본적으로 해제 되어 구성에 정의 된 끝점을 사용 하는 워크플로 설정한 경우. 또한 이 경우 캐시를 설정하면 안전하지 않으므로 캐시를 해제된 상태로 두는 것이 좋습니다. 예를 들어 전송할 때마다 다른 ID(다른 자격 증명 또는 가장 사용)가 필요한 경우가 있습니다.  
...
## <a name="customizing-cache-settings"></a>캐시 설정 사용자 지정  

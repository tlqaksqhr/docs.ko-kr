...
이 항목에서는 클라이언트가 큐에서 읽는 서비스에 주소를 지정하는 방법 및 서비스 끝점이 큐에 매핑되는 방법을 설명합니다. 참고로는 다음 그림에서는 기본 Windows Communication Foundation (WCF) 대기 응용 프로그램 배포  
 ...
 클라이언트가 서비스에 메시지를 보낼 수 있도록 하려면 대상 큐에 메시지 주소를 지정합니다. 서비스가 큐에서 메시지를 읽을 수 있도록 하려면 해당 수신 주소를 대상 큐로 설정합니다. WCF에서 주소 지정은 URI Uniform Resource Identifier 기반 동안 메시지 큐 (MSMQ) 큐 이름은 URI 기반 하지 않습니다. 따라서 WCF를 사용 하 여 MSMQ에서 만든 큐 주소를 지정 하는 방법을 이해 하는 것이 반드시 합니다.  
...
## <a name="netmsmqbinding-and-service-addressing"></a>NetMsmqBinding 및 서비스 주소 지정  
 ...
 MSMQ 대기 중인 WCF 노출 전송은 net.msmq 체계를 사용 합니다. net.msmq 체계를 사용하여 주소가 지정된 메시지는 MSMQ 대기 중인 전송 채널을 통해 `NetMsmqBinding`을 사용하여 보내집니다.  
  
 WCF의 큐 주소 지정은 다음 패턴을 기반으로 합니다.  
...
-   [private]는 선택 사항입니다. 개인 큐인 대상 큐에 주소를 지정할 때 사용됩니다. 공개 큐에 주소를 지정하려면 private를 지정하지 않아야 합니다. Note, MSMQ 경로 달리 한지 "$" WCF URI 형식에서입니다.  
...
## <a name="msmqintegrationbinding-and-service-addressing"></a>MsmqIntegrationBinding 및 서비스 주소 지정  
 `MsmqIntegrationBinding`은 기존 MSMQ 응용 프로그램과의 통신에 사용됩니다. 기존 MSMQ 응용 프로그램의 상호 운용성을 보다 쉽게, WCF 유일한 형식 이름 주소 지정을 지원 합니다. 따라서 이 바인딩을 사용하여 보낸 메시지는 URI 체계를 따라야 합니다.  

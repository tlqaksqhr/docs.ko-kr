---
title: "이식 가능한 하위 집합 프로젝트에 서비스 참조 추가 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
이식 가능한 하위 집합 프로젝트를 사용하면 복수 .NET 플랫폼\(데스크톱, Silverlight, Windows Phone, XBOX\)을 지원하면서 .NET 어셈블리 프로그래머가 하나의 소스 트리를 유지하고 시스템을 빌드할 수 있습니다.이식 가능한 하위 집합 프로젝트는 모든 핵심 .NET 플랫폼에 사용 가능한 .NET Framework 어셈블리인 .NET 이식 가능 라이브러리만 참조합니다.  
...
5.  이식되지 않는 특성인 <xref:System.ComponentModel.DesignerCategoryAttribute>, <xref:System.Serializable> 및 <xref:System.ServiceModel.TransactionFlow>는 생성된 클라이언트 프록시 코드에서 제거됩니다.  
  
6.  이식되지 않는 속성인 ProtectionLevel, SessionMode, IsInitiating, IsTerminating은 <xref:System.ServiceModel.ServiceContractAttribute>, <xref:System.ServiceModel.OperationContract> 및 <xref:System.ServiceModel.FaultContract>에서 제거됩니다.  

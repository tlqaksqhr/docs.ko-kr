...
>  ComSvcConfig.exe를 사용하려면 로컬 컴퓨터의 관리자여야 합니다.  
 ...
 ComSvcConfig.exe에 대 한 자세한 내용은 참조 [하는 방법: COM + 서비스 모델 구성 도구를 사용 하 여](../../../docs/framework/wcf/feature-details/how-to-use-the-com-service-model-configuration-tool.md)합니다.  
...
|`/contract:` \<*ClassID* &#124; *ProgID* &#124; \*,*InterfaceID* &#124; *InterfaceName*    &#124; \*\>|서비스의 계약으로 구성할 COM+ 구성 요소 및 인터페이스를 지정합니다.<br /><br /> 약식은 `/c`입니다.<br /><br /> 하지만 와일드 카드 문자 (\*) 구성 요소 및 인터페이스 이름을 지정할 때 사용할 수 권장를 사용 하지 않는에 원하지 않은 인터페이스가 노출 될 수 있으므로 합니다.|  
...
### <a name="description"></a>설명  
 다음 예제는 OnlineStore COM+ 응용 프로그램에 있는 `IFinances` 구성 요소의 `ItemOrders.IFinancial` 인터페이스를 웹 서비스로 노출되는 인터페이스 집합에 COM+ 호스팅 모드를 사용하여 추가합니다. 발생한 모든 오류 이외에 경고를 모두 출력합니다.  
...
### <a name="description"></a>설명  
 다음 예제는 OnlineWarehouse COM+ 응용 프로그램에 있는 `IStockLevels` 구성 요소의 `ItemInventory.Warehouse` 인터페이스를 웹 서비스로 노출되는 인터페이스 집합에 웹 호스팅 모드를 사용하여 추가합니다. 웹 서비스는 IIS의 OnlineWarehouse 가상 디렉터리에서 웹 호스팅됩니다.  
...
### <a name="description"></a>설명  
 다음 예제는 OnlineStore COM+ 응용 프로그램에 있는 `IFinances` 구성 요소의 `ItemOrders.Financial` 인터페이스를 웹 서비스로 노출되는 인터페이스 집합에서 제거합니다.  

---
title: "전송: UDP 샘플에 의한 사용자 지정 트랜잭션 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# 전송: UDP 샘플에 의한 사용자 지정 트랜잭션
이 샘플은 [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)][전송 확장성](../../../../docs/framework/wcf/samples/transport-extensibility.md)의 [전송: UDP](../../../../docs/framework/wcf/samples/transport-udp.md) 샘플을 기반으로 합니다.이 샘플은 사용자 지정 트랜잭션 흐름을 지원하도록 UDP 전송 샘플을 확장하고 <xref:System.ServiceModel.Channels.TransactionMessageProperty> 속성의 사용 방법을 보여 줍니다.  
  
## UDP 전송 샘플의 코드 변경  
 트랜잭션 흐름을 보여 주기 위해 이 샘플에서는 `CalculatorService.Add()`의 트랜잭션 범위가 필요하도록 `ICalculatorContract`의 서비스 계약을 변경합니다.또한 `Add` 작업의 계약에 `System.Guid` 매개 변수를 추가합니다.이 매개 변수는 클라이언트 트랜잭션의 식별자를 서비스에 전달하는 데 사용됩니다.  
...
```  
 ...
 [전송: UDP](../../../../docs/framework/wcf/samples/transport-udp.md) 샘플에서는 클라이언트와 서비스 간에 메시지를 전달하는 데 UDP 패킷을 사용합니다.[Transport: Custom Transport Sample](../../../../docs/framework/wcf/samples/transport-custom-transactions-over-udp-sample.md)에서도 동일한 메커니즘을 사용하여 메시지를 전송하지만 트랜잭션이 이동하면 인코딩된 메시지와 함께 UDP 패킷에 삽입됩니다.  
...
2.  현재 샘플은 [전송: UDP](../../../../docs/framework/wcf/samples/transport-udp.md) 샘플과 비슷하게 실행해야 합니다.이 샘플을 실행하려면 UdpTestService.exe를 실행하여 서비스를 시작합니다.[!INCLUDE[windowsver](../../../../includes/windowsver-md.md)]를 실행하는 경우에는 높은 권한으로 서비스를 시작해야 합니다.이렇게 하려면 [!INCLUDE[fileExplorer](../../../../includes/fileexplorer-md.md)]에서 UdpTestService.exe를 마우스 오른쪽 단추로 클릭하고 **관리자 권한으로 실행**을 클릭합니다.  
...
6.  서비스 응용 프로그램은 클라이언트가 보낸, `CalculatorService.Add()` 작업의 `clientTransactionId` 매개 변수에 포함된 트랜잭션 식별자를 서비스 트랜잭션의 식별자와 일치시킬 수 있는 경우 `The client transaction has flowed to the service`라는 메시지를 표시합니다.이 두 식별자는 클라이언트 트랜잭션이 서비스로 이동한 경우에만 일치합니다.  

...
---
# WS Transaction Flow
이 샘플에서는 클라이언트에서 조정하는 트랜잭션의 사용법과 WS\-Atomic Transaction 또는 OleTransactions 프로토콜을 사용하는 트랜잭션 흐름의 클라이언트 및 서버 옵션을 보여 줍니다.  이 샘플은 계산기 서비스를 구현하는[시작](../../../../docs/framework/wcf/samples/getting-started-sample.md)을 기반으로 하지만 작업에서는 **TransactionFlowOption** 열거형과 함께 `TransactionFlowAttribute`를 사용하여 트랜잭션 흐름을 사용할 수 있는 범위를 결정하는 방법을 보여 줍니다.  흐름이 지정된 트랜잭션의 범위 내에서는 요청한 작업에 대한 로그가 데이터베이스에 기록되고 클라이언트에서 조정하는 트랜잭션이 완료될 때까지 유지됩니다. 클라이언트 트랜잭션이 완료되지 않으면 웹 서비스 트랜잭션에서 데이터베이스에 적합한 업데이트가 커밋되지 않도록 합니다.  
...
-   `Divide` 연산 요청에는`TransactionFlow` 특성을 생략하여 흐름이 지정된 트랜잭션을 포함하지 않아야 합니다.  
 ...
 트랜잭션 흐름을 사용하려면 적합한 작업 특성과 함께 [\<transactionFlow\>](../../../../docs/framework/configure-apps/file-schema/wcf/transactionflow.md) 속성이 활성화된 바인딩을 사용해야 합니다.  이 샘플의 서비스 구성에서는 메타데이터 교환 끝점뿐만 아니라 TCP 끝점과 HTTP 끝점을 노출합니다.  TCP 끝점과 HTTP 끝점은 모두 다음과 같이 [\<transactionFlow\>](../../../../docs/framework/configure-apps/file-schema/wcf/transactionflow.md) 속성이 활성화된 바인딩을 사용합니다.  
...
-   `Multiply` 요청에서는`ICalculator` 인터페이스에 대해 클라이언트에서 생성한 정의에 <xref:System.ServiceModel.TransactionFlowOption>이`NotAllowed`로 설정된 <xref:System.ServiceModel.TransactionFlowAttribute>가 포함되므로 트랜잭션의 흐름을 서비스로 지정하지 않습니다.  
...
3.  단일 컴퓨터 또는 다중 컴퓨터 구성에서 샘플을 실행하려면 [Windows Communication Foundation 샘플 실행](../../../../docs/framework/wcf/samples/running-the-samples.md)의 지침을 따릅니다.  
    ...
    > [!NOTE]
    >  다중 컴퓨터 구성의 경우 아래의 지침을 사용하여 DTC\(Distributed Transaction Coordinator\)를 사용하도록 설정하고 Windows SDK에서 WsatConfig.exe 도구를 사용하여 WCF 트랜잭션 네트워크 지원을 사용하도록 설정합니다.  WsatConfig.exe 설정에 대한 자세한 내용은 [WS\-Atomic Transaction 지원 구성](http://go.microsoft.com/fwlink/?LinkId=190370)을 참조하세요.  
...
### 샘플을 실행할 수 있도록 MSDTC\(Microsoft Distributed Transaction Coordinator\)를 구성하려면  
...
1.  Windows Server 2003 또는 Windows XP를 실행하는 서비스 컴퓨터에서 다음 지침에 따라 들어오는 네트워크 트랜잭션을 허용하도록 MSDTC를 구성합니다.  
    ...
    2.  **구성 요소 서비스**를 확장하고  **컴퓨터** 폴더를 연 다음  
  
    3.  **내 컴퓨터**를 마우스 오른쪽 단추로 클릭하고 **속성**을 선택합니다.  
  
    4.  **MSDTC** 탭에서 **보안 구성**을 클릭합니다.  
  
    5.  **네트워크 DTC 액세스** 및 **인바운드 허용**을 선택합니다.  
  
    6.  **확인**을 클릭한 다음**예**를 클릭하여 MSDTC 서비스를 다시 시작합니다.  
...
2.  Windows Server 2008 또는 Windows Vista를 실행하는 서비스 컴퓨터에서 다음 지침에 따라 들어오는 네트워크 트랜잭션을 허용하도록 MSDTC를 구성합니다.  
    ...
    2.  **구성 요소 서비스**를 확장하고  **컴퓨터** 폴더를 연 다음  **Distributed Transaction Coordinator**를 선택합니다.  
  
    3.  **DTC Coordinator**를 마우스 오른쪽 단추로 클릭하고 **속성**을 선택합니다.  
  
    4.  **보안** 탭에서 **네트워크 DTC 액세스** 및 **인바운드 허용**을 선택합니다.  
  
    5.  **확인**을 클릭한 다음**예**를 클릭하여 MSDTC 서비스를 다시 시작합니다.  

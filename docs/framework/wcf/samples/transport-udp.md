...
1.  ChannelFactory 및 ChannelListener에서 지원할 채널 [메시지 교환 패턴](#MessageExchangePatterns)\(IOutputChannel, IInputChannel, IDuplexChannel, IRequestChannel 또는 IReplyChannel\)을 결정합니다.그런 다음 이러한 인터페이스의 세션 변형을 지원할지 여부를 결정합니다.  
...
4.  사용자 지정 전송을 채널 스택에 추가하는 [\<binding\>](../../../../docs/framework/misc/binding.md) 요소를 추가합니다.자세한 내용은 [바인딩 요소 추가](#AddingABindingElement)를 참조하십시오.  
  
5.  새 바인딩 요소가 구성 시스템에 노출되도록 바인딩 요소 확장 섹션을 추가합니다.  
  
6.  기능을 다른 끝점에 전달하도록 메타데이터 확장을 추가합니다.  
  
7.  올바르게 정의된 프로필에 따라 바인딩 요소 스택을 미리 구성하는 바인딩을 추가합니다.자세한 내용은 [표준 바인딩 추가](#AddingAStandardBinding)를 참조하십시오.  
  
8.  바인딩이 구성 시스템에 노출되도록 바인딩 섹션 및 바인딩 구성 요소를 추가합니다.자세한 내용은 [구성 지원 추가](#AddingConfigurationSupport)를 참조하십시오.  
...
### ICommunicationObject 및 WCF 개체 수명 주기  
...
-   Faulted: Faulted 상태의 개체는 검사를 위해 액세스할 수 있지만 더 이상 사용할 수는 없습니다.복구 불가능한 오류가 발생하면 해당 개체는 이 상태로 전환됩니다.이 상태에서는 `Closed` 상태로만 전환될 수 있습니다.  
...
### UDP 채널 팩터리  
 `UdpChannelFactory`는 <xref:System.ServiceModel.Channels.ChannelFactoryBase>에서 파생됩니다.샘플에서는 메시지 인코더의 메시지 버전에 액세스할 수 있도록 <xref:System.ServiceModel.Channels.ChannelFactoryBase.GetProperty%2A>를 재정의합니다.또한 샘플에서는 상태 시스템이 전환될 때 <xref:System.ServiceModel.Channels.BufferManager>의 인스턴스를 중지할 수 있도록 <xref:System.ServiceModel.Channels.ChannelFactoryBase.OnClose%2A>를 재정의합니다.  
...
```  
 ...
 그런 다음 `Send()` 및 `BeginSend()`\/`EndSend()`를 구현합니다.그러면 두 개의 기본 섹션으로 분할됩니다.먼저 메시지를 바이트 배열로 serialize합니다.  
...
### UdpChannelListener  
 샘플에서 구현하는 ``UdpChannelListener는 <xref:System.ServiceModel.Channels.ChannelListenerBase> 클래스에서 파생되며,단일 UDP 소켓을 사용하여 데이터그램을 받습니다.`OnOpen` 메서드는 비동기 루프에서 UDP 소켓을 사용하여 데이터를 받습니다.그런 다음 데이터는 메시지 인코딩 프레임워크를 통해 메시지로 변환됩니다.  
  
```  
message = MessageEncoderFactory.Encoder.ReadMessage(new ArraySegment<byte>(buffer, 0, count), bufferManager);  
```  
  
 같은 데이터그램 채널은 여러 소스에서 도착한 메시지를 나타내므로 `UdpChannelListener`는 singleton 수신기입니다.한 번에 최대 하나의 활성 <xref:System.ServiceModel.Channels.IChannel>이이 수신기에 연결됩니다.이 샘플에서는 `AcceptChannel` 메서드에서 반환되는 채널이 이후에 삭제되는 경우에만 새 채널을 생성합니다.메시지를 받으면 이 singleton 채널의 큐에 삽입됩니다.  
...
## 바인딩 요소 추가  
 이제 팩터리와 채널이 빌드되었으므로 바인딩을 통해 ServiceModel 런타임에 팩터리와 채널을 노출해야 합니다.바인딩은 서비스 주소와 연결된 통신 스택을 나타내는 바인딩 요소의 컬렉션입니다.스택의 각 요소는 [\<binding\>](../../../../docs/framework/misc/binding.md) 요소로 표현됩니다.  
  
 이 샘플의 바인딩 요소는 <xref:System.ServiceModel.Channels.TransportBindingElement>에서 파생된 `UdpTransportBindingElement`로,바인딩에 연결되는 팩터리를 빌드하기 위해 다음 메서드를 재정의합니다.  
...
## 전송 바인딩 요소에 대해 메타데이터 지원 추가  
 전송을 메타데이터 시스템에 통합하려면 정책 가져오기 및 내보내기를 모두 지원해야 합니다.그러면 [ServiceModel Metadata 유틸리티 도구\(Svcutil.exe\)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md)를 통해 바인딩의 클라이언트를 생성할 수 있습니다.  
...
```  
 ...
 끝점에서 SOAP 바인딩을 사용할 때 `ExportEndpoint` 메서드의 `UdpTransportBindingElement` 구현이 전송 URI도 내보냅니다.  
...
#### 정책 내보내기  
 `UdpTransportBindingElement` 형식은 `IPolicyExportExtension`을구현하여 정책 내보내기 지원을 추가합니다.그 결과, `System.ServiceModel.MetadataExporter`는 이를 포함하는 모든 바인딩에 대한 정책 생성에 `UdpTransportBindingElement`를 포함합니다.  
...
```  
 ...
 사용자 지정 전송 바인딩 요소가 주소 지정을 처리하기 때문에 `UdpTransportBindingElement`에서의 `IPolicyExportExtension` 구현 역시 사용 중인 WS\-Addressing의 버전을 나타내기 위해 적절한 WS\-Addressing 정책 어설션 내보내기를 처리해야 합니다.  
...
```  
 ...
 그런 다음 등록된 클래스\(`UdpBindingElementImporter`\)로부터 `IPolicyImporterExtension`을 구현합니다.`ImportPolicy()`에서는 네임스페이스의 어설션을 검토하면서 전송 생성을 위한 어설션을 처리하고 멀티캐스트인지 여부를 확인합니다.또한 처리하는 어설션을 바인딩 어설션 목록에서 제거해야 합니다.다시 한 번 말하지만 Svcutil.exe를 실행할 때 통합하는 방법은 두 가지가 있습니다.  
...
-   해당 바인딩 요소가 포함된 시스템 제공 바인딩 사용[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]에서는 `BasicHttpBinding`, `NetTcpBinding`, `WsHttpBinding` 등의 다양한 시스템 정의 바인딩을 제공합니다.이러한 바인딩 각각은 제대로 정의된 프로필에 연결됩니다.  
 ...
 샘플에서는 <xref:System.ServiceModel.Channels.Binding>에서 파생된 `SampleProfileUdpBinding`의 프로필 바인딩을 구현합니다.`SampleProfileUdpBinding`은 최대 네 개의 바인딩 요소, 즉 `UdpTransportBindingElement`, `TextMessageEncodingBindingElement CompositeDuplexBindingElement` 및 `ReliableSessionBindingElement`를 포함합니다.  
...
### 사용자 지정 표준 바인딩 가져오기 추가  
 ...
 사용자 지정 표준 바인딩 가져오기는 메타데이터로부터 가져온 `CustomBinding` 인스턴스를 검사하여 특정 표준 바인딩에 의해 생성되었는지 여부를 확인하기 위해 `IWsdlImportExtension` 인터페이스에 `ImportEndpoint` 메서드를 구현합니다.  
...
## 구성 지원 추가  
 구성을 통해 전송을 노출하려면 두 가지 구성 섹션을 구현해야 합니다.첫 번째는 `UdpTransportBindingElement`의 `BindingElementExtensionElement`입니다.이는 `CustomBinding` 구현이 해당 바인딩 요소를 참조하기 위해 필요합니다.두 번째는 `SampleProfileUdpBinding`의 `Configuration`입니다.  
  
### 바인딩 요소 확장 요소  
 `UdpTransportElement` 섹션은구성 시스템에 `UdpTransportBindingElement`를 노출하는 `BindingElementExtensionElement`입니다.몇 가지 기본 재정의를 통해 구성 섹션 이름, 바인딩 요소의 형식 및 바인딩 요소를 만드는 방법을 정의합니다.그러면 다음 코드와 같이 구성 파일에 확장 섹션을 등록할 수 있습니다.  
...
### 바인딩 섹션  
 `SampleProfileUdpBindingCollectionElement` 섹션은 구성 시스템에 `SampleProfileUdpBinding`을 노출하는 `StandardBindingCollectionElement`입니다.대량의 구현은 `StandardBindingElement`로부터 파생되는 `SampleProfileUdpBindingConfigurationElement`에 위임됩니다.`SampleProfileUdpBindingConfigurationElement`에는 `SampleProfileUdpBinding`의 속성에 해당하는 속성과 `ConfigurationElement` 바인딩에서 매핑하는 함수가 있습니다.마지막으로 다음 샘플 코드와 같이 `SampleProfileUdpBinding`에서 `OnApplyConfiguration` 메서드를 재정의합니다.  
...
## UDP 테스트 서비스 및 클라이언트  
 이 샘플 전송을 사용하기 위한 테스트 코드는 UdpTestService 및 UdpTestClient 디렉터리에 있습니다.서비스 코드는 두 가지 테스트로 구성됩니다. 하나는 코드에서 바인딩 및 끝점을 설정하고, 다른 하나는 구성을 통해 바인딩 및 끝점을 설정합니다.두 테스트 모두 두 개의 끝점을 사용합니다.끝점 하나는 [\<reliableSession\>](http://msdn.microsoft.com/ko-kr/9c93818a-7dfa-43d5-b3a1-1aafccf3a00b)이 `true`로 설정된 `SampleUdpProfileBinding`을 사용합니다.다른 끝점은 `UdpTransportBindingElement`가 포함된 사용자 지정 바인딩을 사용합니다.이는 [\<reliableSession\>](http://msdn.microsoft.com/ko-kr/9c93818a-7dfa-43d5-b3a1-1aafccf3a00b)이 `false`로 설정된 `SampleUdpProfileBinding`을 사용하는 것과 같습니다.두 테스트 모두 서비스를 만들고 각 바인딩에 끝점을 추가하며 서비스를 연 다음 서비스를 닫기 전에 사용자가 Enter 키를 누를 때까지 기다립니다.  

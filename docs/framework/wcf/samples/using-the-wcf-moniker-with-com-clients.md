...
## 형식화된 계약  
 형식화된 계약 사용과 함께 모니커를 사용하려면 서비스 계약에 대한 적절한 특성 사용 형식을 COM에 등록해야 합니다.  먼저 [ServiceModel Metadata 유틸리티 도구\(Svcutil.exe\)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md)를 사용하여 클라이언트를 생성해야 합니다.  클라이언트 디렉터리의 명령 프롬프트에서 다음 명령을 실행하여 형식화된 프록시를 생성합니다.  
...
4.  `tlbexp.exe client.dll /out:CalcProxy.tlb`를 입력하여 dll을 tlb 파일로 내보냅니다.  "형식 라이브러리 내보내기 경고"가 발생할 수 있지만 제네릭 형식이 필요하지 않으므로 문제가 되지는 않습니다.  
  
5.  `regasm.exe /tlb:CalcProxy.tlb client.dll`을 입력하여 형식을 COM에 등록합니다.  "형식 라이브러리 내보내기 경고"가 발생할 수 있지만 제네릭 형식이 필요하지 않으므로 문제가 되지는 않습니다.  
  
6.  `gacutil.exe /i client.dll`을 입력하여 어셈블리를 전역 어셈블리 캐시에 추가합니다.  
...
3.  클라이언트와 서비스가 통신할 수 없는 경우 [Troubleshooting Tips](http://msdn.microsoft.com/ko-kr/8787c877-5e96-42da-8214-fa737a38f10b)을 참조하세요.  
...
8.  `tlbexp.exe client.dll /out:CalcProxy.tlb`를 입력하여 dll을 tlb 파일로 내보냅니다.  "형식 라이브러리 내보내기 경고"가 발생할 수 있지만 제네릭 형식이 필요하지 않으므로 문제가 되지는 않습니다.  
  
9. `regasm.exe /tlb:CalcProxy.tlb client.dll`을 입력하여 형식을 COM에 등록합니다.  명령을 실행하기 전에 `regasm.exe`가 포함된 폴더로 경로가 설정되었는지 확인합니다.  
  
10. `gacutil.exe /i client.dll`을 입력하여 어셈블리를 전역 어셈블리 캐시에 추가합니다.  명령을 실행하기 전에 `gacutil.exe`가 포함된 폴더로 경로가 설정되었는지 확인합니다.  

---
title: "사용자 지정 채널 디스패처 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## 사용자 지정 ServiceHostBase  
 이 샘플에서는 <xref:System.ServiceModel.ServiceHost> 대신 기본 형식 <xref:System.ServiceModel.ServiceHostBase>를 구현하여 [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 스택 구현을 채널 스택 맨 위의 사용자 지정 메시지 처리 계층으로 대체하는 방법을 보여 줍니다.가상 메서드 <xref:System.ServiceModel.ServiceHostBase.InitializeRuntime%2A>을 재정의하여 채널 수신기 및 채널 디스패처를 빌드합니다.  
  
 웹 호스팅 서비스를 구현하려면 전송 계층에서 호스팅 환경 설정, 즉 IIS\(인터넷 정보 서비스\)\/WAS\(Windows Process Activation Service\) 설정을 기반으로 채널 수신기를 구성하는 방법을 알 수 있도록 <xref:System.ServiceModel.ServiceHostBase.Extensions%2A> 컬렉션에서 서비스 확장 <xref:System.ServiceModel.Activation.VirtualPathExtension>을 가져와 <xref:System.ServiceModel.Channels.BindingParameterCollection>에 추가합니다.  

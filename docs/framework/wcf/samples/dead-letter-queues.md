---
title: "배달 못 한 편지 큐 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# 배달 못 한 편지 큐
이 샘플에서는 배달에 실패한 메시지를 처리하는 방법을 보여 줍니다.  이 샘플은 [트랜잭션된 MSMQ 바인딩](../../../../docs/framework/wcf/samples/transacted-msmq-binding.md) 샘플을 기반으로 하며,  이 샘플에서는 `netMsmqBinding` 바인딩을 사용합니다.  이 서비스는 자체적으로 호스트되는 콘솔 응용 프로그램으로서 이를 사용하여 서비스에서 대기된 메시지를 받는 것을 볼 수 있습니다.  
...
```  
 ...
 이 샘플의 서비스 코드는 [트랜잭션된 MSMQ 바인딩](../../../../docs/framework/wcf/samples/transacted-msmq-binding.md)의 서비스 코드입니다.  
...
>  배달 못 한 편지 큐는 클라이언트 큐이며 클라이언트 큐 관리자에 대해 로컬입니다.  
 ...
 배달 못 한 메시지 서비스 구현에서는 메시지 배달 실패의 원인을 확인하고 올바른 방법으로 해결합니다.  메시지 실패 원인은 <xref:System.ServiceModel.Channels.MsmqMessageProperty.DeliveryFailure%2A> 및 <xref:System.ServiceModel.Channels.MsmqMessageProperty.DeliveryStatus%2A> 열거형으로 캡처됩니다.  다음 샘플 코드와 같이 <xref:System.ServiceModel.OperationContext>에서 <xref:System.ServiceModel.Channels.MsmqMessageProperty>를 검색할 수 있습니다.  
...
4.  단일 컴퓨터 또는 다중 컴퓨터 구성에서 샘플을 실행하려면 큐 이름을 적절하게 변경하여 localhost를 컴퓨터의 전체 이름으로 바꾸고 [Windows Communication Foundation 샘플 실행](../../../../docs/framework/wcf/samples/running-the-samples.md)의 지침을 따릅니다.  

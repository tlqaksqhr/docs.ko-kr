...
> [!NOTE]
>  <xref:System.Messaging>을 사용하여 큐를 만들 때 큐 이름은 로컬 컴퓨터에 점(.)을, 그 경로에는 백슬래시 구분 기호를 사용합니다. Windows Communication Foundation (WCF) 끝점 주소 지정에서는 net.msmq: 체계를 "localhost"는 로컬 컴퓨터 및 해당 경로에 슬래시를 사용 합니다.  

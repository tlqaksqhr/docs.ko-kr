...
> [!IMPORTANT]
>  FindPrivateKey는 사용하기 전에 컴파일되어야 하는 샘플입니다. FindPrivateKey 도구를 작성 하는 방법에 "FindPrivateKey 프로젝트를 빌드" 절을 아래의 지침은 참조 하십시오.  
...
3.  에 **빌드** 메뉴 선택 **솔루션 다시 빌드**합니다. 클라이언트 프로그램 파일이 client\bin에 빌드되고 서비스 프로그램 파일이 service\bin에 빌드됩니다.  

...
```  
 ...
 두 번째 필터는 `calculatorEndpoint`에서 받은 모든 메시지를 일치시키는 <xref:System.ServiceModel.Dispatcher.EndpointNameMessageFilter>입니다.끝점 이름은 서비스 끝점 개체가 만들어질 때 정의됩니다.  
...
9. 클라이언트 콘솔 창에서 `quit`를 입력하고 Enter 키를 눌러 종료합니다.  

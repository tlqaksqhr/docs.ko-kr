...
```  
 ...
 `CalculatorService` 클래스는 기본 `ICalculatorDuplex` 인터페이스를 구현합니다.  서비스는 <xref:System.ServiceModel.InstanceContextMode> 인스턴스 모드를 사용하여 각 세션의 결과를 유지합니다.  `Callback`이라는 private 속성은 클라이언트에 대한 콜백 채널에 액세스하는 데 사용됩니다.  서비스는 콜백 인터페이스를 통해 클라이언트에 메시지를 다시 전송하기 위해 콜백을 사용합니다.  
...
```  
 ...
 클라이언트는 서비스에서 메시지를 수신하기 위해 이중 계약의 콜백 인터페이스를 구현하는 클래스를 제공해야 합니다.  이 샘플에서는 `ICalculatorDuplexCallback` 인터페이스를 구현하기 위해 `CallbackHandler` 클래스가 정의됩니다.  
...
3.  단일 컴퓨터 또는 다중 컴퓨터 구성에서 샘플을 실행하려면 [Windows Communication Foundation 샘플 실행](../../../../docs/framework/wcf/samples/running-the-samples.md)의 지침을 따릅니다.  
    ...
    > [!IMPORTANT]
    >  다중 컴퓨터 구성에서 클라이언트를 실행할 경우 다음에 표시된 것처럼 [endpoint](http://msdn.microsoft.com/ko-kr/13aa23b7-2f08-4add-8dbf-a99f8127c017) 요소의 `address` 특성과 [\<wsDualHttpBinding\>](../../../../docs/framework/configure-apps/file-schema/wcf/wsdualhttpbinding.md) 요소의 [\<binding\>](../../../../docs/framework/misc/binding.md) 요소의 `clientBaseAddress` 특성에서 "localhost"를 해당 컴퓨터의 이름으로 바꾸어야 합니다.  

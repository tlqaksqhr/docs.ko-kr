...
## HttpCookieSession 채널 메시지 교환 패턴  
...
## 구성 시스템에 채널 추가  
 이 샘플에서는 구성을 통해 샘플 채널을 노출하는 두 개의 클래스를 제공합니다.첫 번째 클래스는 `HttpCookieSessionBindingElement`의 <xref:System.ServiceModel.Configuration.BindingElementExtensionElement>입니다대량의 구현은 <xref:System.ServiceModel.Configuration.StandardBindingElement>로부터 파생되는 `HttpCookieSessionBindingConfigurationElement`에 위임됩니다.`HttpCookieSessionBindingConfigurationElement`에는 `HttpCookieSessionBindingElement`의 속성에 대응하는 속성이 있습니다.  
  
### 바인딩 요소 확장 섹션  
 `HttpCookieSessionBindingElementSection` 섹션은 구성 시스템에 `HttpCookieSessionBindingElement`를 노출하는 <xref:System.ServiceModel.Configuration.BindingElementExtensionElement>입니다.몇 가지 재정의를 통해 구성 섹션 이름, 바인딩 요소의 형식 및 바인딩 요소를 만드는 방법을 정의합니다.그러면 다음과 같이 구성 파일의 확장 섹션을 등록할 수 있습니다.  
...
## 테스트 코드  
 이 샘플 전송을 사용하기 위한 테스트 코드는 Client 및 Service 디렉터리에 있습니다.이 코드는 두 개의 테스트로 구성되며 이 중 한 테스트는 클라이언트에서 `allowCookies`가 `true`로 설정된 바인딩을 사용합니다.두 번째 테스트에서는 종료 메시지 교환을 사용하여 바인딩에서 명시적인 종료를 사용하도록 설정합니다.  

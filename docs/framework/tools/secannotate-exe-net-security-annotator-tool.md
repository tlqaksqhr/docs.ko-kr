...
.NET Security Annotator 도구\(SecAnnotate.exe\)는 하나 이상의 어셈블리의 `SecurityCritical` 및 `SecuritySafeCritical` 부분을 식별하는 명령줄 응용 프로그램입니다.  
 ...
 명령 프롬프트에서 다음을 입력하십시오. *parameters*는 다음 섹션에서 설명하며 *assemblies*는 공백으로 구분된 하나 이상의 어셈블리 이름으로 구성됩니다.  
...
## 설명  
 ...
 .NET Security Annotator에 대한 자세한 내용은 .NET Security 블로그의 [SecAnnotate를 사용하여 어셈블리의 투명도 위반 분석](http://go.microsoft.com/fwlink/?LinkId=187648) 항목을 참조하십시오.  

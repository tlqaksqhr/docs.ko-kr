---
title: "Lc.exe(라이선스 컴파일러) | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
helpviewer_keywords: 
  ...
  - ".licx 파일"
  - "라이선스 파일 컴파일"
  - "컨트롤 라이선스[Windows Forms]"
  - "Lc.exe"
  - "라이선스 컴파일러"
  - "라이선스 파일"
  - "라이선스가 있는 컨트롤[Windows Forms]"
  - "Windows Forms, 컨트롤 라이선스"
...
|옵션|설명|  
|--------|--------|  
|**\/complist:** *파일 이름*|.licenses 파일에 포함할 라이선스가 있는 구성 요소의 목록이 들어 있는 파일 이름을 지정합니다.  각 구성 요소는 한 줄에 하나의 구성 요소로 전체 이름을 사용하여 참조됩니다.<br /><br /> 명령줄 사용자는 프로젝트의 각 형식마다 별도의 파일을 지정할 수 있습니다.  Lc.exe로 여러 개의 입력 파일을 사용하여 하나의 .licenses 파일을 생성할 수도 있습니다.|  
...
|**\/outdir:** *경로*|출력된 .licenses 파일을 포함할 디렉터리를 지정합니다.|  
...
## 지시 파일 예  
 다음 목록에서는 지시 파일의 예로 `response.rsp`를 보여 줍니다.  지시 파일에 대한 자세한 내용은 [Response Files](../Topic/MSBuild%20Response%20Files.md)을 참조하세요.  

---
title: "Fuslogvw.exe(어셈블리 바인딩 로그 뷰어) | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
dev_langs: 
  ...
  - "jsharp"
helpviewer_keywords: 
  - "어셈블리[.NET Framework], 실패한 어셈블리 바인딩"
  - "어셈블리 바인딩 로그 뷰어"
  - "실패한 어셈블리 바인딩 정보 표시"
  - "실패한 어셈블리 바인딩"
  - "Fuslogvw.exe"
...
-   **새로 고침** 단추를 클릭합니다.  뷰어는 실행 중인 동안에는 새 로그 엔트리를 자동으로 검색하지 않습니다.  새 엔트리를 표시하려면 **새로 고침** 단추를 사용해야 합니다.  
...
> [!IMPORTANT]
>  예를 들어 <xref:System.AppDomainSetup.LoaderOptimization%2A> 속성을 <xref:System.LoaderOptimization?displayProperty=fullName> 또는 <xref:System.LoaderOptimization?displayProperty=fullName>로 설정하여 어셈블리가 도메인 중립적으로 로드되는 경우 로깅을 설정하면 경우에 따라 메모리가 누수될 수 있습니다.  도메인 중립 모듈이 응용 프로그램 도메인에 로드되었을 때 로그 엔트리가 만들어지고 나중에 응용 프로그램 도메인이 언로드되면 이러한 누수가 발생할 수 있습니다.  프로세스가 끝날 때까지는 로그 엔트리가 해제되지 않을 수도 있습니다.  로깅이 자동으로 설정되는 디버거도 있습니다.  
...
#### Windows 앱 컨테이너에서 실행되는 앱에 대해 로깅을 사용하려면  

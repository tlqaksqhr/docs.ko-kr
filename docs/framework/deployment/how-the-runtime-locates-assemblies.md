...
dev_langs: 
  ...
  - "jsharp"
helpviewer_keywords: 
  - ".NET Framework 응용 프로그램 배포, 어셈블리 찾기"
  - "app.config 파일, 어셈블리 위치"
  - "응용 프로그램 구성 파일, 어셈블리 찾기"
  - "어셈블리[.NET Framework], 위치"
  - "응용 프로그램 배포[.NET Framework], 어셈블리 위치"
  - "어셈블리 찾기"
...
> [!NOTE]
>  [!INCLUDE[winsdklong](../../../includes/winsdklong-md.md)]에 포함된 [어셈블리 바인딩 로그 뷰어\(Fuslogvw.exe\)](../../../docs/framework/tools/fuslogvw-exe-assembly-binding-log-viewer.md)를 사용하여 로그 파일의 바인딩 정보를 볼 수 있습니다.  
...
> [!NOTE]
>  바인딩 실패를 캐시하지 않은 .NET Framework 버전 1.0 및 1.1의 동작으로 되돌리려면 [\<disableCachingBindingFailures\> 요소](../../../docs/framework/configure-apps/file-schema/runtime/disablecachingbindingfailures-element.md) 요소를 구성 파일에 포함합니다.  
...
> [!NOTE]
>  같은 디렉터리에 여러 버전의 어셈블리가 있고 그중에서 특정 버전을 참조하려는 경우 [\<probing\>](../../../docs/framework/configure-apps/file-schema/runtime/codebase-element.md) 요소의 `privatePath` 특성 대신 [\<codeBase\>](../../../docs/framework/configure-apps/file-schema/runtime/probing-element.md) 요소를 사용해야 합니다.[\<probing\>](../../../docs/framework/configure-apps/file-schema/runtime/probing-element.md) 요소를 사용할 경우 참조되는 단순 어셈블리 이름에 일치하는 어셈블리를 런타임에서 처음 찾았으면 정확하게 일치하는지 여부에 관계없이 검색이 중지됩니다. 정확하게 일치하는 항목이면 해당 어셈블리가 사용됩니다. 정확하게 일치하는 항목이 아니면 검색이 중지되고 바인딩이 실패합니다.  

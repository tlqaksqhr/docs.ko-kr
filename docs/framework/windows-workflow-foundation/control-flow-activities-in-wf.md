---
title: "WF의 흐름 제어 활동 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|<xref:System.Activities.Statements.ForEach%601>|컬렉션의 각 요소에 대한 포함 문을 직렬로 실행합니다.<xref:System.Activities.Statements.ForEach%601>은 `foreach` 키워드와 유사하지만 언어 문이 아니라 활동으로 구현됩니다.|  
|<xref:System.Activities.Statements.If>|조건이 `true`이면 포함된 활동을 실행하고, 조건이 `false`이면 <xref:System.Activities.Statements.If.Else%2A> 속성에 포함된 활동을 실행할 수 있습니다.|  

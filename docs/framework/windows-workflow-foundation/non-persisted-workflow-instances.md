---
title: "비지속형 워크플로 인스턴스 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   워크플로 인스턴스가 처음으로 유지되기 전에 워크플로 인스턴스에서 예외가 발생한 경우.반환되는 <xref:System.Activities.UnhandledExceptionAction>에 따라 다음과 같은 시나리오가 발생합니다.  
    ...
    -   <xref:System.Activities.UnhandledExceptionAction>이 <xref:System.Activities.UnhandledExceptionAction>로 설정된 경우: 예외가 발생하면 서비스 배포 정보가 인스턴스 저장소에 기록되고 워크플로 인스턴스가 메모리에서 언로드됩니다.워크플로 인스턴스는 비지속형 상태로 남아 있으며 다시 로드될 수 없습니다.  
  
    -   <xref:System.Activities.UnhandledExceptionAction>이 <xref:System.Activities.UnhandledExceptionAction> 또는 <xref:System.Activities.UnhandledExceptionAction>로 설정된 경우: 예외가 발생하면 서비스 배포 정보가 인스턴스 저장소에 기록되고 활동 인스턴스 상태가 <xref:System.Activities.ActivityInstanceState>로 설정됩니다.  
  
 언로드된 비지속형 워크플로 인스턴스가 발생하는 위험을 최소화하려면 수명 주기의 초기에 워크플로를 유지하는 것이 좋습니다.  
  
## 비지속형 인스턴스의 검색 및 제거  
 <xref:System.Activities.DurableInstancing.SqlWorkflowInstanceStore>는 비지속형 워크플로 인스턴스를 인스턴스 저장소에서 제거하지 않으며비지속형 워크플로 인스턴스가 연결되어 있는 만료된 잠금 소유자도 제거하지 않습니다.  

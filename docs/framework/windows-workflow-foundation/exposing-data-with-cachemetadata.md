---
title: "CacheMetadata를 사용하여 데이터 노출 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## 기본 CacheMetadata 동작  
 <xref:System.Activities.NativeActivity>에서 파생되는 작업에 대한 <xref:System.Activities.NativeActivity.CacheMetadata%2A>의 기본 구현에서는 다음 메서드 형식을 다음과 같은 방식으로 처리합니다.  
...
-   <xref:System.Activities.Variable> 형식의 <xref:System.Collections.ICollection>: 컬렉션의 모든 요소가 런타임에 공용 변수로 노출됩니다.  
  
-   <xref:System.Activities.Activity> 형식의 <xref:System.Collections.ICollection>: 컬렉션의 모든 요소가 런타임에 공용 자식으로 노출됩니다.  
  
-   <xref:System.Activities.ActivityDelegate> 형식의 <xref:System.Collections.ICollection>: 컬렉션의 모든 요소가 런타임에 공용 대리자로 노출됩니다.  
  
 <xref:System.Activities.Activity>, <xref:System.Workflow.Activities.CodeActivity> 및 <xref:System.Activities.AsyncCodeActivity>에서 파생되는 작업에 대한 <xref:System.Activities.Activity.CacheMetadata%2A>도 다음과 같은 차이점을 제외하고 위와 동일하게 작동합니다.  

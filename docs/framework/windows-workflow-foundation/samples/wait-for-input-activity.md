...
|WaitForInputTestClient|워크플로 디자이너를 통해 여러 가지 WaitForInput 활동을 사용하여 워크플로를 구성하고 실행하는 샘플 클라이언트 응용 프로그램입니다.|Sequence1.xaml<br /><br /> `WaitForInput` 활동을 사용하는 순차 워크플로입니다.|  
...
## WaitForInput 활동  
 ...
 `WaitForInput` 활동은 <xref:System.Activities.NativeActivityContext> 클래스를 통해서만 액세스할 수 있는 책갈피를 만들어야 하므로 <xref:System.Activities.NativeActivity> 클래스에서 파생됩니다.  
...
|BookmarkName|InArgument\<string\>|책갈피의 이름입니다.|  
|Result|InArgument\<TResult\>|책갈피를 다시 시작할 때 활동에 전달할 데이터입니다.|  

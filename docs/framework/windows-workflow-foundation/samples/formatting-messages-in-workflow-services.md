---
title: "워크플로 서비스에서 메시지 서식 지정 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# 워크플로 서비스에서 메시지 서식 지정
이 샘플에서는 메시징 활동\(WF 서비스\)에서 여러 가지 사용자 형식을 사용하는 방법을 보여 줍니다.샘플 서비스는 간단한 비용 승인 서비스이며 세 가지 작업을 노출합니다.`ApproveExpense`는 데이터 계약 형식을 사용하며 알려진 형식을 사용하는 방법을 보여줍니다.작업은 비용 금액을 기준으로 `true` 또는 `false`를 반환합니다.`ApprovePO`는 XmlSerializer 형식을 사용하며 비용 금액을 기준으로 `true` 또는 `false`를 반환합니다. `ApprovedVendor`는 메시지 계약 형식을 사용하며, 공급업체가 승인된 공급업체 목록에 있는지 요청이 재무 부서의 요청\(재무 부서는 모든 공급업체를 사용할 수 있음\)인지에 따라 `true` 또는 `false`를 반환합니다.  

...
---
# 채용 프로세스
이 샘플에서는 워크플로 서비스로 호스팅되는 두 개의 워크플로와 메시징 활동을 사용하여 비즈니스 프로세스를 구현하는 방법을 보여 줍니다.이 워크플로는 Contoso, Inc라는 가상 회사의 IT 인프라 중 일부입니다.  
  
 <xref:System.Activities.Statements.Flowchart>로 구현된 `HiringRequest` 워크플로 프로세스는 조직에 있는 여러 관리자의 권한 부여를 요구합니다.이를 위해 워크플로에서는 조직에 있는 다른 기존 서비스를 사용하며 이 샘플에서는 기본 [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 서비스로 구현된 조직 데이터 서비스와 받은 편지함 서비스를 사용합니다.  
  
 <xref:System.Activities.Statements.Sequence>로 구현된 `ResumeRequest` 워크플로에서는 Contoso의 외부 채용 웹 사이트에 직원 모집 공고를 게시하고 이력서 접수를 관리합니다.직원 모집 공고는 마감 시한이 되거나 Contoso의 담당자가 삭제할 때까지 일정 기간 동안 외부 웹 사이트에서 볼 수 있습니다.  
...
|HiringRequestService|채용 요청 프로세스 워크플로의 정의를 포함합니다.<br /><br /> 이 프로젝트는 워크플로\(xaml 파일\)를 서비스로 자체 호스팅하는 콘솔 응용 프로그램으로 구현됩니다.|  
...
|InternalClient|프로세스와 상호 작용하기 위한 웹 응용 프로그램입니다.사용자는 HiringProcess 워크플로를 시작하고 참여하고 볼 수 있습니다.사용자는 이 응용 프로그램을 사용하여 ResumeRequest 프로세스를 시작하고 모니터링할 수도 있습니다.<br /><br /> 이 사이트는 Contoso의 인트라넷에 내부적으로 구현되고이 프로젝트는 ASP.NET 웹 사이트로 구현됩니다.|  
...
|워크플로 서비스|프로세스 정의가 포함된 순서도는 서비스에서 호스팅됩니다. 이 예에서는 서비스가 콘솔 응용 프로그램에서 호스팅됩니다.|HiringRequestService|  
|메시징 활동|순서도는 다음 두 가지 방식으로 메시징 활동을 사용합니다.<br /><br /> -   사용자로부터 정보를 받아 각 승인 단계에서 결정된 사항과 관련 정보를 얻습니다.<br />-   다른 기존 서비스\(서비스 참조를 통해 사용되는 InboxService 및 OrgDataService\)와 상호 작용합니다.|HiringRequestService|  
|내용 기반 상관 관계|승인 메시지는 채용 요청의 ID 속성과 상관 관계가 있습니다.<br /><br /> -   프로세스가 시작되면 상관 관계 핸들이 요청 ID를 사용하여 초기화됩니다.<br />-   들어오는 승인 메시지는 해당 ID와 상관 관계가 있으며 이때 각 승인 메시지의 첫 번째 매개 변수가 요청 ID입니다.|HiringRequestService\/ResumeRequestService|  
|사용자 지정 활동\(선언적 및 코드 기반\)|이 샘플에는 다음과 같은 몇 가지 사용자 지정 활동이 있습니다.<br /><br /> -   `SaveActionTracking`: 이 활동은 <xref:System.Activities.NativeActivityContext.Track%2A>을 사용하여 사용자 지정 <xref:System.Activities.Tracking.TrackingRecord>를 내보냅니다.이 활동은 <xref:System.Activities.NativeActivity>를 확장하는 명령적 코드를 사용하여 작성되었습니다.<br />-   `GetEmployeesByPositionTypes`: 이 활동은 직무 종류 ID 목록을 받고 Contoso에서 해당 직무를 가지고 있는 직원의 목록을 반환합니다.이 활동은 활동 디자이너를 사용하여 선언적으로 작성되었습니다.<br />-   `SaveHiringRequestInfo`: 이 활동은 `HiringRequestRepository.Save`를 사용하여 `HiringRequest`의 정보를 저장합니다.이 활동은 <xref:System.Activities.CodeActivity>를 확장하는 명령적 코드를 사용하여 작성되었습니다.|HiringRequestService|  
|시스템 제공 SQL Server 지속성|순서도 프로세스 정의를 호스팅하는 <xref:System.ServiceModel.Activities.WorkflowServiceHost> 인스턴스는 시스템 제공 SQL Server 지속성을 사용하도록 구성됩니다.|HiringRequestService\/ResumeRequestService|  
...
|병렬 활동|-   <xref:System.Activities.Statements.ParallelForEach%601>은 CEO 및 HR 관리자의 편지함에 병렬로 등록하여 두 HR 관리자의 승인 단계를 기다리는 데 사용됩니다.<br />-   <xref:System.Activities.Statements.Parallel>은 완료된 단계와 거부된 단계에서 몇 가지 정리 작업을 수행하는 데 사용됩니다.|HiringRequestService|  
...
|[!INCLUDE[vstecasp](../../../../includes/vstecasp-md.md)] 응용 프로그램에서 [!INCLUDE[wf1](../../../../includes/wf1-md.md)]을 사용합니다.|두 [!INCLUDE[vstecasp](../../../../includes/vstecasp-md.md)] 응용 프로그램에서 워크플로에 액세스합니다.|InternalClient\/CareersWebSite|  
...
2.  **솔루션 탐색기**에서 솔루션을 마우스 오른쪽 단추로 클릭하고 **속성**을 선택합니다.  
  
3.  **여러 개의 시작 프로젝트** 옵션을 선택하고 **CareersWebSite**, **InternalClient**, **HiringRequestService** 및 **ResumeRequestService**를 **시작**으로 설정합니다.**ContosoHR**, **InboxService** 및 **OrgService**는 없음 상태로 둡니다.  
...
2.  솔루션에서 **InternalClient**를 마우스 오른쪽 단추로 클릭한 다음 **브라우저에서 보기**를 선택합니다.`InternalClient`의 기본 페이지가 표시됩니다.서비스가 실행되고 있는지 확인한 다음 링크를 클릭합니다.  
  
3.  **HiringRequest** 모듈이 표시됩니다.여기에서 자세히 설명된 시나리오를 따르면 됩니다.  
...
6.  솔루션에서 **CareersWebSite**를 마우스 오른쪽 단추로 클릭한 다음 **브라우저에서 보기**를 선택합니다.  
  
7.  솔루션에서 **InternalClient**를 마우스 오른쪽 단추로 클릭한 다음 **브라우저에서 보기**를 선택하여 다시 `InternalClient`로 이동합니다.  
  
8.  받은 사서함의 맨 위 메뉴에서 **Job Postings** 링크를 클릭하여 **JobPostings** 섹션으로 이동합니다.여기에서 자세히 설명된 시나리오를 따르면 됩니다.  
...
2.  요청을 만들면 해당 요청은 Michael의 받은 편지함에 표시되며 Michael의 관리자인 Peter Brehm의 승인을 기다립니다. 요청이 표시되지 않는 경우에는 **새로 고침**을 클릭합니다.  
...
7.  이제 두 명의 HR 관리자나 CEO가 요청을 승인해야 합니다.CEO인 Brian Richard Goldstein은 Tsvi의 긴급 요청을 확인한 후요청을 수락하고 두 명의 HR 관리자의 승인을 건너뛰어 요청에 대한 조치를 취합니다.  
  
8.  이제 이 요청은 Michael의 받은 편지함에서 제거되고 SDET 채용 프로세스가 시작되었습니다.  
  
### 이력서 요청 시작  
  
1.  이제 지원자가 지원할 수 있는 외부 웹 사이트에 직무가 게시되기 위해 대기 중이며, **Job Postings** 링크를 클릭하면 해당 웹 사이트를 볼 수 있습니다.현재 직무를 종료하고 게시하는 HR 담당자가 직무에 대한 처리를 담당합니다.  
  
2.  HR에서는 **Edit** 링크 클릭을 통해 제한 시간을 60분으로 설정하여 직무를 편집하려고 합니다. 실제 상황에서는 60분이 아닌 일 단위나 주 단위가 될 수 있습니다.시간 제한을 통해 지정한 시간에 따라 외부 웹 사이트에서 직무를 중단할 수 있습니다.  
  
3.  편집한 직무를 저장하면 해당 직무가 **Receiving Resumes** 탭에 표시됩니다. 웹 페이지를 새로 고치면 새로운 직무가 표시됩니다.  
  
### 이력서 접수  
  
1.  외부 웹 사이트에 직무가 표시되어야 합니다.구직하려는 지원자는 이 직무에 지원하고 이력서를 제출할 수 있습니다.  
  
2.  Job Postings List 서비스로 돌아가면 지금까지 접수된 “이력서를 볼 수 있습니다”.  
...
2.  솔루션이 제대로 빌드되지 않으면 다음 내용을 확인합니다.  
    ...
    -   `ContosoHR`에 대한 참조가 `InternalClient` 또는 `CareersWebSite` 프로젝트에서 누락되지 않았는지 여부  
...
3.  솔루션이 제대로 실행되지 않으면 다음 내용을 확인합니다.  
    ...
    2.  서비스 참조가 업데이트되었는지 여부  
        ...
        1.  App\_WebReferences 폴더를 엽니다.  
  
        2.  **Contoso**를 마우스 오른쪽 단추로 클릭하고 **Update Web\/Service References**를 선택합니다.  

...
## 이 샘플을 사용하려면  
 이 샘플은 두 가지 버전으로 제공됩니다.CodedWorkflow\\CS 폴더에 있는 이 샘플의 첫 번째 버전에서는 [!INCLUDE[wf](../../../../includes/wf-md.md)] 프로그래밍 모델을 사용하는 C\# 코드를 통해 <xref:System.Activities.Statements.InvokeMethod>를 사용하는 방법을 보여 줍니다.DesignerWorkflow\\CS 폴더에 있는 두 번째 버전에서는 XAML을 통해 <xref:System.Activities.Statements.InvokeMethod>를 사용하는 방법을 보여 줍니다.  

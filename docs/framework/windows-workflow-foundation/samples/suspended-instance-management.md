---
title: "일시 중단된 인스턴스 관리 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## 세부 항목  
 [!INCLUDE[wf](../../../../includes/wf-md.md)]에서 <xref:System.ServiceModel.Activities.Description.WorkflowUnhandledExceptionBehavior> 및 <xref:System.ServiceModel.Activities.WorkflowControlEndpoint>가 있는 <xref:System.ServiceModel.WorkflowServiceHost>  
...
## 추가 설명  
 ...
 제공된 구현에서는 SQL 인스턴스 저장소에 대해 직접 SQL 명령을 실행하여 일시 중단된 인스턴스를 나열하고, 인스턴스를 다시 시작하거나 종료하기 위해 <xref:System.ServiceModel.WorkflowServiceHost>에 추가된 <xref:System.ServiceModel.Activities.WorkflowControlEndpoint>를 사용합니다.  
...
2.  SQL Server 데이터베이스를 설정합니다.  
    ...
    1.  [!INCLUDE[vs2010](../../../../includes/vs2010-md.md)] 명령 프롬프트에서 SuspendedInstanceManagement 샘플 디렉터리의 “setup.cmd”를 실행하여 다음 작업을 수행합니다.  
...
3.  서비스 큐를 설정합니다.  
    ...
    1.  [!INCLUDE[vs2010](../../../../includes/vs2010-md.md)]에서 **SampleWorkflowApp** 프로젝트를 마우스 오른쪽 단추로 클릭하고 **시작 프로젝트로 설정**을 클릭합니다.  
    ...
    5.  **서비스 및 응용 프로그램**, **메시지 큐** 및 **개인 큐**를 확장합니다.  
  
    6.  **ReceiveTx** 큐를 마우스 오른쪽 단추로 클릭한 다음 **속성**을 선택합니다.  
  
    7.  **보안** 탭을 선택하고 **Everyone**이 **메시지 받기**, **메시지 보기** 및 **메시지 보내기** 권한을 갖도록 허용합니다.  
  
4.  이제 샘플을 실행합니다.  
  
    1.  [!INCLUDE[vs2010](../../../../includes/vs2010-md.md)]에서 **Ctrl\+F5**를 눌러 디버깅 없이 SampleWorkflowApp 프로젝트를 다시 실행합니다.두 끝점 주소가 콘솔 창에 출력됩니다. 이 주소 중 하나를 응용 프로그램 끝점에 대한 주소이고, 다른 하나는 <xref:System.ServiceModel.Activities.WorkflowControlEndpoint>의 주소입니다.그런 다음 워크플로 인스턴스가 만들어지고, 해당 인스턴스에 대한 추적 레코드가 콘솔 창에 나타납니다.워크플로 인스턴스를 예외를 throw하며, 인스턴스가 일시 중단되고 중단됩니다.  
...
2.  **서비스 및 응용 프로그램**, **메시지 큐** 및 **개인 큐**를 확장합니다.  

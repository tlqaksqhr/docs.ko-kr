---
title: "명령형 TransactionScope에서 워크플로 실행 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# 명령형 TransactionScope에서 워크플로 실행
이 샘플에서는 명령형 C\# 코드에서 <xref:System.Transactions.Transaction>의 <xref:System.Activities.WorkflowInvoker>를 사용하여 워크플로를 실행하는 방법을 보여 줍니다.  
...
## 샘플 세부 정보  
 ...
 이 샘플에서는 새 <xref:System.Transactions.Transaction.TransactionScope>를 시작하고, 트랜잭션 ID를 출력하고, <xref:System.Activities.WorkflowInvoker>를 사용하여 워크플로를 시작합니다.이 워크플로는 트랜잭션 ID를 다시 출력하여 동일한 트랜잭션 내에서 진행되고 있음을 확인시켜 준 다음 <xref:System.Activities.Statements.TransactionScope>를 실행한 후 과정을 마칩니다.<xref:System.Activities.WorkflowInvoker>에 대한 <xref:System.Activities.WorkflowInvoker.Invoke%2A> 호출은 동기적이므로 워크플로가 완료될 때까지 원래 스레드가 차단됩니다.워크플로가 완료되고 나면 트랜잭션이 완료되고 리소스가 삭제됩니다.  

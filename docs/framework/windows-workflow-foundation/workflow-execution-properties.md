---
title: "워크플로 실행 속성 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## 워크플로 실행 속성 만들기 및 사용  
 워크플로 실행 속성은 보통<xref:System.Activities.IExecutionProperty> 인터페이스를 구현하고, 메시징에 중점을 둔 속성을 통해 대신 <xref:> System.ServiceModel.Activities.ISendMessageCallback?qualifyHint=False&autoUpgrade=True 및 <xref:> System.ServiceModel.Activities.IReceiveMessageCallback?qualifyHint=False&autoUpgrade=True를 구현합니다.워크플로 실행 속성을 만들려면 <xref:System.Activities.IExecutionProperty> 인터페이스를 구현하는 클래스를 만들고 멤버 <xref:System.Activities.IExecutionProperty.SetupWorkflowThread%2A> 및 <xref:System.Activities.IExecutionProperty.CleanupWorkflowThread%2A>를 구현합니다.이 멤버는 자식 활동을 비롯하여 속성이 포함된 활동의 각 작업 펄스 중 스레드 로컬 저장소를 올바르게 설정하고 중지할 기회를 실행 속성에 제공합니다.이 예제에서는 `Console.ForegroundColor`를 설정하는 `ConsoleColorProperty`를 만듭니다.  
  
> [!NOTE]
>  이 항목의 다음 예제 코드는 [실행 속성](../../../docs/framework/windows-workflow-foundation/samples/execution-properties.md) 샘플을 기반으로 합니다.  
...
```  
 ...
 활동 작성자는 활동 실행 재정의에 이 속성을 등록하여 속성을 사용할 수 있습니다.이 예제에서는 `ConsoleColorProperty`를 현재 <xref:System.Activities.NativeActivityContext>의 <xref:System.Activities.NativeActivityContext.Properties%2A> 컬렉션에 추가하여 등록하는 `ConsoleColorScope` 활동을 정의합니다.  

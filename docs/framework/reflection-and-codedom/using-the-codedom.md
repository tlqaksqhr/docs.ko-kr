...
### <a name="building-a-source-code-model-for-a-sample-hello-world-program"></a>샘플 Hello World 프로그램에 대한 소스 코드 모델 빌드  
 다음 연습에서는 간단한 Hello World 응용 프로그램의 코드를 나타내는 CodeDOM 개체 그래프를 빌드하는 방법의 예제를 제공합니다. 이 코드 예제의 전체 소스 코드를 보려면 <xref:System.CodeDom.Compiler.CodeDomProvider?displayProperty=fullName> 항목을 참조하세요.  

...
|`bypassonlocal`|로컬 리소스에 대 한 프록시를 바이패스 하는지 여부를 지정 합니다. 로컬 리소스에는 로컬 서버 포함 (`http://localhost`, `http://loopback`, 또는 `http://127.0.0.1`) 및 마침표 없는 URI (`http://webserver`). 기본값은 `unspecified`입니다.|  

...
|performanceCounters|어셈블리에 대해 성능 카운터를 사용할 수 있는지 여부를 지정합니다.  유효한 값은 다음과 같습니다.<br /><br /> -   Off: 성능 카운터의 설정을 사용하지 않습니다.<br />-   ServiceOnly: 이 서비스와 관련된 성능 카운터만 사용합니다.<br />-   All: 성능 카운터를 런타임으로 볼 수 있습니다.<br />-   기본값: 단일 성능 카운터 인스턴스 \_WCF\_Admin이 만들어집니다.  이 인스턴스는 인프라에서 사용되는 SQM 데이터 컬렉션을 설정하는 데 사용됩니다.  이 인스턴스의 어떤 카운터 값도 업데이트되지 않으므로 0으로 남게 됩니다.  이것은 WCF에 대한 구성이 없을 경우 기본값입니다.|  
...
|[\<messageLogging\>](../../../../../docs/framework/configure-apps/file-schema/wcf/messagelogging.md)|WCF 메시지 로깅의 설정을 설명합니다.|  

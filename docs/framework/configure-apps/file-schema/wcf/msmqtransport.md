...
|deadLetterQueue|사용할 배달 못 한 편지 큐의 형식을 지정합니다.<br /><br /> 유효한 값은 다음과 같습니다.<br /><br /> -   Custom: 사용자 지정 배달 못 한 편지 큐입니다.<br />-   None: 배달 못 한 편지 큐를 사용하지 않습니다.<br />-   System: 시스템 배달 못 한 편지 큐를 사용합니다.<br /><br /> 이 특성은 DeadLetterQueue 형식입니다.|  
...
|queueTransferProtocol|이 바인딩에서 사용하는 대기 중인 통신 채널 전송을 지정합니다.  유효한 값은 다음과 같습니다.<br /><br /> -   Native: 네이티브 MSMQ 프로토콜을 사용합니다.<br />-   Srmp: SRMP\(Soap Reliable Messaging Protocol\)를 사용합니다.<br />-   SrmpSecure: SRMPS\(Soap Reliable Messaging Protocol Secure\) 전송을 사용합니다.<br /><br /> 이 특성은 <xref:System.ServiceModel.QueueTransferProtocol> 형식입니다.<br /><br /> MSMQ는 SRMP\(SOAP Reliable Messaging Protocol\) 사용 시 Active Directory 주소 지정을 지원하지 않으므로 `u``seActiveDirectory`가 `true`로 설정된 경우 이 특성을 Srmp 또는 Srmps로 설정하면 안 됩니다.|  

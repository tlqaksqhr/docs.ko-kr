...
|deadLetterQueue|사용할 배달 못 한 편지 큐의 형식을 지정합니다.<br /><br /> 유효한 값은 다음과 같습니다.<br /><br /> 사용자 지정: 사용자 지정 배달 못 한 편지 큐입니다.<br />-None: 배달 못 한 편지 큐는 데 사용할 합니다.<br />-시스템: 시스템 배달 못 한 편지 큐를 사용 합니다.<br /><br /> 이 특성은 DeadLetterQueue 형식입니다.|  
...
|serializationFormat|MSMQ 메시지의 일부로 보내지는 개체를 serialize하는 데 사용되는 포맷터를 지정합니다. 유효한 값은 다음과 같습니다.<br /><br /> -ActiveX: ActiveX 포맷터는 COM 개체를 직렬화 할 때 사용 됩니다.<br />-Binary: 개체를 이진 패킷으로 Serialize합니다.<br />-ByteArray: 개체를 바이트 배열로 Serialize합니다.<br />-Stream: 개체를 스트림으로 Serialize합니다.<br />Xml: 개체를 XML 패킷으로 Serialize합니다. 기본값은 XML입니다.<br /><br /> 이 특성은 <xref:System.ServiceModel.MsmqIntegration.MsmqMessageSerializationFormat> 형식입니다.|  

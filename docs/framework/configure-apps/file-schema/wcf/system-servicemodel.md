...
|[\<라우팅 >](../../../../../docs/framework/configure-apps/file-schema/wcf/routing.md)|이 섹션에서는 Windows Communication Foundation (WCF)의 형식을 결정 하는 라우팅 필터 집합을 정의<xref:System.ServiceModel.Dispatcher.MessageFilter> 들어오는 메시지를 평가할 수 있을 뿐만 아니라 라우팅에 메시지를 보낼 때 대상 끝점을 정의 하는 테이블 때 사용 하는 필터와 일치합니다.|  

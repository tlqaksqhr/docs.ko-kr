---
title: "구성 파일을 사용하여 앱 구성 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
dev_langs: 
  ...
  - "jsharp"
helpviewer_keywords: 
  - ".NET Framework 응용 프로그램 구성, 구성 파일"
  - "<runtime> 요소"
  - "응용 프로그램 구성 파일"
  - "응용 프로그램 구성 파일, 정보"
  - "응용 프로그램 구성 파일, format"
  - "응용 프로그램 구성 파일, 컴퓨터"
  - "응용 프로그램 구성 파일, 보안"
  - "응용 프로그램[.NET Framework], 구성 파일"
  - "ASP.NET, 구성"
  - "구성 파일[.NET Framework]"
  - "구성 파일[.NET Framework], 응용 프로그램"
  - "구성 파일[.NET Framework], format"
  - "구성 파일[.NET Framework], 컴퓨터"
  - "구성 파일[.NET Framework], 보안"
  - "마침 태그"
  - "호스트, 응용 프로그램"
  - "컴퓨터 구성 파일"
  - "보안 구성 파일"
...
## 구성 파일 서식  
 구성 파일에는 구성 정보를 설정하는 논리적 데이터 구조인 요소가 포함되어 있습니다.  구성 파일에서는 태그를 사용하여 요소의 시작과 끝을 표시합니다.  예를 들어, `<runtime>` 요소는 `<runtime>`*자식 요소*`</runtime>`으로 구성됩니다.  빈 요소는 `<runtime/>` 또는 `<runtime>``</runtime>`으로 기록됩니다.  
  
 모든 XML 파일에서와 같이 구성 파일의 구문은 대\/소문자를 구분합니다.  
  
 요소의 시작 태그 안에 이름\/값 쌍으로 된 미리 정의된 특성을 사용하여 구성 설정을 지정합니다.  다음 예제에서는 런타임에 어셈블리를 찾을 위치를 지정하는 `version` 요소의 두 가지 특성인 `href`과 `<codeBase>`를 지정합니다. 자세한 내용은 [어셈블리 위치 지정](../../../docs/framework/configure-apps/specify-assembly-location.md)을 참조하세요.  
...
## 컴퓨터 구성 파일  
 컴퓨터 구성 파일인 Machine.config 에는 전체 컴퓨터에 적용되는 설정이 포함되어 있습니다.  이 파일은 %*런타임 설치 경로*%\\Config 디렉터리에 있습니다.  Machine.config는 컴퓨터 전반의 어셈블리 바인딩, 내장 [원격 채널](http://msdn.microsoft.com/ko-kr/6e9b60e0-9bc0-47b4-a8ef-3b78585f9a18) 및 ASP.NET을 포함합니다.  
  
 구성 시스템은 먼저 컴퓨터 구성 파일에서 [appSettings 요소](http://msdn.microsoft.com/ko-kr/0d65a3f1-c522-423d-89b6-44921b6daebb)를 찾아본 후 개발자가 정의한 다른 구성 섹션에서 찾아봅니다.  그런 다음 응용 프로그램 구성 파일에서 찾아봅니다.  컴퓨터 구성 파일을 지속적으로 관리하려면 이러한 설정을 응용 프로그램 구성 파일에 넣는 것이 가장 좋습니다.  그러나 컴퓨터 구성 파일에 설정을 넣으면 시스템을 유지 관리하기가 더 쉬울 수 있습니다.  예를 들어, 클라이언트 및 서버 응용 프로그램이 모두 사용하는 타사 제품의 구성 요소를 가지고 있을 경우에는 이 구성 요소의 설정을 한 곳에 두는 것이 더 용이합니다.  이 때 설정을 넣을 적합한 위치는 컴퓨터 구성 파일이며, 이럴 경우 동일한 설정을 서로 다른 두 파일에 둘 필요가 없습니다.  
...
-   실행 파일에서 호스팅되는 응용 프로그램.  
     ...
     Visual Studio에서 개발할 때는 애플리케이션용 원본 구성 파일을 프로젝트 디렉터리에 두고 **출력 디렉터리에 복사** 속성을 **항상 복사**나 **새 버전이면 복사**로 설정합니다.  구성 파일의 이름은 응용 프로그램의 이름과 같으며 .config 확장명을 갖습니다.  예를 들어, myApp.exe라는 응용 프로그램에는 myApp.exe.config라는 소스 구성 파일이 있어야 합니다.  
...
-   ASP.NET에서 호스팅되는 응용 프로그램.  
     ...
     ASP.NET 구성 파일에 대한 자세한 내용은 [ASP.NET 구성 설정](http://msdn.microsoft.com/ko-kr/116608f3-c03d-4413-9fc7-978703e18b0f)를 참조하세요.  
...
## 참고 항목  
 ...
 [어셈블리 위치 지정](../../../docs/framework/configure-apps/specify-assembly-location.md)   
 [어셈블리 버전 리디렉션](../../../docs/framework/configure-apps/redirect-assembly-versions.md)   
 [구성 파일을 사용하여 원격 개체 등록](http://msdn.microsoft.com/ko-kr/bc503ee1-c811-4f82-9525-470343326adc)   
 [ASP.NET Web Site Administration](../Topic/ASP.NET%20Web%20Site%20Administration.md)   
 [NIB: Security Policy Management](http://msdn.microsoft.com/ko-kr/d754e05d-29dc-4d3a-a2c2-95eaaf1b82b9)   
 [Caspol.exe\(코드 액세스 보안 정책 도구\)](../../../docs/framework/tools/caspol-exe-code-access-security-policy-tool.md)   
 [공용 언어 런타임의 어셈블리](../../../docs/framework/app-domains/assemblies-in-the-common-language-runtime.md)   
 [원격 개체](http://msdn.microsoft.com/ko-kr/515686e6-0a8d-42f7-8188-73abede57c58)
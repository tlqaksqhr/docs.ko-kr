---
title: "MEF(Managed Extensibility Framework) | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## <a name="what-mef-provides"></a>MEF에서 제공하는 기능  
 명시적으로 사용 가능한 구성 요소 등록을 하는 대신 MEF는 암시적으로 검색 하는 방법을 제공을 통해 *컴퍼지션*합니다.  라고 하는 MEF 구성 요소는 *부분*선언적 모두 종속성을 지정 (라고 *가져옵니다*) 및 기능 (라고 *내보냅니다*) 사용할 수 있는 합니다. 파트를 작성할 때 MEF 컴퍼지션 엔진은 다른 파트에서 제공되는 항목을 사용하여 Import를 충족합니다.  
...
> [!NOTE]
>  SimpleCalculator는 사용 방식을 보여 주는 실제 시나리오를 제공하기보다는 MEF의 개념과 구문을 제시하는 데 사용됩니다. MEF의 이점을 가장 효율적으로 활용할 수 있는 대부분의 응용 프로그램은 SimpleCalculator보다 복잡합니다. 더 광범위 한 예제를 보려면는 [Managed Extensibility Framework](http://go.microsoft.com/fwlink/?LinkId=144282) Codeplex에 있습니다.  
  
 에 시작 하려면 [!INCLUDE[vs_dev10_long](../../../includes/vs-dev10-long-md.md)], 라는 새 콘솔 응용 프로그램 프로젝트를 만듭니다 `SimpleCalculator`합니다. 그런 다음 MEF가 상주하는 System.ComponentModel.Composition 어셈블리에 대한 참조를 추가합니다. Module1.vb나 Program.cs를 열고 System.ComponentModel.Composition 및 System.ComponentModel.Composition.Hosting에 대해 `Imports` 또는 `using` 문을 추가합니다. 이 두 네임스페이스는 확장 가능한 응용 프로그램을 개발하는 데 필요한 MEF 형식을 포함합니다. Visual Basic에서 `Public` 모듈을 선언하는 줄에 `Module1` 키워드를 추가합니다.  
  
<a name="composition_container_and_catalogs"></a>   
## <a name="composition-container-and-catalogs"></a>컴퍼지션 컨테이너 및 카탈로그  
 MEF 컴퍼지션 모델의 핵심은는 *컴퍼지션 컨테이너*, 사용 가능한 모든 파트를 포함 하며 컴퍼지션을 수행 하 합니다.  컴퍼지션이란 Import를 Export에 일치시키는 작업입니다.  가장 일반적인 유형의 컴퍼지션 컨테이너는 <xref:System.ComponentModel.Composition.Hosting.CompositionContainer>, SimpleCalculator에 대해이 사용 합니다.  
...
```  
 ...
 가능한 파트를 컴퍼지션 컨테이너를 검색 하기 위해 활용 된 *카탈로그*합니다. 카탈로그는 일부 소스에서 사용 가능한 파트를 검색하는 개체입니다.  MEF는 제공된 형식, 어셈블리 또는 디렉터리에서 파트를 검색하기 위한 카탈로그를 제공합니다. 응용 프로그램 개발자는 쉽게 새 카탈로그를 만들어 웹 서비스 등의 다른 소스에서 파트를 검색할 수 있습니다.  
...
```  
 ...
 에 대 한 호출 <xref:System.ComponentModel.Composition.AttributedModelServices.ComposeParts%2A> 여기서에서 파트의 특정 집합을 작성 하는 컴퍼지션 컨테이너의 현재 인스턴스에 지시 `Program`합니다. 그러나 이 시점에서는 `Program`에 채울 Import가 없으므로 아무 작업도 수행되지 않습니다.  
...
```  
 ...
 선언에서 `calculator` 으로 데코 레이트 하지만 사용자 개체는 일반적인 방식으로 <xref:System.ComponentModel.Composition.ImportAttribute> 특성입니다.  이 특성은 Import로 지정할 항목을 선언합니다 즉, 개체를 구성할 때 컴퍼지션 엔진에서 특성이 채워집니다.  
...
## <a name="further-imports-and-importmany"></a>추가 Import 및 ImportMany  
 SimpleCalculator를 확장하려면 operations 목록을 가져와야 합니다. 일반 <xref:System.ComponentModel.Composition.ImportAttribute> 하나만 하 여 특성을 채웁니다 <xref:System.ComponentModel.Composition.ExportAttribute>합니다.  이 클래스를 둘 이상 사용할 수 있는 경우에는 컴퍼지션 엔진에서 오류가 발생합니다.  임의 개수의 내보내기를 통해 채울 수 있는 import를 만들려면 사용할 수는 <xref:System.ComponentModel.Composition.ImportManyAttribute> 특성입니다.  
  
 다음과 같은 작업 속성을 추가 하는 `MySimpleCalculator` 클래스:  
...
```  
 ...
 <xref:System.Lazy%602> export에 대 한 간접 참조를 저장할 수 있도록 MEF에서 제공 되는 형식입니다.</T, TMetadata>  여기에서 내보낸된 개체 자체 뿐 아니라도 얻게 *메타 데이터 내보내기*, 또는 내보낸된 개체를 설명 하는 정보입니다. 각 <xref:System.Lazy%602> 포함는 `IOperation` 는 실제 연산을 나타내는 개체와 `IOperationData` 해당 메타 데이터를 나타내는 개체입니다.\</T, TMetadata>  
...
```  
 ...
 <xref:System.ComponentModel.Composition.ExportAttribute> 이전과 같이 함수 특성입니다.  <xref:System.ComponentModel.Composition.ExportMetadataAttribute> 특성은 해당 export에 이름-값 쌍의 형태로 메타 데이터를 연결 합니다.  `Add` 클래스가 `IOperation`을 구현하는 동안에는 `IOperationData`를 구현하는 클래스가 명시적으로 정의되지 않습니다. 대신 제공된 메타데이터 이름을 기준으로 하는 속성을 사용하여 MEF에서 클래스를 암시적으로 만듭니다.  이는 MEF에서 메타데이터에 액세스하는 여러 방법 중 하나입니다.  
...
```  
 ...
 초기 단계에서는 입력 문자열을 왼쪽 및 오른쪽 피연산자와 연산자 문자로 구문 분석합니다.  `foreach` 루프에서는 `operations` 컬렉션의 모든 멤버를 검사합니다. 이러한 개체 유형의 일부인 <xref:System.Lazy%602>, 해당 메타 데이터 값과 내보낸된 개체를 액세스할 수는 <xref:System.Lazy%602.Metadata%2A> 속성 및 <xref:System.Lazy%601.Value%2A>속성 각각.</T, TMetadata> 여기서 `Symbol` 개체의 `IOperationData` 속성이 일치하는 것으로 확인되면 계산기는 `Operate` 개체의 `IOperation` 메서드를 호출하고 결과를 반환합니다.  
...
## <a name="extending-simplecalculator-using-a-new-assembly"></a>새 어셈블리를 사용하여 SimpleCalculator 확장  
 소스 코드에 클래스를 추가하는 것은 간단하지만 MEF는 응용 프로그램 자체의 소스 외부에서 파트를 확인할 수 있는 기능을 제공합니다. 이 보여 주기 위해 추가 하 여 파트를 자체 어셈블리 뿐만 아니라 디렉터리를 검색 하도록 SimpleCalculator를 수정 해야 합니다는 <xref:System.ComponentModel.Composition.Hosting.DirectoryCatalog>합니다.  
  
 라는 새 디렉터리를 추가 `Extensions` 를 SimpleCalculator 프로젝트입니다.  솔루션 수준이 아닌 프로젝트 수준에 디렉터리를 추가해야 합니다. 라는 솔루션에 새 클래스 라이브러리 프로젝트를 추가 `ExtendedOperations`합니다. 새 프로젝트가 별도의 어셈블리로 컴파일됩니다.  
...
```  
 ...
 예제 경로를 Extensions 디렉터리의 경로로 바꿉니다.  이 경로는 디버깅용으로만 사용할 수 있습니다.  프로덕션 응용 프로그램에서는 상대 경로를 사용합니다. <xref:System.ComponentModel.Composition.Hosting.DirectoryCatalog> 이제 컴퍼지션 컨테이너를 Extensions 디렉터리의 모든 어셈블리에서 발견 되는 파트를 추가 합니다.  
...
```  
 ...
 계약이 일치 하도록 하려면 해당 참고는 <xref:System.ComponentModel.Composition.ExportAttribute> 특성으로 같은 형식 이어야 합니다.는 <xref:System.ComponentModel.Composition.ImportAttribute>합니다.  
...
## <a name="where-do-i-go-now"></a>다음 단계  
 ...
 자세한 내용과 코드 예제에 대 한 참조 [Managed Extensibility Framework](http://go.microsoft.com/fwlink/?LinkId=144282)합니다. MEF 형식 목록은 대 한 참조는 <xref:System.ComponentModel.Composition?displayProperty=fullName> 네임 스페이스입니다.
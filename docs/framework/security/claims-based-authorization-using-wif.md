---
title: "WIF를 사용하여 클레임 기반 권한 부여  | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.reviewer: ""
ms.suite: ""
...
## 개요  
 첫 번째 버전 이후로 .NET Framework에서는 권한 부여를 구현할 수 있는 유연한 메커니즘을 제공해왔습니다.  이 메커니즘은 **IPrincipal** 및 **IIdentity**와 같은 두 가지 간단한 인터페이스를 기반으로 합니다.  **IIdentity**의 구체적인 구현은 인증된 사용자를 나타냅니다.  예를 들어, **WindowsIdentity** 구현은 Active Directory에 의해 인증된 사용자를 나타내고, **GenericIdentity**는 사용자 지정 인증 프로세스를 통해 ID가 확인된 사용자를 나타냅니다.  **IPrincipal**의 구체적인 구현은 역할 저장소에 따라 역할을 사용하여 사용 권한을 확인하는 데 유용합니다.  예를 들어, **WindowsPrincipal**은 Active Directory 그룹의 멤버에 대한 **WindowsIdentity**를 확인합니다.  **IPrincipal** 인터페이스에서 **IsInRole** 메서드를 호출하면 이러한 확인이 수행됩니다.  역할을 기반으로 액세스 권한을 확인하는 것을 RBAC\(역할 기반 액세스 제어\)라고 합니다.  자세한 내용은 [역할 기반 액세스 제어](../../../docs/framework/security/claims-based-authorization-using-wif.md#BKMK_1)를 참조하십시오.  클레임을 사용하여 친숙한 역할 기반 권한 부여 메커니즘을 지원하기 위해 역할에 대한 정보를 전달할 수 있습니다.  
  
 또한 클레임을 사용하여 역할보다 훨씬 복잡한 권한 부여 결정을 수행할 수도 있습니다.  클레임은 사실상 연령, 우편 번호, 신발 치수 등과 같은 사용자에 대한 정보를 기반으로 할 수 있습니다. 임의의 클레임을 기반으로 하는 액세스 제어 메커니즘을 클레임 기반 권한 부여라고 합니다.  자세한 내용은 [클레임 기반 권한 부여](../../../docs/framework/security/claims-based-authorization-using-wif.md#BKMK_2)를 참조하십시오.  
...
-   보안 요청 **PrincipalPermission.Demand\(\)** 사용  이 방법에서는 요청이 충족되지 않는 경우 결과가 예외입니다.  이는 예외 처리 전략에 맞아야 합니다.  부울 회수와 비교했을 때 예외가 발생하면 성능적인 측면에서 비용이 훨씬 많이 듭니다.  이는 코드에서 임의의 위치에 사용할 수 있습니다.  
...
-   **web.config**의 **\<권한 부여\>** 섹션을 사용하여 URL 권한 부여 사용  이 방법은 URL 수준에서 권한 부여를 관리하는 경우 적합합니다.  이는 앞서 언급한 것 중에서 가장 정교하지 않은 수준입니다.  이 방법의 장점은 구성 파일에서 내용이 변경되므로 변경 내용을 활용하기 위해 코드를 컴파일하지 않는다는 점입니다.  
...
-   **토큰 발급 중**.  사용자가 인증되면 Microsoft Azure ACS\(액세스 제어 서비스\)와 같은 페더레이션 공급자 또는 ID 공급자 STS에 의해 역할 클레임이 발급될 수 있습니다.  
  
-   **ClaimsAuthenticationManager를 사용하여 임의의 클레임을 클레임 역할 형식으로 변환** ClaimsAuthenticationManager는 WIF의 일부로 제공되는 구성 요소입니다.  이는 토큰을 검사하고 클레임을 추가, 변경 또는 제거하여 해당 토큰을 변환하면서 응용 프로그램을 시작할 때 요청을 가로챌 수 있도록 허용합니다.  클레임을 변환하기 위해 ClaimsAuthenticationManager를 사용하는 방법에 대한 자세한 내용은 [방법: WIF 및 ACS를 사용하여 클레임 인식 ASP.NET 응용 프로그램에서 역할 기반 액세스 제어\(RBAC\) 구현](http://go.microsoft.com/fwlink/?LinkID=247445)\(http:\/\/go.microsoft.com\/fwlink\/?LinkID\=247444\)을 참조하십시오.  
  
-   **samlSecurityTokenRequirement 구성 섹션을 사용하여 임의의 클레임을 역할 형식으로 매핑** \- 구성만 사용하여 클레임 변환을 완료하며 코딩이 필요하지 않은 선언적 방법입니다.  

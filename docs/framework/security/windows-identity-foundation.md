---
title: "Windows Identity Foundation | Microsoft Docs"
ms.custom: ""
ms.date: "12/14/2016"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "dotnet-clr"
ms.tgt_pltfrm: ""
ms.topic: "article"
ms.assetid: 7eb50bcb-38d8-4afb-86aa-2573586697c2
caps.latest.revision: 15
author: "BrucePerlerMS"
ms.author: "bruceper"
manager: "mbaldwin"
---
# Windows Identity Foundation
-   [Windows Identity Foundation 4.5의 새로운 기능](../../../docs/framework/security/whats-new-in-wif.md)  
  
-   [Windows Identity Foundation 4.5 개요](../../../docs/framework/security/wif-overview.md)  
  
    -   [클레임 기반 ID 모델](../../../docs/framework/security/claims-based-identity-model.md)  
  
    -   [WIF를 사용하여 클레임 기반 권한 부여 ](../../../docs/framework/security/claims-based-authorization-using-wif.md)  
  
    -   [WIF 클레임 프로그래밍 모델](../../../docs/framework/security/wif-claims-programming-model.md)  
  
-   [WIF 시작](../../../docs/framework/security/getting-started-with-wif.md)  
  
    -   [내 첫 번째 클레임 인식 ASP.NET 웹 응용 프로그램 구축](../../../docs/framework/security/building-my-first-claims-aware-aspnet-web-app.md)  
  
    -   [내 첫 번째 클레임 인식 WCF 서비스 빌드](../../../docs/framework/security/building-my-first-claims-aware-wcf-service.md)  
  
-   [WIF 기능](../../../docs/framework/security/wif-features.md)  
  
    -   [Visual Studio 2012용 ID 및 액세스 도구](../../../docs/framework/security/identity-and-access-tool-for-vs.md)  
  
    -   [WIF 세션 관리](../../../docs/framework/security/wif-session-management.md)  
  
    -   [WIF 및 웹 팜](../../../docs/framework/security/wif-and-web-farms.md)  
  
    -   [WSFederation 인증 모듈 개요](../../../docs/framework/security/wsfederation-authentication-module-overview.md)  
  
    -   [WSTrustChannelFactory 및 WSTrustChannel](../../../docs/framework/security/wstrustchannelfactory-and-wstrustchannel.md)  
  
-   [WIF 방법 인덱스](../../../docs/framework/security/wif-how-tos-index.md)  
  
    -   [방법: WIF를 사용하여 클레임 인식 ASP.NET MVC 웹 응용 프로그램 빌드](../../../docs/framework/security/how-to-build-claims-aware-aspnet-mvc-web-app-using-wif.md)  
  
    -   [방법: WIF를 사용하여 클레임 인식 ASP.NET Web Forms 응용 프로그램 빌드](../../../docs/framework/security/how-to-build-claims-aware-aspnet-web-forms-app-using-wif.md)  
  
    -   [방법: 양식 기반 인증을 사용하여 클레임 인식 ASP.NET 응용 프로그램 빌드](../../../docs/framework/security/claims-aware-aspnet-app-forms-authentication.md)  
  
    -   [방법: Windows 인증을 사용하여 클레임 인식 ASP.NET 응용 프로그램 빌드](../../../docs/framework/security/how-to-build-claims-aware-aspnet-app-using-windows-authentication.md)  
  
    -   [방법: WIF 추적을 사용하여 클레임 인식 응용 프로그램 및 서비스 디버깅](../../../docs/framework/security/how-to-debug-claims-aware-applications-and-services-using-wif-tracing.md)  
  
    -   [방법: WIF를 사용하여 로그인 상태 표시](../../../docs/framework/security/how-to-display-signed-in-status-using-wif.md)  
  
    -   [방법: 토큰 재생 검색 사용](../../../docs/framework/security/how-to-enable-token-replay-detection.md)  
  
    -   [방법: WIF 추적 사용](../../../docs/framework/security/how-to-enable-wif-tracing.md)  
  
    -   [방법: WCF 웹 서비스 응용 프로그램에 WIF 사용](../../../docs/framework/security/how-to-enable-wif-for-a-wcf-web-service-application.md)  
  
    -   [방법: 들어오는 클레임 변환](../../../docs/framework/security/how-to-transform-incoming-claims.md)  
  
-   [WIF 지침](../../../docs/framework/security/wif-guidelines.md)  
  
    -   [WIF 3.5를 사용하여 빌드된 응용 프로그램을 WIF 4.5로 마이그레이션하는 지침](../../../docs/framework/security/guidelines-for-migrating-an-application-built-using-wif-3-5-to-wif-4-5.md)  
  
    -   [WIF 3.5와 WIF 4.5 간의 네임스페이스 매핑](../../../docs/framework/security/namespace-mapping-between-wif-3-5-and-wif-4-5.md)  
  
-   [WIF 코드 샘플 인덱스](../../../docs/framework/security/wif-code-sample-index.md)  
  
-   [WIF 확장](../../../docs/framework/security/wif-extensions.md)  
  
-   [WIF API 참조](../../../docs/framework/security/wif-api-reference.md)  
  
-   [WIF 구성 참조](../../../docs/framework/security/wif-configuration-reference.md)  
  
    -   [WIF 구성 스키마 규칙](../../../docs/framework/security/wif-configuration-schema-conventions.md)
---
title: "방법: Windows Forms DataGridView 컨트롤에서 열 고정 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
helpviewer_keywords: 
  - "열[Windows Forms], 고정"
  - "DataGridView 컨트롤[Windows Forms], 뷰에 항상 표시되는 열"
...
>  열 다시 정렬을 사용하는 경우 고정된 열은 고정되지 않은 열과 별개인 그룹으로 처리됩니다.  사용자는 각 그룹에서 열의 위치를 변경할 수 있지만 그룹 간에 열을 이동할 수는 없습니다.  
 ...
 Visual Studio에서는 이 작업이 지원됩니다.  [방법: 디자이너를 사용하여 Windows Forms DataGridView 컨트롤에서 열 고정](http://msdn.microsoft.com/library/717ss6s6\(v=vs.110\))을 참조하세요.  
  
### 프로그래밍 방식으로 열을 고정하려면  
  
-   <xref:System.Windows.Forms.DataGridViewColumn.Frozen%2A?displayProperty=fullName> 속성을 `true`으로 설정합니다.  

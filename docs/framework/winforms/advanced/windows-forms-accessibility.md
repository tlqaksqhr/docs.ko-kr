---
title: "Windows Forms 내게 필요한 옵션 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net-framework"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "dotnet-winforms"
ms.tgt_pltfrm: ""
ms.topic: "article"
dev_langs: 
  - "jsharp"
helpviewer_keywords: 
  - "Windows Forms 컨트롤, 접근성"
  - "접근성, Windows Forms 컨트롤"
ms.assetid: 23a3f168-ebc0-4f23-9efc-799109aa4a2d
caps.latest.revision: 5
author: "dotnet-bot"
ms.author: "dotnetcontent"
manager: "wpickett"
caps.handback.revision: 5
---
# Windows Forms 내게 필요한 옵션
Windows Forms의 접근성 기능을 사용하면 다양한 사용자에게 응용 프로그램을 제공할 수 있습니다.  
  
## 단원 내용  
 [연습: 내게 필요한 옵션이 지원되는 Windows 기반 응용 프로그램 만들기](../../../../docs/framework/winforms/advanced/walkthrough-creating-an-accessible-windows-based-application.md)  
 접근성을 늘리기 위해 지원해야 하는 모든 기능에 대해 설명합니다.  
  
## 참조  
 <xref:Accessibility>  
 접근성과 관련된 여러 클래스를 포함하는 네임스페이스입니다.  
  
 <xref:System.Windows.Forms.AccessibleObject>  
 접근성 응용 프로그램이 장애가 있는 사용자를 위해 응용 프로그램 UI\(사용자 인터페이스\)를 조정하는 데 사용하는 정보를 제공합니다.  
  
## 관련 단원  
 [Windows Form의 컨트롤에 내게 필요한 옵션 정보 제공](../../../../docs/framework/winforms/controls/providing-accessibility-information-for-controls-on-a-windows-form.md)  
 Windows Forms 컨트롤이 장애가 있는 사용자를 지원하기 위해 사용할 수 있는 정보를 제공하는 방법을 설명합니다.  
  
 [Windows Forms의 자동 배율 조정](../../../../docs/framework/winforms/automatic-scaling-in-windows-forms.md)  
 Windows Forms 응용 프로그램이 시스템 글꼴 크기 변화에 맞게 조정되도록 하는 방법을 설명합니다.
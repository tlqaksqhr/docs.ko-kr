...
| **호스팅**(관리되지 않는 API) | 호스팅 기능의 향상을 위해 일부 호스팅 활성화 API가 사용되지 않습니다. In-Process Side-by-Side 실행 기능을 통해 응용 프로그램은 동일한 프로세스에서 여러 버전의 .NET Framework를 로드하고 시작할 수 있습니다. 예를 들어 .NET Framework 2.0 SP1을 기반으로 하는 추가 기능(또는 구성 요소) 및 동일한 프로세스에서 .NET Framework 4를 기반으로 하는 추가 기능을 로드하는 응용 프로그램을 실행할 수 있습니다. 이전 구성 요소는 이전 .NET Framework 버전을 계속 사용하고 새 구성 요소는 새 .NET Framework 버전을 사용합니다. | [In-Process Side-by-Side 실행](https://msdn.microsoft.com/library/ee518876(v=vs.100).aspx)에 설명된 구성을 사용하세요. |
| **새 보안 모델** | [.NET Framework 4의 보안 변경 내용](https://msdn.microsoft.com/library/dd233103(v=vs.100).aspx)에 설명된 대로 CAS(코드 액세스 보안) 정책이 해제되어 단순화된 모델로 대체되었습니다. | 응용 프로그램에서 CAS를 사용하는 경우 수정이 필요할 수 있습니다. 자세한 내용은 [코드 액세스 보안 정책 호환성 및 마이그레이션](https://msdn.microsoft.com/library/ee191568(v=vs.100).aspx)을 참조하세요. |
...
| 기능 | 3.5 SP1과의 차이점 | 권장 변경 내용 |
| ------- | ------------------------ | ------------------- |
| **버퍼 길이**(관리되지 않는 API) | 메모리를 절약하기 위해 [ICorProfilerInfo2::GetStringLayout](https://msdn.microsoft.com/library/ms230869(v=vs.100).aspx) 메서드에 대한 `pBufferLengthOffset` 매개 변수의 기능이 `pStringLengthOffset` 매개 변수와 일치하도록 변경되었습니다. 이제 두 매개 변수가 문자열 길이의 오프셋 위치를 가리킵니다. 문자열 클래스의 표시에서 버퍼 길이가 제거되었습니다. | 버퍼 길이에 대한 종속성을 제거하세요. |
...
| 기능 | 3.5 SP1과의 차이점 |
| ------- | ------------------------ |
| **이벤트** | 컬렉션이 로드될 때 이벤트를 일으키는 것 외에도, [EntitySet\<TEntity>](https://msdn.microsoft.com/library/bb341748(v=vs.100).aspx)가 언로드될 때 [System.Data.Linq.EntitySet\<TEntity>](https://msdn.microsoft.com/library/bb341748(v=vs.100).aspx) 컬렉션은 이제 추가 및 제거 작업에 대해 [ListChanged](https://msdn.microsoft.com/library/bb548946(v=vs.100).aspx) 이벤트를 일으킵니다. |
...
| **요청** | [DataService\<T>.OnStartProcessingRequest](https://msdn.microsoft.com/library/cc646889(v=vs.100).aspx) 메서드는 이제 데이터 서비스에 대한 요청이 처리되기 전에 호출됩니다. 따라서 <xref:System.Data.Services.Providers.ServiceOperation> 서비스에 대한 요청이 올바르게 작동할 수 있습니다. |
...
| **스크립트 함수** | 함수가 실제로 사용 가능할 때 [function-available 함수](https://msdn.microsoft.com/library/ms256124(v=vs.100).aspx)는 더 이상 올바르지 않게 `false`를 반환하지 않습니다. |
...
[.NET Framework 4로 Office 솔루션 마이그레이션](https://msdn.microsoft.com/library/ee207231(v=vs.100).aspx)

### <a name="other-resources"></a>기타 리소스

[.NET Framework에서 사용되지 않는 기능](https://msdn.microsoft.com/library/ee461502(v=vs.100).aspx)   

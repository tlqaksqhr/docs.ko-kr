...
[!INCLUDE[net_v451](../../../includes/net-v451-md.md)]에서는 <xref:System.Diagnostics.Tracing.EventSource?displayProperty=fullName>에서 파생된 클래스의 ETW 이벤트 메서드와 해당 기본 클래스의 <xref:System.Diagnostics.Tracing.EventSource.WriteEvent%2A> 메서드 간에 계약을 적용합니다. ETW 이벤트 메서드는 <xref:System.Diagnostics.Tracing.EventSource.WriteEvent%2A> 메서드에 이벤트 ID 및 이벤트 메서드에 전달된 동일한 인수를 순서대로 전달해야 합니다.  
...
```  
 ...
 이 계약이 위반되면 <xref:System.Diagnostics.Tracing.EventListener> 개체가 프로세스의 <xref:System.Diagnostics.Tracing.EventSource> 데이터를 읽는 경우 런타임에 <xref:System.IndexOutOfRangeException> 예외가 throw됩니다.  

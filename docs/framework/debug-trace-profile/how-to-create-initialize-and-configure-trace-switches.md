...
## 추적 스위치 구성  
 ...
 구성할 스위치의 이름을 지정하는 것은 물론 스위치의 값도 지정해야 합니다. 이 값은 정수입니다.<xref:System.Diagnostics.BooleanSwitch>에서 값 0은 **Off**에 해당하고 0이 아닌 값은 **On**에 해당합니다.<xref:System.Diagnostics.TraceSwitch>에서 0, 1, 2, 3, 4는 각각 **Off**, **Error**, **Warning**, **Info** 및 **Verbose**에 해당합니다. 4보다 큰 숫자는 **Verbose**로 처리되고 0보다 작은 숫자는 **Off**로 처리됩니다.  
...
1.  추적 스위치를 사용하려면 [추적 스위치 생성 및 초기화](#create) 섹션에서 설명되어 있는 대로 먼저 그것을 생성하고 코드 안에 위치시켜야합니다.  

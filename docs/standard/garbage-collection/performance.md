...
이 항목에서는 가비지 수집 및 메모리 사용과 관련된 문제를 설명합니다.  관리되는 힙과 관련된 문제를 해결하고 응용 프로그램에 미치는 가비지 수집의 영향을 최소화하는 방법을 설명합니다.  각 문제에는 문제를 조사하는 데 사용할 수 있는 절차 링크가 있습니다.  
...
## 성능 분석 도구  
 다음 섹션에서는 메모리 사용 및 가비지 수집 문제를 조사하는 데 사용할 수 있는 도구를 설명합니다.  이 항목의 뒷부분에 제공된 [절차](#performance_check_procedures)에서는 이러한 도구를 참조합니다.  
...
### 프로파일링 API  
 CLR\(공용 언어 런타임\) 프로파일링 인터페이스는 가비지 수집 중에 영향을 받은 개체에 대한 자세한 정보를 제공합니다.  가비지 수집이 시작되고 끝날 때 프로파일러에 알릴 수 있습니다.  각 세대의 개체 ID를 포함하여 관리되는 힙의 개체에 대한 보고서를 제공할 수 있습니다.  자세한 내용은 [프로파일링 개요](../../../ocs/framework/unmanaged-api/profiling/profiling-overview.md)를 참조하세요.  
  
 프로파일러는 포괄적인 정보를 제공할 수 있습니다.  그러나 복잡한 프로파일러는 응용 프로그램의 동작을 수정할 가능성이 있습니다.  
  
### 응용 프로그램 도메인 리소스 모니터링  
 [!INCLUDE[net_v40_short](../../../includes/net-v40-short-md.md)]부터 ARM\(응용 프로그램 도메인 리소스 모니터링\)을 통해 호스트가 응용 프로그램 도메인별로 CPU 및 메모리 사용량을 모니터링할 수 있습니다.  자세한 내용은 [Application Domain Resource Monitoring](../../../docs/standard/garbage-collection/app-domain-resource-monitoring.md)을 참조하세요.  
  
 [맨 위로 이동](#top)  
  
<a name="troubleshooting_performance_issues"></a>   
## 성능 문제 해결  
 첫 번째 단계에서는 [문제가 실제로 가비지 수집인지 여부를 확인](#IsGC)합니다.  확인되었으면 다음 목록에서 선택하여 문제를 해결합니다.  
...
-   [가비지 수집 중 CPU 사용량이 너무 많습니다.](#Issue_HighCPU)  
...
### 문제: 가비지 수집기가 개체를 충분히 빠르게 회수하지 않습니다.  
...
-   대형 임시 개체를 만들어 대형 개체 힙이 힙 세그먼트를 자주 할당 및 해제  
     ...
     CLR을 호스트하는 경우 응용 프로그램에서 가비지 수집기가 해당 세그먼트를 유지하도록 요청할 수 있습니다.  이 경우 세그먼트 할당 빈도가 감소합니다.  이 작업을 수행하려면 [STARTUP\_FLAGS 열거형](../../../ocs/framework/unmanaged-api/hosting/startup-flags-enumeration.md)의 STARTUP\_HOARD\_GC\_VM 플래그를 사용합니다.  
...
### 문제: 가비지 컬렉션 일시 중지가 너무 깁니다.  
 ...
 단기 가비지 수집\(0세대 및 1세대\)은 몇 밀리초 동안만 지속되므로 일시 중지를 줄이는 것이 일반적으로 불가능합니다.  그러나 응용 프로그램에 의한 할당 요청 패턴을 변경하면 2세대 수집의 일시 중지를 줄일 수 있습니다.  
  
 보다 정확한 다른 방법은 [가비지 수집 ETW 이벤트](../../../docs/framework/performance/garbage-collection-etw-events.md)를 사용하는 것입니다.  이벤트 시퀀스에 대한 타임스탬프 차이를 추가하여 수집 타이밍을 찾을 수 있습니다.  전체 수집 시퀀스에는 실행 엔진 일시 중단, 가비지 수집 자체 및 실행 엔진 다시 시작이 포함됩니다.  
  
 [Garbage Collection Notifications](../../../docs/standard/garbage-collection/notifications.md)을 사용하여 서버가 2세대 수집을 수행할지 여부 및 요청을 다른 서버로 다시 라우팅할 경우 일시 중지 문제를 줄일 수 있는지 여부를 확인할 수 있습니다.  
  
|성능 검사|  
|-----------|  
|[가비지 수집 기간을 확인합니다.](#TimeInGC)<br /><br /> [가비지 수집이 발생한 이유를 확인합니다.](#Triggered)|  
...
### 문제: 가비지 수집 중 CPU 사용량이 너무 많습니다.  
 ...
 수집 기간은 주로 할당 후 남아 있는 개체 수의 비율입니다.  수집할 많은 개체가 남아 있으면 가비지 수집기가 많은 양의 메모리를 처리해야 합니다.  남은 개체를 압축하는 작업은 오랜 시간이 소요됩니다.  수집 중 처리된 개체 수를 확인하려면 디버거에서 지정된 세대에 대한 가비지 수집의 끝에 중단점을 설정합니다.  
  
|성능 검사|  
|-----------|  
|[높은 CPU 사용량이 가비지 수집에 의해 발생한 것인지 여부를 확인합니다.](#HighCPU)<br /><br /> [가비지 수집의 끝에 중단점을 설정합니다.](#GenBreak)|  
...
-   SOS 디버거 확장이 로드된 WinDbg에서 다음 명령을 입력합니다.  
     ...
     **bp mscorwks\!WKS::GCHeap::RestartEE "j \(dwo\(mscorwks\!WKS::GCHeap::GcCondemnedGeneration\)\=\=2\) 'kb';'g'"**  
     ...
     이 명령은 가비지 수집을 위해 2세대 개체가 회수된 후 **RestartEE**가 실행되는 경우 강제로 중단합니다.  
  
     서버 가비지 수집에서는 하나의 스레드만 **RestartEE**를 호출하므로 2세대 가비지 수집 중 한 번만 중단점이 발생합니다.  
...
-   [문제가 가비지 수집에 의해 발생한 것인지 여부를 확인합니다.](#IsGC)  
...
-   [가비지 수집 기간을 확인합니다.](#TimeInGC)  
  
-   [가비지 수집 트리거를 확인합니다.](#Triggered)  
  
-   [높은 CPU 사용량이 가비지 수집에 의해 발생한 것인지 여부를 확인합니다.](#HighCPU)  
  
<a name="IsGC"></a>   
##### 문제가 가비지 수집에 의해 발생한 것인지 여부를 확인하려면  
  
-   다음 두 개의 메모리 성능 카운터를 검사합니다.  
  
    -   **% Time in GC**.  마지막 가비지 수집 주기 이후 가비지 수집을 수행하는 데 소요된 경과 시간의 백분율을 표시합니다.  이 카운터를 사용하여 가비지 수집기가 관리되는 힙 공간을 사용할 수 있도록 하는 데 너무 많은 시간을 소비하고 있는지 여부를 확인할 수 있습니다.  가비지 수집에 소요된 시간이 비교적 적은 경우 관리되는 힙 외부의 리소스 문제를 나타낼 수 있습니다.  이 카운터는 동시 또는 백그라운드 가비지 수집이 관련된 경우 정확하지 않을 수 있습니다.  
  
    -   **\# Total committed Bytes**.  가비지 수집기에서 현재 커밋한 가상 메모리 크기를 표시합니다.  이 카운터를 사용하여 가비지 수집기에서 사용되는 메모리가 응용 프로그램에서 사용되는 메모리에 비해 과도하게 많은지 여부를 확인할 수 있습니다.  
...
-   SOS 디버거 확장이 로드된 WinDbg 또는 Visual Studio 디버거에서 다음 명령을 입력합니다.  
    ...
    ```  
     ...
     ID 문자열 대신 정수를 사용하면 더 효율적일 수 있습니다.  같은 문자열이 수천 번 반복되는 경우 문자열 인터닝을 고려합니다.  문자열 인터닝에 대한 자세한 내용은 <xref:System.String.Intern%2A?displayProperty=fullName> 메서드에 대한 참조 항목을 참조하세요.  
...
-   특정 개체에 대한 참조를 확인하려면 다음과 같이 주소를 포함합니다.  
     ...
     **gcroot** 명령을 완료하는 데 오랜 시간이 걸릴 수 있습니다.  가비지 수집에 의해 회수되지 않은 개체는 모두 라이브 개체입니다.  이는 일부 루트가 직접 또는 간접적으로 개체에 연결되어 있으므로 **gcroot**에서 개체의 경로 정보가 반환되어야 함을 의미합니다.  반환된 그래프를 검사하고 이러한 개체가 여전히 참조되는 이유를 확인해야 합니다.  
...
-   다음 코드가 포함된 테스트 프로그램을 실행합니다.  
    ...
    ```  
     ...
     테스트에서 문제가 해결되는 경우 이는 해당 개체에 대한 종료자가 일시 중단되어 가비지 수집기가 개체를 회수하지 못했음을 의미합니다.  <xref:System.GC.WaitForPendingFinalizers%2A?displayProperty=fullName> 메서드는 종료자가 해당 작업을 완료할 수 있도록 하며 문제를 해결합니다.  
  
<a name="Finalize"></a>   
##### 종료 대기 중인 개체가 있는지 여부를 확인하려면  
...
2.  스레드의 출력을 가져오려면 다음 명령을 입력합니다.  
    ...
    ```  
     ...
     종료자 스레드는 현재 실행 중인 종료자\(있는 경우\)를 나타냅니다.  종료자 스레드가 종료자를 실행하지 않는 경우 해당 작업을 수행하도록 지시할 이벤트를 기다리고 있습니다.  종료자 스레드는 THREAD\_HIGHEST\_PRIORITY에서 실행되고 종료자\(있는 경우\) 실행을 매우 빠르게 완료하기 때문에 대부분의 경우 이 상태의 종료자 스레드가 표시됩니다.  
  
<a name="Fragmented"></a>   
##### 관리되는 힙의 여유 공간 크기를 확인하려면  
...
-   또는 [가비지 수집 ETW 이벤트](../../../docs/framework/performance/garbage-collection-etw-events.md)를 사용하여 가비지 수집 길이를 결정하고 정보를 분석하여 가비지 수집 기간을 확인할 수 있습니다.  
  
     예를 들어 다음 데이터는 비 동시 가비지 수집 중에 발생한 이벤트 시퀀스를 보여 줍니다.  
...
<a name="HighCPU"></a>   
##### 높은 CPU 사용량이 가비지 수집에 의해 발생한 것인지 여부를 확인하려면  

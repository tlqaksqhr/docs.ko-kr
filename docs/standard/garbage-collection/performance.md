...
-   <span data-ttu-id="e0022-125">수집되는 개체 세대</span><span class="sxs-lookup"><span data-stu-id="e0022-125">Which generation of objects is being collected.</span></span>  
...
### <a name="application-domain-resource-monitoring"></a><span data-ttu-id="e0022-139">응용 프로그램 도메인 리소스 모니터링</span><span class="sxs-lookup"><span data-stu-id="e0022-139">Application Domain Resource Monitoring</span></span>  
 ...
 [<span data-ttu-id="e0022-142">맨 위로 이동</span><span class="sxs-lookup"><span data-stu-id="e0022-142">Back to top</span></span>](#top)  
...
-   <span data-ttu-id="e0022-158">할당할 실제 메모리 부족</span><span class="sxs-lookup"><span data-stu-id="e0022-158">Not having enough physical memory to allocate.</span></span>  
...
-   <span data-ttu-id="e0022-198">다수의 작은 어셈블리를 자주 로드 및 언로드</span><span class="sxs-lookup"><span data-stu-id="e0022-198">Frequent loading and unloading of many small assemblies.</span></span>  
...
-   <span data-ttu-id="e0022-250">2세대 가비지 수집 후에 측정하는 경우 전체 관리되는 힙의 가비지(비활성 개체)가 제거됩니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-250">If you measure after a generation 2 garbage collection, the entire managed heap will be free of garbage (dead objects).</span></span>  
...
-   [<span data-ttu-id="e0022-279">고정된 개체 수를 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-279">Determine the number of pinned objects.</span></span>](#Pinned)  
...
1.  <span data-ttu-id="e0022-296">SOS 디버거 확장이 로드된 WinDbg 또는 Visual Studio 디버거에서 예외 인쇄(**pe**) 명령을 입력합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-296">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the print exception (**pe**) command:</span></span>  
...
-   <span data-ttu-id="e0022-308">SOS 디버거 확장이 로드된 WinDbg에서 다음 명령을 입력하여 최대 여유 영역을 가져옵니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-308">In WinDbg with the SOS debugger extension loaded, type the following command to get the largest free region:</span></span>  
...
2.  <span data-ttu-id="e0022-319">**성능** 탭에서 커밋된 값을 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-319">On the **Performance** tab, look at the committed value.</span></span> <span data-ttu-id="e0022-320">Windows 7에서는 **시스템 그룹**의 **커밋(KB)**을 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-320">(In Windows 7, look at **Commit (KB)** in the **System group**.)</span></span>  
     ...
     <span data-ttu-id="e0022-321">**합계**가 **제한**에 가까우면 실제 메모리가 부족한 것입니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-321">If the **Total** is close to the **Limit**, you are running low on physical memory.</span></span>  
...
-   <span data-ttu-id="e0022-333">SOS 디버거 확장이 로드된 WinDbg 또는 Visual Studio 디버거에서 다음 명령을 입력합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-333">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>  
...
-   <span data-ttu-id="e0022-338">SOS 디버거 확장이 로드된 WinDbg 또는 Visual Studio 디버거에서 다음 명령을 입력합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-338">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>  
     ...
     <span data-ttu-id="e0022-339">**!dumpheap –stat**</span><span class="sxs-lookup"><span data-stu-id="e0022-339">**!dumpheap –stat**</span></span>  
...
-   <span data-ttu-id="e0022-363">다음 코드가 포함된 테스트 프로그램을 실행합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-363">Run a test program that contains the following code:</span></span>  
...
-   <span data-ttu-id="e0022-385">0세대에서 사용되는 공간을 계산합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-385">Calculate the space used by generation 0:</span></span>  
     ...
     <span data-ttu-id="e0022-386">**? 49e05d04-0x49521f8c**</span><span class="sxs-lookup"><span data-stu-id="e0022-386">**? 49e05d04-0x49521f8c**</span></span>  
...
-   <span data-ttu-id="e0022-389">다음 명령은 0세대 범위 내의 여유 공간을 덤프합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-389">The following command dumps the free space within the generation 0 range:</span></span>  
  
     <span data-ttu-id="e0022-390">**!dumpheap -type Free -stat 0x49521f8c 49e05d04**</span><span class="sxs-lookup"><span data-stu-id="e0022-390">**!dumpheap -type Free -stat 0x49521f8c 49e05d04**</span></span>  
  
     <span data-ttu-id="e0022-391">결과는 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-391">The result is as follows.</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="e0022-392">이 출력은 힙의 0세대 부분에서 개체에 9MB 공간을 사용하고 있으며 여유 공간이 7MB임을 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-392">This output shows that the generation 0 portion of the heap is using 9 MB of space for objects and has 7 MB free.</span></span> <span data-ttu-id="e0022-393">이 분석은 0세대가 조각화에 기여하는 정도를 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-393">This analysis shows the extent to which generation 0 contributes to fragmentation.</span></span> <span data-ttu-id="e0022-394">장기 개체에 의한 조각화의 원인으로 계산된 전체 크기에서 이 힙 사용량을 차감해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-394">This amount of heap usage should be discounted from the total amount as the cause of fragmentation by long-term objects.</span></span>  
  
<a name="Pinned"></a>   
##### <a name="to-determine-the-number-of-pinned-objects"></a><span data-ttu-id="e0022-395">고정된 개체 수를 확인하려면</span><span class="sxs-lookup"><span data-stu-id="e0022-395">To determine the number of pinned objects</span></span>  
  
-   <span data-ttu-id="e0022-396">SOS 디버거 확장이 로드된 WinDbg 또는 Visual Studio 디버거에서 다음 명령을 입력합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-396">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>  
  
     <span data-ttu-id="e0022-397">**!gchandles**</span><span class="sxs-lookup"><span data-stu-id="e0022-397">**!gchandles**</span></span>  
  
     <span data-ttu-id="e0022-398">다음 예제와 같이 표시된 통계는 고정된 핸들 수를 포함합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-398">The statistics displayed includes the number of pinned handles, as the following example shows.</span></span>  
...
<a name="TimeInGC"></a>   
##### <a name="to-determine-the-length-of-time-in-a-garbage-collection"></a><span data-ttu-id="e0022-399">가비지 수집 기간을 확인하려면</span><span class="sxs-lookup"><span data-stu-id="e0022-399">To determine the length of time in a garbage collection</span></span>  
  
-   <span data-ttu-id="e0022-400">`% Time in GC` 메모리 성능 카운터를 검사합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-400">Examine the `% Time in GC` memory performance counter.</span></span>  
  
     <span data-ttu-id="e0022-401">값은 샘플 간격 시간을 사용하여 계산됩니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-401">The value is calculated by using a sample interval time.</span></span> <span data-ttu-id="e0022-402">각 가비지 수집이 끝날 때 카운터가 업데이트되므로 간격 중에 수집이 발생하지 않은 경우 현재 샘플은 이전 샘플과 동일한 값을 갖습니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-402">Because the counters are updated at the end of each garbage collection, the current sample will have the same value as the previous sample if no collections occurred during the interval.</span></span>  
  
     <span data-ttu-id="e0022-403">수집 시간은 샘플 간격 시간에 백분율 값을 곱하여 구합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-403">Collection time is obtained by multiplying the sample interval time with the percentage value.</span></span>  
  
     <span data-ttu-id="e0022-404">다음 데이터는 8초 연구를 위해 2초 샘플링 간격 4개를 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-404">The following data shows four sampling intervals of two seconds, for an 8-second study.</span></span> <span data-ttu-id="e0022-405">`Gen0`, `Gen1` 및 `Gen2` 열은 해당 세대에 대해 해당 간격 중에 발생한 가비지 수집 횟수를 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-405">The `Gen0`, `Gen1`, and `Gen2` columns show the number of garbage collections that occurred during that interval for that generation.</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="e0022-410">2번째 2세대 가비지 수집은 3번째 간격 중에 시작되고 5번째 간격에 완료되었습니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-410">The second generation 2 garbage collection started during the third interval and finished at the fifth interval.</span></span> <span data-ttu-id="e0022-411">최악의 경우를 가정하여 0세대에 대한 마지막 수집은 2번째 간격이 시작될 때 완료된 수집이었으며 5번째 간격이 끝날 때 2세대 가비지 수집이 완료되었습니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-411">Assuming the worst case, the last garbage collection was for a generation 0 collection that finished at the start of the second interval, and the generation 2 garbage collection finished at the end of the fifth interval.</span></span> <span data-ttu-id="e0022-412">따라서 0세대 가비지 수집의 끝과 2세대 가비지 수집의 끝 사이의 시간은 4초입니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-412">Therefore, the time between the end of the generation 0 garbage collection and the end of the generation 2 garbage collection is 4 seconds.</span></span> <span data-ttu-id="e0022-413">`% Time in GC` 카운터는 20%이므로 2세대 가비지 수집에 소요되었을 수 있는 최대 시간은 (4초 * 20% = 800ms)입니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-413">Because the `% Time in GC` counter is 20%, the maximum amount of time the generation 2 garbage collection could have taken is (4 seconds * 20% = 800ms).</span></span>  
  
-   <span data-ttu-id="e0022-414">또는 [가비지 컬렉션 ETW 이벤트](../../../docs/framework/performance/garbage-collection-etw-events.md)를 사용하여 가비지 컬렉션 길이를 결정하고 정보를 분석하여 가비지 컬렉션 기간을 확인할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-414">Alternatively, you can determine the length of a garbage collection by using [garbage collection ETW events](../../../docs/framework/performance/garbage-collection-etw-events.md), and analyze the information to determine the duration of garbage collection.</span></span>  
  
     <span data-ttu-id="e0022-415">예를 들어 다음 데이터는 비 동시 가비지 컬렉션 중에 발생한 이벤트 시퀀스를 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-415">For example, the following data shows an event sequence that occurred during a non-concurrent garbage collection.</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="e0022-416">관리되는 스레드를 일시 중단하는 데 26us(`GCSuspendEEEnd` – `GCSuspendEEBegin_V1`) 걸렸습니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-416">Suspending the managed thread took 26us (`GCSuspendEEEnd` – `GCSuspendEEBegin_V1`).</span></span>  
  
     <span data-ttu-id="e0022-417">실제 가비지 수집에 4.8ms(`GCEnd_V1` – `GCStart_V1`) 걸렸습니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-417">The actual garbage collection took 4.8ms (`GCEnd_V1` – `GCStart_V1`).</span></span>  
  
     <span data-ttu-id="e0022-418">관리되는 스레드를 다시 시작하는 데 21us(`GCRestartEEEnd` – `GCRestartEEBegin`) 걸렸습니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-418">Resuming the managed threads took 21us (`GCRestartEEEnd` – `GCRestartEEBegin`).</span></span>  
  
     <span data-ttu-id="e0022-419">다음 출력은 백그라운드 가비지 수집에 대한 예제를 제공하며 프로세스, 스레드 및 이벤트 필드를 포함합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-419">The following output provides an example for background garbage collection, and includes the process, thread, and event fields.</span></span> <span data-ttu-id="e0022-420">일부 데이터만 표시됩니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-420">(Not all data is shown.)</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="e0022-421">42504816의 `GCStart_V1` 이벤트는 마지막 필드가 `1`이므로 백그라운드 가비지 수집임을 나타냅니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-421">The `GCStart_V1` event at 42504816 indicates that this is a background garbage collection, because the last field is `1`.</span></span> <span data-ttu-id="e0022-422">가비지 수집 번호</span><span class="sxs-lookup"><span data-stu-id="e0022-422">This becomes garbage collection No.</span></span> <span data-ttu-id="e0022-423">102019가 됩니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-423">102019.</span></span>  
  
     <span data-ttu-id="e0022-424">`GCStart` 이벤트는 백그라운드 가비지 수집을 시작하기 전에 단기 가비지 수집이 필요한 경우에 발생합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-424">The `GCStart` event occurs because there is a need for an ephemeral garbage collection before you start a background garbage collection.</span></span> <span data-ttu-id="e0022-425">가비지 수집 번호</span><span class="sxs-lookup"><span data-stu-id="e0022-425">This becomes garbage collection No.</span></span> <span data-ttu-id="e0022-426">102020이 됩니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-426">102020.</span></span>  
  
     <span data-ttu-id="e0022-427">42514170에서 가비지 수집 번호</span><span class="sxs-lookup"><span data-stu-id="e0022-427">At 42514170, garbage collection No.</span></span> <span data-ttu-id="e0022-428">102020이 완료됩니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-428">102020 finishes.</span></span> <span data-ttu-id="e0022-429">이 지점에서 관리되는 스레드가 다시 시작됩니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-429">The managed threads are restarted at this point.</span></span> <span data-ttu-id="e0022-430">이 수집은 이 백그라운드 가비지 수집을 트리거한 스레드 4372에서 완료됩니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-430">This is completed on thread 4372, which triggered this background garbage collection.</span></span>  
  
     <span data-ttu-id="e0022-431">스레드 4744에서 일시 중단이 발생합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-431">On thread 4744, a suspension occurs.</span></span> <span data-ttu-id="e0022-432">백그라운드 가비지 수집이 관리되는 스레드를 일시 중단해야 하는 경우는 이때뿐입니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-432">This is the only time at which the background garbage collection has to suspend managed threads.</span></span> <span data-ttu-id="e0022-433">이 기간은 약 99ms((63784407-63685394)/1000)입니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-433">This duration is approximately 99ms ((63784407-63685394)/1000).</span></span>  
  
     <span data-ttu-id="e0022-434">백그라운드 가비지 수집에 대한 `GCEnd` 이벤트는 89931423에 있습니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-434">The `GCEnd` event for the background garbage collection is at 89931423.</span></span> <span data-ttu-id="e0022-435">이는 백그라운드 가비지 수집이 약 47초((89931423-42504816)/1000) 동안 지속되었음을 의미합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-435">This means that the background garbage collection lasted for about 47seconds ((89931423-42504816)/1000).</span></span>  
  
     <span data-ttu-id="e0022-436">관리되는 스레드가 실행되는 동안 개수에 관계없이 발생하는 단기 가비지 수집을 확인할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-436">While the managed threads are running, you can see any number of ephemeral garbage collections occurring.</span></span>  
  
<a name="Triggered"></a>   
##### <a name="to-determine-what-triggered-a-garbage-collection"></a><span data-ttu-id="e0022-437">가비지 수집 트리거를 확인하려면</span><span class="sxs-lookup"><span data-stu-id="e0022-437">To determine what triggered a garbage collection</span></span>  
  
-   <span data-ttu-id="e0022-438">SOS 디버거 확장이 로드된 WinDbg 또는 Visual Studio 디버거에서 다음 명령을 입력하여 호출 스택과 함께 모든 스레드를 표시합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-438">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command to show all the threads with their call stacks:</span></span>  
  
     <span data-ttu-id="e0022-439">**~\*kb**</span><span class="sxs-lookup"><span data-stu-id="e0022-439">**~\*kb**</span></span>  
  
     <span data-ttu-id="e0022-440">이 명령은 다음과 유사한 출력을 표시합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-440">This command displays output similar to the following.</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="e0022-441">가비지 수집이 운영 체제의 메모리 부족 알림으로 인해 발생한 경우 스레드가 종료자 스레드라는 것을 제외하면 호출 스택이 비슷합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-441">If the garbage collection was caused by a low memory notification from the operating system, the call stack is similar, except that the thread is the finalizer thread.</span></span> <span data-ttu-id="e0022-442">종료자 스레드는 비동기 메모리 부족 알림을 가져오고 가비지 수집을 실행합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-442">The finalizer thread gets an asynchronous low memory notification and induces the garbage collection.</span></span>  
  
     <span data-ttu-id="e0022-443">가비지 수집이 메모리 할당에 의해 발생한 경우 스택이 다음과 같이 나타납니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-443">If the garbage collection was caused by memory allocation, the stack appears as follows:</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="e0022-444">결국 Just-In-Time 도우미(`JIT_New*`)가 `GCHeap::GarbageCollectGeneration`을 호출합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-444">A just-in-time helper (`JIT_New*`) eventually calls `GCHeap::GarbageCollectGeneration`.</span></span> <span data-ttu-id="e0022-445">2세대 가비지 수집이 할당에 의해 발생한 경우 2세대 가비지 수집에서 수집된 개체와 이를 방지하는 방법을 확인해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-445">If you determine that generation 2 garbage collections are caused by allocations, you must determine which objects are collected by a generation 2 garbage collection and how to avoid them.</span></span> <span data-ttu-id="e0022-446">즉, 2세대 가비지 수집의 시작과 끝 사이의 차이 및 2세대 수집을 발생시킨 개체를 확인하려고 합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-446">That is, you want to determine the difference between the start and the end of a generation 2 garbage collection, and the objects that caused the generation 2 collection.</span></span>  
  
     <span data-ttu-id="e0022-447">예를 들어 디버거에서 다음 명령을 입력하여 2세대 컬렉션의 시작을 표시합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-447">For example, type the following command in the debugger to show the beginning of a generation 2 collection:</span></span>  
  
     <span data-ttu-id="e0022-448">**!dumpheap –stat**</span><span class="sxs-lookup"><span data-stu-id="e0022-448">**!dumpheap –stat**</span></span>  
  
     <span data-ttu-id="e0022-449">예제 출력(가장 많은 공간을 사용하는 개체를 표시하기 위해 요약됨):</span><span class="sxs-lookup"><span data-stu-id="e0022-449">Example output (abridged to show the objects that use the most space):</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="e0022-450">2세대의 끝에서 명령을 반복합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-450">Repeat the command at the end of generation 2:</span></span>  
  
     <span data-ttu-id="e0022-451">**!dumpheap –stat**</span><span class="sxs-lookup"><span data-stu-id="e0022-451">**!dumpheap –stat**</span></span>  
  
     <span data-ttu-id="e0022-452">예제 출력(가장 많은 공간을 사용하는 개체를 표시하기 위해 요약됨):</span><span class="sxs-lookup"><span data-stu-id="e0022-452">Example output (abridged to show the objects that use the most space):</span></span>  
    ...
    ```  
     ...
     <span data-ttu-id="e0022-453">`double[]` 개체가 출력의 끝에서 사라졌으며, 이는 수집되었음을 의미합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-453">The `double[]` objects disappeared from the end of the output, which means that they were collected.</span></span> <span data-ttu-id="e0022-454">이러한 개체가 약 70MB를 차지합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-454">These objects account for approximately 70 MB.</span></span> <span data-ttu-id="e0022-455">나머지 개체는 많이 변경되지 않았습니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-455">The remaining objects did not change much.</span></span> <span data-ttu-id="e0022-456">따라서 이러한 `double[]` 개체가 이 2세대 가비지 수집이 발생한 이유입니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-456">Therefore, these `double[]` objects were the reason why this generation 2 garbage collection occurred.</span></span> <span data-ttu-id="e0022-457">다음 단계에서는 `double[]` 개체가 있는 이유 및 작동하지 않는 이유를 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-457">Your next step is to determine why the `double[]` objects are there and why they died.</span></span> <span data-ttu-id="e0022-458">이러한 개체가 어디서 온 것인지 코드 개발자에게 묻거나 **gcroot** 명령을 사용할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-458">You can ask the code developer where these objects came from, or you can use the **gcroot** command.</span></span>  
  
<a name="HighCPU"></a>   
##### <a name="to-determine-whether-high-cpu-usage-is-caused-by-garbage-collection"></a><span data-ttu-id="e0022-459">높은 CPU 사용량이 가비지 컬렉션에 의해 발생한 것인지 여부를 확인하려면</span><span class="sxs-lookup"><span data-stu-id="e0022-459">To determine whether high CPU usage is caused by garbage collection</span></span>  
  
-   <span data-ttu-id="e0022-460">`% Time in GC` 메모리 성능 카운터 값을 프로세스 시간과 상호 연결합니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-460">Correlate the `% Time in GC` memory performance counter value with the process time.</span></span>  
  
     <span data-ttu-id="e0022-461">`% Time in GC` 값이 프로세스 시간과 동시에 급증하는 경우 가비지 수집으로 인해 CPU 사용량이 증가한 것입니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-461">If the `% Time in GC` value spikes at the same time as process time, garbage collection is causing a high CPU usage.</span></span> <span data-ttu-id="e0022-462">그러지 않으면 응용 프로그램을 프로파일링하여 높은 사용량이 발생하는 위치를 찾습니다.</span><span class="sxs-lookup"><span data-stu-id="e0022-462">Otherwise, profile the application to find where the high usage is occurring.</span></span>  
  
## <a name="see-also"></a><span data-ttu-id="e0022-463">참고 항목</span><span class="sxs-lookup"><span data-stu-id="e0022-463">See Also</span></span>  
 [<span data-ttu-id="e0022-464">가비지 수집</span><span class="sxs-lookup"><span data-stu-id="e0022-464">Garbage Collection</span></span>](../../../docs/standard/garbage-collection/index.md)

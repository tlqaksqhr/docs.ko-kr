...
>  특히 `BeginInvoke` 및 `EndInvoke` 메서드와 같은 비동기 대리자는 .NET Compact Framework에서 호출할 수 없습니다.  
 ...
 `EndInvoke` 메서드는 비동기 호출의 결과를 검색합니다. 이 메서드는 `BeginInvoke`를 호출한 후 언제든지 호출할 수 있습니다. 비동기 호출이 완료되지 않은 경우 `EndInvoke` 는 호출이 완료될 때까지 호출하는 스레드를 차단합니다. 매개 변수 `EndInvoke` 포함는 `out` 및 `ref` 매개 변수 (`<Out>` `ByRef` 및 `ByRef` Visual basic에서)를 비동기적으로 실행 되는 메서드의 <xref:System.IAsyncResult> 반환한 `BeginInvoke`.  
...
-   가져올는 <xref:System.Threading.WaitHandle> 를 사용 하 여는 <xref:System.IAsyncResult.AsyncWaitHandle%2A?displayProperty=nameWithType> 속성을 사용 하 여 해당 <xref:System.Threading.WaitHandle.WaitOne%2A> 실행을 차단할 때까지 메서드는 <xref:System.Threading.WaitHandle> 발생 한 다음 호출 `EndInvoke`합니다.  
...
> [!NOTE]
>  대기 핸들은 `EndInvoke`를 호출할 때 자동으로 닫히지 않습니다. 대기 핸들에 대한 모든 참조를 해제하면 가비지 수집에서 대기 핸들을 회수할 때 시스템 리소스가 확보됩니다. 시스템 리소스를 대기 핸들을 사용 하 여 완료 되는 즉시 확보 하기 위해 작업을 삭제할 호출 하 여는 <xref:System.Threading.WaitHandle.Close%2A?displayProperty=nameWithType> 메서드. 삭제 가능한 개체를 명시적으로 삭제하면 가비지 수집의 효율성이 향상됩니다.  
...
## <a name="executing-a-callback-method-when-an-asynchronous-call-completes"></a>비동기 호출이 완료될 때 콜백 메서드 실행  
 ...
 콜백 메서드를 사용하려면 `BeginInvoke` 에 콜백 메서드를 나타내는 <xref:System.AsyncCallback> 대리자를 전달해야 합니다. 콜백 메서드에서 사용할 정보가 들어 있는 개체를 전달할 수도 있습니다. 콜백 메서드에서는 콜백 메서드의 유일한 매개 변수인 <xref:System.IAsyncResult>를 <xref:System.Runtime.Remoting.Messaging.AsyncResult> 개체로 캐스팅할 수 있습니다. 그런 다음 사용할 수는 <xref:System.Runtime.Remoting.Messaging.AsyncResult.AsyncDelegate%2A?displayProperty=nameWithType> 는 호출할 수 있도록 호출을 시작 하는 데 사용 된 대리자를 가져올 속성 `EndInvoke`합니다.  
  
 예제 관련 참고 사항:  
  
-   `threadId` 의 매개 변수 `TestMethod` 는 `out` 매개 변수 ([`<Out>` `ByRef` Visual basic에서) 입력된 값 사용 되지 않으므로 `TestMethod`합니다. `BeginInvoke` 를 호출할 때는 더미 변수가 전달됩니다. `threadId` 매개 변수가 `ref` 매개 변수(Visual Basic의 경우`ByRef` )인 경우에는 변수가 `BeginInvoke` 및 `EndInvoke`에 전달 가능한 클래스 수준 필드여야 합니다.  

...
-   익숙한 이벤트-대리자 모델을 사용하여 보류 중인 비동기 작업과 통신할 수 있습니다. 대리자 및 이벤트 처리기 사용에 대 한 자세한 내용은 참조 하십시오. [이벤트](../../../docs/standard/events/index.md)합니다.  
  
 라는 메서드 하나 이상 가진 이벤트 기반 비동기 패턴을 지 원하는 클래스가 *MethodName*`Async`합니다. 이러한 메서드는 현재 스레드에서 동일한 작업을 수행하는 동기 버전을 미러링할 수 있습니다. 클래스도 있을 수는 *MethodName* `Completed` 이벤트와 해당 있을 수 있습니다는 *MethodName* `AsyncCancel` (또는 단순히 `CancelAsync`) 메서드.  
...
> [!CAUTION]
>  다운로드가 <xref:System.Windows.Forms.PictureBox.CancelAsync%2A> 요청이 수행된 것처럼 완료될 수 있으므로 <xref:System.ComponentModel.AsyncCompletedEventArgs.Cancelled%2A>가 취소 요청을 반영하지 않을 수 있습니다. 이 라고는 *경합 상태* 는 다중 스레드 프로그래밍의 일반적인 문제입니다. 다중 스레드 프로그래밍에 대 한 문제에 대 한 자세한 내용은 참조 하십시오. [관리 되는 스레딩 유용한](../../../docs/standard/threading/managed-threading-best-practices.md)합니다.  
  
## <a name="characteristics-of-the-event-based-asynchronous-pattern"></a>이벤트 기반 비동기 패턴의 특징  
 이벤트 기반 비동기 패턴은 특정 클래스에서 지원하는 작업의 복잡성에 따라 여러 가지 형식을 취할 수 있습니다. 가장 단순한 클래스는 단일 있을 수 *MethodName* `Async` 메서드 및 해당 *MethodName* `Completed` 이벤트입니다. 보다 복잡 한 클래스는 여러 개 있을 수 *MethodName* `Async` 메서드, 각 해당 *MethodName* `Completed` 으로 이벤트를 이러한 메서드의 동기 버전입니다. 클래스는 선택적으로 각 비동기 메서드에 대해 취소, 진행률 보고 및 증분 결과를 지원할 수 있습니다.  
...
### <a name="asynchronous-method-overloads"></a>비동기 메서드 오버로드  
 ...
 다중 호출 오버로드의 `userState` 매개 변수를 사용하면 비동기 작업 간에 구별할 수 있습니다. 각 호출에 대 한 고유한 값 (예: GUID 또는 해시 코드)을 제공 `Method1Async(string param, object userState)`, 각 작업이 완료 될 때 이벤트 처리기에서 완료 이벤트가 발생 하는 작업의 인스턴스를 결정할 수 있습니다.  
  
### <a name="tracking-pending-operations"></a>보류 중인 작업 추적  
 다중 호출 오버로드를 사용하는 경우 코드에서 보류 중인 작업의 `userState` 개체(작업 ID)를 추적해야 합니다. 각 호출에 대 한 `Method1Async(string param, object userState)`, 새 생성 일반적으로 고유 `userState` 개체를 컬렉션에 추가 합니다. 이 `userState` 개체에 해당하는 작업에서 완료 이벤트가 발생하면 완료 메서드 구현에서 <xref:System.ComponentModel.AsyncCompletedEventArgs.UserState%2A?displayProperty=nameWithType>를 검토하고 컬렉션에서 제거합니다. 이 방법을 사용할 경우 `userState` 매개 변수는 작업 ID 역할을 합니다.  
...
### <a name="canceling-pending-operations"></a>보류 중인 작업 취소  
 비동기 작업이 완료되기 전에 언제든지 해당 작업을 취소할 수 있어야 합니다. 이벤트 기반 비동기 패턴을 구현 하는 클래스는 `CancelAsync` 메서드 (비동기 메서드가 하나뿐인 경우) 또는 *MethodName* `AsyncCancel` 메서드 (여러 비동기 메서드가 있는 경우).  
...
### <a name="receiving-progress-updates-and-incremental-results"></a>진행률 업데이트 및 증분 결과 받기  
 이벤트 기반 비동기 패턴을 준수하는 클래스는 진행률 및 증분 결과를 추적하기 위한 이벤트를 선택적으로 제공할 수 있습니다. 이름은 일반적으로이 `ProgressChanged` 또는 *MethodName*`ProgressChanged`, 및 해당 이벤트 처리기 걸리는 <xref:System.ComponentModel.ProgressChangedEventArgs> 매개 변수입니다.  
  
 에 대 한 이벤트 처리기는 `ProgressChanged` 이벤트를 검사할 수는 <xref:System.ComponentModel.ProgressChangedEventArgs.ProgressPercentage%2A?displayProperty=nameWithType> 완료 된 비동기 작업의 백분율을 결정 하는 속성입니다. 이 속성은 범위가 0~100이며, <xref:System.Windows.Forms.ProgressBar.Value%2A>의 <xref:System.Windows.Forms.ProgressBar> 속성을 업데이트하는 데 사용될 수 있습니다. 보류 중인 비동기 작업이 여러 개인 경우 <xref:System.ComponentModel.ProgressChangedEventArgs.UserState%2A?displayProperty=nameWithType> 속성을 사용하여 진행률을 보고 중인 작업을 구별할 수 있습니다.  

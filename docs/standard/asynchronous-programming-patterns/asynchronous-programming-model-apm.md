...
-   응용 프로그램의 주 스레드에서 **End** *OperationName*을 호출하여 작업이 완료될 때까지 응용 프로그램 실행을 블로킹합니다. 이 방법을 설명하는 예제는 [Blocking Application Execution by Ending an Async Operation](../../../docs/standard/asynchronous-programming-patterns/blocking-application-execution-by-ending-an-async-operation.md)을 참조하세요.  

...
-   APM\(비동기 프로그래밍 모델\) \- <xref:System.IO.FileStream.BeginRead%2A?displayProperty=fullName> 및 <xref:System.IO.Stream.EndRead%2A?displayProperty=fullName>와 같은 Begin\/End 메서드 쌍으로 비동기 작업을 표현합니다.  
  
-   EAP\(이벤트 기반 비동기 패턴\) \- <xref:System.Net.WebClient.DownloadStringAsync%2A?displayProperty=fullName> 및 <xref:System.Net.WebClient.DownloadStringCompleted?displayProperty=fullName>와 같이 *OperationName*Async 및 *OperationName*Completed라는 메서드\/이벤트 쌍으로 비동기 작업을 표현합니다.  EAP는 .NET Framework 버전 2.0에 도입되었습니다.  
...
## 작업으로 APM 작업 래핑  
 <xref:System.Threading.Tasks.TaskFactory?displayProperty=fullName> 및 <xref:System.Threading.Tasks.TaskFactory%601?displayProperty=fullName> 클래스는 둘 다 APM Begin\/End 메서드 쌍을 <xref:System.Threading.Tasks.Task> 또는 <xref:System.Threading.Tasks.Task%601> 인스턴스 하나로 캡슐화하도록 하는 <xref:System.Threading.Tasks.TaskFactory.FromAsync%2A?displayProperty=fullName> 및 <xref:System.Threading.Tasks.TaskFactory%601.FromAsync%2A?displayProperty=fullName> 메서드의 여러 가지 오버로드를 제공합니다.  다양한 오버로드가 입력 매개 변수 수가 0~3개인 Begin\/End 메서드 쌍을 수용합니다.  
...
### End 메서드 전용 FromAsync 작업  
 `Begin` 메서드에 입력 매개 변수가 4개 이상 필요하거나 `ref` 또는 `out` 매개 변수가 있는 경우 `End` 메서드만 표현하는 `FromAsync` 오버로드\(예: <xref:System.Threading.Tasks.TaskFactory%601.FromAsync%28System.IAsyncResult%2CSystem.Func%7BSystem.IAsyncResult%2C%600%7D%29?displayProperty=fullName>\)를 사용할 수 있습니다.  <xref:System.IAsyncResult>를 전달하고 이를 작업으로 캡슐화하려는 시나리오에서 이들 메서드를 사용할 수도 있습니다.  
...
## 복잡한 EAP 작업을 작업으로 노출  
 ...
 추가적인 예외 처리가 포함되고 클라이언트 코드에서 메서드를 호출하는 방법을 보여 주는 더 자세한 예제는 [How to: Wrap EAP Patterns in a Task](../../../docs/standard/parallel-programming/how-to-wrap-eap-patterns-in-a-task.md)을 참조하세요.  
...
## StreamExtensions 샘플 코드 사용  
 MSDN 웹 사이트의 [.NET Framework 4를 사용한 병렬 프로그래밍 샘플\(영문\)](http://go.microsoft.com/fwlink/?LinkID=165717)에서 제공되는 Streamextensions.cs 파일에는 비동기 파일 및 네트워크 I\/O에 Task 개체를 사용하는 몇 가지 참조 구현이 포함되어 있습니다.  

...
클래스 및 멤버에는 <xref:System.Collections> 기본적으로 문화권 구분 하는 동작을 제공 하는 네임 스페이스입니다. 에 대 한 기본 생성자는 <xref:System.Collections.CaseInsensitiveComparer> 및 <xref:System.Collections.CaseInsensitiveHashCodeProvider> 클래스를 사용 하 여 새 인스턴스 초기화는 <xref:System.Threading.Thread.CurrentCulture%2A?displayProperty=nameWithType> 속성입니다. 모든 오버 로드는 <xref:System.Collections.Specialized.CollectionsUtil.CreateCaseInsensitiveHashtable%2A?displayProperty=nameWithType> 메서드의 새 인스턴스를 만들는 <xref:System.Collections.Hashtable> 클래스 사용은 `Thread.CurrentCulture` 기본적으로 속성입니다. 오버 로드는 <xref:System.Collections.ArrayList.Sort%2A?displayProperty=nameWithType> 메서드 기본을 사용 하 여 문화권 구분 정렬을 수행 `Thread.CurrentCulture`합니다. 정렬 및 조회는 <xref:System.Collections.SortedList> 영향을 받을 수 `Thread.CurrentCulture` 문자열 키로 사용 되는 경우. `Collections` 네임스페이스의 이러한 클래스 및 메서드에서 문화권을 구분하는 결과를 얻으려면 이 섹션에 제공된 사용 권장 사항을 따릅니다.  
  
 **참고** 전달 <xref:System.Globalization.CultureInfo.InvariantCulture%2A?displayProperty=nameWithType> 메서드 비교에 문화권을 구분 하지 않는 비교를 수행지 않습니다. 그러나 파일 경로, 레지스트리 키 및 환경 변수 등과 같은 비언어적 비교는 수행되지 않습니다. 비교 결과에 따라 보안 결정을 지원하지도 않습니다. 비 언어적 비교 또는 결과 기반 보안 결정에 대 한 지원에 대 한 응용 프로그램을 허용 하는 비교 메서드를 사용 해야는 <xref:System.StringComparison> 값입니다. 응용 프로그램을 다음으로 전달할지 <xref:System.StringComparison>합니다.  
...
```  
 ...
 만들 문화권을 구분 하지 않는 경우 `Hashtable` 를 사용 하 여는 `CaseInsensitiveComparer` 및 `CaseInsensitiveHashCodeProvider` 클래스를 사용 하는 생성자를 사용 하 여 이러한 클래스의 새 인스턴스를 초기화 한 `culture` 매개 변수입니다. `culture` 매개 변수의 경우 <xref:System.Globalization.CultureInfo.InvariantCulture%2A?displayProperty=nameWithType>를 지정합니다. 다음 코드 예제에서는 문화권을 구분하지 않는 `Hashtable`에 대한 생성자를 보여 줍니다.  

...
전체 DevOps 스패닝 외부 반복 워크플로 트리거하기 전에 순환, 모든 정보가 각 개발자의 컴퓨터에서 해당 앱을 코딩 하 고 그의 기본 설정된 언어 또는 플랫폼을 사용 하 여 로컬로 테스트 (그림 4-14). 하겠지만 모든 경우에 매우 중요 한 점은 공통적으로 선택 하는 언어, 프레임 워크 또는 플랫폼에 관계 없이 합니다. 이 특정 워크플로의 항상 개발 및 Docker 컨테이너를 로컬로 테스트 합니다.

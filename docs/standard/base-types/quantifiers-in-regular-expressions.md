...
수량자는 찾을 일치 항목의 입력에 있어야 하는 문자, 그룹 또는 문자 클래스의 인스턴스 수를 지정합니다.  다음 테이블에서는 .NET에서 지원하는 수량자를 보여줍니다.  
...
|`+`|`+?`|1번 이상 일치합니다.|  
|`?`|`??`|0번 또는 1번 일치합니다.|  
|`{` *n* `}`|`{` *n* `}?`|정확히 *n*번 일치 항목을 찾습니다.|  
|`{` *n* `,}`|`{` *n* `,}?`|*n*번 이상 일치 항목을 찾습니다.|  
|`{` *n* `,` *분* `}`|`{` *n* `,` *분* `}?`|*n*번에서 *m*번까지 일치 항목을 찾습니다.|  
...
## <a name="regular-expression-quantifiers"></a>정규식 수량자  
 다음 섹션에는 .NET의 정규식에서 지원하는 수량자를 보여줍니다.  
...
### <a name="match-exactly-n-times-n"></a>정확히 n번 일치: {n}  
 `{`*n*`}` 수량자는 이전 요소를 정확하게 *n*번 찾으며, 여기서 *n*은 정수입니다. `{`*n*`}`는 게으른 수량자가 `{`*n*`}?`인 탐욕적 수량자입니다.  
...
### <a name="match-at-least-n-times-n"></a>적어도 n번 일치: {n,}  
 `{`*n*`,}` 수량자는 이전 요소를 *n*번 이상 찾으며, 여기서 *n*은 정수입니다. `{`*n*`,}`는 게으른 수량자가 `{`*n*`}?`인 탐욕적 수량자입니다.  
...
### <a name="match-between-n-and-m-times-nm"></a>n번에서 m번 사이에 일치: {n, m}  
 `{`*n*`,`*m*`}` 수량자는 이전 요소를 *n*번 이상, *m*번 미만 찾으며, 여기서 *n* 및 *m*은 정수입니다. `{`*n*`,`*m*`}`은 게으른 수량자가 `{`*n*`,`*m*`}?`인 탐욕적 수량자입니다.  
...
### <a name="match-exactly-n-times-lazy-match-n"></a>정확히 n번 일치(게으른 일치): {n}?  
 `{`*n*`}?` 수량자는 이전 요소를 정확하게 `n`번 찾으며, 여기서 *n*은 정수입니다. 탐욕적 수량자 `{`*n*`}+`의 게으른 수량자입니다.  
...
### <a name="match-at-least-n-times-lazy-match-n"></a>적어도 n번 일치(게으른 일치): {n,}?  
 `{`*n*`,}?` 수량자는 이전 요소를 `n`번 이상 찾지만 가능한 적은 횟수로 찾으며, 여기서 *n*은 정수입니다. 탐욕적 수량자 `{`*n*`,}`의 게으른 수량자입니다.  
  
 설명은 이전 섹션의 `{`*n*`}?` 수량자에 대한 예제를 참조하세요. 해당 예제에서 정규식은 `{`*n*`,}` 수량자를 사용하여 3개 이상의 문자가 있으며 뒤에 마침표가 있는 문자열을 찾습니다.  
...
-   욕심 없는 (또는 게으른) 버전.  
     ...
     욕심 없는 수량자는 요소를 최대한 적게 찾으려고 합니다. `?`를 추가하기만 하면 탐욕적 수량자를 게으른 수량자로 설정할 수 있습니다.  

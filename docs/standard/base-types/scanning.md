...
`=` |등호와 일치합니다.
`\s*` | 0개 이상의 공백 문자가 일치하는지 확인합니다.
`(?:["']&#40;?<1>[^"']*)"&#124;(?<1>\S+))` | 캡처된 그룹에 결과를 할당하지 않고 다음 중 하나가 일치하는지 확인합니다. 따옴표 또는 아포스트로피 뒤에 따옴표 또는 아포스트로피 이외의 다른 문자가&0;개 이상 나오고 그 뒤에 다시 따옴표 또는 아포스트로피가 나오는 항목. `1`이라는 그룹이 이 패턴에 포함됩니다. 또는 하나 이상의 공백이 아닌 문자를 찾습니다. `1`이라는 그룹이 이 패턴에 포함됩니다.
`(?<1>[^"']*)` | `1`이라는 캡처 그룹에 따옴표 또는 아포스트로피 이외의 다른 문자 항목을&0;개 이상 할당합니다.

...
다음 코드 예제에서는 <xref:System.Text.RegularExpressions.Regex.Replace%2A?displayProperty=nameWithType> 대체할 날짜 형식을 사용 하는 메서드를 *mm*/*dd*/*yy* 하는 날짜 형식으로 되어 *dd*-*mm*-*yy*합니다.  
...
## <a name="comments"></a>설명  
 정규식 패턴 `\b(?<month>\d{1,2})/(?<day>\d{1,2})/(?<year>\d{2,4})\b` 다음 표와 같이 해석 됩니다.  

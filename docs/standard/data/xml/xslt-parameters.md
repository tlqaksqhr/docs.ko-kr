 ...
 *단일 노드가 포함된 노드 집합과 같습니다.  
 ...
 매개 변수 개체가 위 클래스에 해당하지 않으면 다음 규칙에 따라 변환됩니다. CLR(공용 언어 런타임) 숫자 형식은 <xref:System.Double>로 변환됩니다. <xref:System.DateTime> 형식은 <xref:System.String>으로 변환되고, <xref:System.Xml.XPath.IXPathNavigable> 형식은 <xref:System.Xml.XPath.XPathNavigator>로 변환됩니다. **XPathNavigator** 변환할 <xref:System.Xml.XPath.XPathNodeIterator>합니다.  

---
title: "XPathNavigator를 사용하여 XML 데이터 추출 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
## XPathNavigator를 문자열로 변환  
 <xref:System.Xml.XPath.XPathNavigator> 클래스의 <xref:System.Xml.XPath.XPathNavigator.OuterXml%2A> 속성을 사용하여 전체 XML 문서의 태그를 가져오거나 단일 노드 및 해당 자식 노드의 태그만 가져올 수 있습니다.  
...
## XPathNavigator를 XmlReader로 변환  
 ...
 현재 노드 및 자식 노드를 사용하여 <xref:System.Xml.XmlReader> 개체를 만든 경우 <xref:System.Xml.XmlReader> 개체의 <xref:System.Xml.XmlReader.ReadState%2A> 속성은 <xref:System.Xml.ReadState>로 설정됩니다.  <xref:System.Xml.XmlReader> 개체의 <xref:System.Xml.XmlReader.Read%2A> 메서드를 처음으로 호출하면 <xref:System.Xml.XmlReader>가 <xref:System.Xml.XPath.XPathNavigator>의 현재 노드로 이동합니다.  새 <xref:System.Xml.XmlReader> 개체는 XML 트리 끝에 도달할 때까지 계속 읽습니다.  이때 <xref:System.Xml.XmlReader.Read%2A> 메서드는 `false`를 반환하고 <xref:System.Xml.XmlReader> 개체의 <xref:System.Xml.XmlReader.ReadState%2A> 속성은 <xref:System.Xml.ReadState>로 설정됩니다.  
  
 <xref:System.Xml.XmlReader> 개체를 만들거나 이동해도 <xref:System.Xml.XPath.XPathNavigator> 개체의 위치는 변경되지 않습니다.  <xref:System.Xml.XPath.XPathNavigator.ReadSubtree%2A> 메서드는 요소 또는 루트 노드에 있을 경우에만 유효합니다.  
  
 다음 예제에서는 <xref:System.Xml.XPath.XPathDocument> 개체의 전체 XML 문서를 포함하는 <xref:System.Xml.XmlReader> 개체뿐만 아니라 단일 노드 및 해당 자식 노드도 가져오는 방법을 보여 줍니다.  
...
## XPathNavigator를 XmlWriter로 변환  
 ...
 <xref:System.Xml.XmlWriter> 개체를 만들어도 <xref:System.Xml.XPath.XPathNavigator> 개체의 위치는 변경되지 않습니다.  
  
 다음 예제에서는 <xref:System.Xml.XPath.XPathDocument> 개체의 전체 XML 문서를 포함하는 <xref:System.Xml.XmlWriter> 개체뿐만 아니라 단일 노드 및 해당 자식 노드도 가져오는 방법을 보여 줍니다.  

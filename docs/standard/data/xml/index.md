---
title: "XML 문서 및 데이터 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
---
# XML 문서 및 데이터
.NET Framework에서 XML 인식 응용 프로그램을 쉽게 작성할 수 있도록 하는 종합적이고 통합된 클래스 집합을 제공합니다.  다음 네임스페이스의 클래스에서는 XML 구문 분석 및 작성, 메모리에서의 XML 데이터 편집, 데이터 유효성 검사 및 XSLT 변환을 지원합니다.  
...
-   <xref:System.Xml.Linq>  
 ...
 전체 목록은 [System.Xml 네임스페이스](http://msdn.microsoft.com/library/gg145036.aspx) 웹 페이지를 참조하세요.  
  
 이러한 네임스페이스의 클래스는 W3C\(World Wide Web 컨소시엄\) 권장 사항을 지원합니다.  예를 들면 다음과 같습니다.  
  
-   <xref:System.Xml.XmlDocument?displayProperty=fullName> 클래스는 [W3C DOM\(문서 개체 모델\) 수준 1 코어](http://www.w3.org/TR/REC-DOM-Level-1/) 및 [DOM 수준 2 코어](http://www.w3.org/TR/DOM-Level-2-Core/) 권장 사항을 구현합니다.  
  
-   <xref:System.Xml.XmlReader?displayProperty=fullName> 및 <xref:System.Xml.XmlWriter?displayProperty=fullName> 클래스는 [W3C XML 1.0](http://www.w3.org/TR/2006/REC-xml-20060816/) 및 [Namespaces in XML](http://www.w3.org/TR/REC-xml-names/) 권장 사항을 지원합니다.  
  
-   <xref:System.Xml.Schema.XmlSchemaSet?displayProperty=fullName> 클래스의 스키마는 [W3C XML 스키마\(1부\): 구조체](http://www.w3.org/TR/xmlschema-1/) 및 [XML 스키마\(2부\): 데이터 형식](http://www.w3.org/TR/xmlschema-2/) 권장 사항을 지원합니다.  
...
-   **생산성.** [LINQ to XML](../../../../ocs/visual-basic/programming-guide/concepts/linq/linq-to-xml.md)을 사용하면 XML을 사용하여 더욱 쉽게 프로그래밍하고 SQL와 유사한 쿼리 환경을 제공합니다.  
...
-   **플러그형 아키텍처.** .NET Framework는 구성 요소가 각기 다른 구성 요소를 사용할 수 있으며 구성 요소 간에 데이터를 스트리밍할 수 있는 아키텍처를 제공합니다.  예를 들어 <xref:System.Xml.XPath.XPathDocument> 또는 <xref:System.Xml.XmlDocument> 개체와 같은 데이터 저장소는 <xref:System.Xml.Xsl.XslCompiledTransform> 클래스를 사용하여 변환할 수 있고, 출력이 다른 저장소로 스트리밍되거나 웹 서비스에서 스트림으로 반환됩니다.  
  
-   **성능.** 앱 성능 개선을 위해 .NET Framework의 일부 XML 클래스는 다음과 같은 특징을 가진 스트리밍 기반 모델을 지원합니다.  
    ...
    -   노드 생성이 단일 가상 노드로 최소화되지만 문서에 임의로 액세스할 수 있는 커서 스타일 탐색\(<xref:System.Xml.XPath.XPathNavigator>\).  
     ...
     성능 향상을 위해 XSLT 처리가 필요할 때마다 <xref:System.Xml.XPath.XPathDocument> 클래스를 사용합니다. <xref:System.Xml.XPath.XPathDocument>는 <xref:System.Xml.Xsl.XslCompiledTransform> 클래스와 함께 효율적으로 사용하도록 디자인된 XPath 쿼리를 위한 최적화된 읽기 전용 저장소입니다.  
...
## 단원 내용  
 ...
 메모리 내 XML 데이터 처리에 대한 세 가지 모델을 설명합니다.  [LINQ to XML](../../../../ocs/visual-basic/programming-guide/concepts/linq/linq-to-xml.md), <xref:System.Xml.XmlDocument> 클래스\(W3C 문서 개체 모델 기반\) 및 <xref:System.Xml.XPath.XPathDocument> 클래스\(XPath 데이터 모델 기반\).  

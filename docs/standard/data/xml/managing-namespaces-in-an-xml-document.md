---
title: "XML 문서의 네임스페이스 관리 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
<a name="scope"></a>   
## 선언 범위  
 네임스페이스는 선언 지점부터 선언된 요소의 끝까지 유효합니다.  이 예에서 `BOOK` 요소에서 정의된 네임스페이스는 `Publisher` 요소와 같은 `BOOK` 요소 외부의 요소에는 적용되지 않습니다.  
...
## 네임스페이스 관리  
 ...
 네임스페이스 관리자는 네임스페이스에 대한 유효성 검사를 수행하지 않지만 접두사 및 네임스페이스가 이미 확인되었고 [W3C 네임스페이스](http://www.w3.org/TR/REC-xml-names/)\(영문\) 사양을 따르는 것으로 간주합니다.  
  
> [!NOTE]
>  [LINQ to XML](../../../../ocs/visual-basic/programming-guide/concepts/linq/linq-to-xml.md)은 네임스페이스를 관리하기 위해 <xref:System.Xml.XmlNamespaceManager>를 사용하지 않습니다.  LINQ to XML을 사용할 때 네임스페이스를 관리하는 방법에 대한 내용은 LINQ 설명서에서 [XML 네임스페이스 작업](../../../../ocs/visual-basic/programming-guide/concepts/linq/working-with-xml-namespaces.md)을 참조하세요.  

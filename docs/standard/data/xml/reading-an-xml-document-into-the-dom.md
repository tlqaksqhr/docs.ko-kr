...
|Subject|항목|  
|-------------|-----------|  
|공백 노드 만들기|DOM을 로드하는 데 사용된 개체는 DOM에 생성된 공백 및 유효 공백 노드에 영향을 줍니다. 자세한 내용은 참조 [공백 및 DOM을 로드할 때 중요 한 공백 처리](../../../../docs/standard/data/xml/white-space-and-significant-white-space-handling-when-loading-the-dom.md)합니다.|  
|특정 노드부터 XML 로드 또는 전체 XML 문서 로드|사용 하 여 <xref:System.Xml.XmlDocument.Load%2A?displayProperty=nameWithType> DOM에 있으며 특정 노드에서 메서드 데이터를 로드할 수 있습니다 자세한 내용은 참조 [판독기에서 데이터 로드](../../../../docs/standard/data/xml/load-data-from-a-reader.md)합니다.|  
|XML을 로드할 때 유효성 검사|DOM으로 XML 데이터를 로드할 때 유효성을 검사할 수 있습니다. <xref:System.Xml.XmlReader> 유효성 검사를 사용하면 됩니다. 로드 되 면 XML의 유효성을 검사 하는 방법에 대 한 자세한 내용은 참조 [DOM에서 XML 문서 유효성 검사](../../../../docs/standard/data/xml/validating-an-xml-document-in-the-dom.md)합니다.|  

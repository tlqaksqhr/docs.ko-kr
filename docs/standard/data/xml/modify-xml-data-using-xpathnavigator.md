---
title: "XPathNavigator를 사용하여 XML 데이터 수정 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
## 노드 수정  
 <xref:System.Xml.XPath.XPathNavigator> 클래스의 <xref:System.Xml.XPath.XPathNavigator.SetValue%2A> 및 <xref:System.Xml.XPath.XPathNavigator.SetTypedValue%2A> 메서드를 사용하면 쉽게 노드 값을 변경할 수 있습니다.  
  
 다음 표에서는 여러 노드 형식에서 이러한 메서드를 사용한 결과를 보여 줍니다.  
  
|<xref:System.Xml.XPath.XPathNodeType>|변경되는 데이터|  
|---------------------------------------------------------------------------------------------------------------------------------------------|--------------|  
|<xref:System.Xml.XPath.XPathNodeType>|지원되지 않습니다.|  
|<xref:System.Xml.XPath.XPathNodeType>|요소 내용입니다.|  
|<xref:System.Xml.XPath.XPathNodeType>|특성 값|  
|<xref:System.Xml.XPath.XPathNodeType>|텍스트 내용입니다.|  
|<xref:System.Xml.XPath.XPathNodeType>|대상을 제외한 내용|  
|<xref:System.Xml.XPath.XPathNodeType>|주석의 내용입니다.|  
|<xref:System.Xml.XPath.XPathNodeType>|지원되지 않음|  
  
> [!NOTE]
>  <xref:System.Xml.XPath.XPathNodeType> 노드나 <xref:System.Xml.XPath.XPathNodeType> 노드를 편집할 수 없습니다.  
...
>  특성에는 단순 형식만 있을 수 있습니다.  
 ...
 요소의 유효성이 자식 요소 및 특성의 유효성에 따라 결정되는 경우 자식 요소나 특성을 수정하면 요소가 이전에 유효했더라도 요소의 유효성이 변경됩니다.  특히 요소의 자식 요소 또는 특성을 삽입, 업데이트 또는 삭제할 경우 요소의 유효성을 알 수 없습니다.  요소의 유효성은 요소의 <xref:System.Xml.XPath.XPathNavigator.SchemaInfo%2A> 속성의 <xref:System.Xml.Schema.IXmlSchemaInfo.Validity%2A> 속성을 <xref:System.Xml.Schema.XmlSchemaValidity>으로 설정하여 나타낼 수 있습니다.  더욱이 요소의 부모 요소, 이 부모 요소의 부모 요소 등의 유효성도 알 수 없기 때문에 이 결과는 XML 문서에서 재귀적 상향 계단식으로 나타납니다.  
...
### 특성 수정  
 ...
 다음 예제에서는 `books.xml` 파일에서 첫 번째 `book` 요소의 `genre` 특성 값을 변경합니다.  
...
## InnerXml 및 OuterXml 속성  
 ...
 다음 예제에서는 <xref:System.Xml.XPath.XPathNavigator.OuterXml%2A> 속성을 사용하여 `price` 요소 값을 수정하고 `contosoBooks.xml` 파일의 첫 번째 `book` 요소에 새 `discount` 특성을 삽입합니다.  
...
```  
 ...
 위 XML 예제에서는 `http://www.contoso.com/parent-id` 네임스페이스의 `parent` 요소에 `a:parent-id` 특성을 삽입합니다.  `parent` 요소에서 <xref:System.Xml.XPath.XPathNavigator.CreateAttribute%2A> 메서드를 사용하여 특성을 삽입할 수 있습니다.  <xref:System.Xml.XPath.XPathNavigator> 클래스에서 `http://www.contoso.com` 네임스페이스 선언을 자동으로 삽입하여 XML 문서 나머지 부분의 일관성을 유지합니다.  
...
## xsi:nil 노드 수정  
 W3C XML 스키마 권장 사항에는 nillable 요소의 개념이 새로 추가되었습니다.  nillable 요소는 내용이 없고 여전히 유효할 수 있습니다.  nillable 요소의 개념은 `null`인 개체의 개념과 비슷합니다.  주요 차이점은 `null` 개체에는 액세스할 수 없으며 `xsi:nil` 요소에는 액세스할 수 있지만 자식 요소나 텍스트 등의 내용이 없는 특성과 같은 속성이 있다는 점입니다.  XML 문서의 요소에서 `true` 값의 `xsi:nil` 특성을 사용하여 요소에 내용이 없음을 나타낼 수 있습니다.  
  
 <xref:System.Xml.XPath.XPathNavigator> 개체를 사용하여 `true` 값의 `xsi:nil` 특성이 있는 유효한 요소에 내용을 추가하는 경우 `xsi:nil` 특성 값은 `false`로 설정됩니다.  

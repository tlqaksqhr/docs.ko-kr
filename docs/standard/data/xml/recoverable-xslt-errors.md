---
title: "복구할 수 있는 XSLT 오류 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
-   복구는 <xref:System.Xml.Xsl.XslCompiledTransform> 클래스가 이 오류로부터 복구하는 것을 나타냅니다.  <xref:System.Xml.Xsl.XsltArgumentList.XsltMessageEncountered?displayProperty=fullName> 이벤트를 사용하여 XSLT 프로세서의 모든 이벤트를 보고할 수 있습니다.  
  
-   오류는 이 조건에 대해 예외가 발생함을 나타냅니다.  
  
-   [W3C XSLT\(XSL 변환\) 버전 1.0 권장 사항](http://go.microsoft.com/fwlink/?LinkId=49919)\(영문\) 및 [W3C XSLT\(XSL 변환\) 버전 1.0 사양 오류](http://go.microsoft.com/fwlink/?LinkId=49917)\(영문\)에서 섹션 참조를 찾을 수 있습니다.  
...
|특성 값에서 생성된 `xsl:attribute` 및 `xsl:element`의 `name` 특성은 QName이 아닙니다.|7.1.2, 7.1.3|오류\*|  
|가져오기 및 확장 이름이 같은 두 특성 집합에는 공통적인 특성이 하나 있으며 중요도가 더 높고 동일한 이름의 공통 특성을 포함하는 다른 특성 집합은 없습니다.|7.1.4|복구|  
...
|`xsl:processing-instruction`의 `name` 특성은 NCName과 처리 명령 대상을 둘 다 생성하지 않습니다.|7.3|오류\*|  
...
|`xsl:number`의 `value` 특성이 NAN 또는 무한 값이거나 0.5 미만입니다.|errata 24|복구|  

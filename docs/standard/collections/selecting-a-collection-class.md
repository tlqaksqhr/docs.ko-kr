...
ms.topic: "article"
helpviewer_keywords: 
  - "컬렉션[.NET Framework], collection 클래스 선택"
  - "Collections 클래스"
  - "선입선출(first-in-first-out) 컬렉션"
  - "컬렉션의 데이터 그룹화, collection 클래스 선택"
  - "인덱싱된 컬렉션"
  - "후입선출(last-in-first-out) 컬렉션"
...
## LINQ to Objects 및 PLINQ  
 LINQ to Objects를 사용하면 개체 형식이 <xref:System.Collections.IEnumerable> 또는 <xref:System.Collections.Generic.IEnumerable%601>를 구현하는 경우 개발자가 LINQ 쿼리를 통해 메모리 내 개체에 액세스할 수 있습니다.  LINQ 쿼리는 데이터 액세스를 위한 일반 패턴을 제공하고, 표준 `foreach` 루프에 비해 간결하고 쉽게 읽을 수 있으며, 필터링, 순서 지정 및 그룹화 기능을 제공합니다.  자세한 내용은 [LINQ to Objects](../../../ocs/visual-basic/programming-guide/concepts/linq/linq-to-objects.md)를 참조하세요.  
  
 PLINQ는 다중 코어 컴퓨터의 보다 효율적인 사용을 통해 많은 시나리오에서 더 빠른 쿼리 실행을 제공할 수 있는 LINQ to Objects의 병렬 구현을 제공합니다.  자세한 내용은 [Parallel LINQ \(PLINQ\)](../../../docs/standard/parallel-programming/parallel-linq-plinq.md)를 참조하세요.  

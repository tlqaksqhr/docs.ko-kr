...
관리 코드와 비관리 코드를 함께 사용하여 예외를 처리합니다. 메서드가 관리 코드에서 예외를 throw하면 공용 언어 런타임이 HRESULT를 COM 개체에 전달할 수 있습니다. 메서드가 비관리 코드에서 오류 HRESULT를 반환함으로써 실패하면 런타임에서는 관리 코드가 catch할 수 있는 예외를 throw합니다.  
 ...
 HRESULT가 사용자 지정 결과이거나 런타임에 인식되지 않으면 런타임에서는 제네릭 <xref:System.Runtime.InteropServices.COMException>을 클라이언트에 전달합니다. **ErrorCode** 의 속성은 **COMException** HRESULT 값을 포함 합니다.  
...
## <a name="working-with-ierrorinfo"></a>IErrorInfo 작업  
 ...
 비관리 코드에서 메서드가 실패하면 관리 코드 세그먼트에 예외가 전달될 수 있습니다. 항목 [HRESULT 및 예외](../../../docs/framework/interop/how-to-map-hresults-and-exceptions.md) HRESULT 런타임 예외 개체에 매핑하는 방법을 보여 주는 표가 포함 되어 있습니다.  

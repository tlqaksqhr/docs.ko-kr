...
description: "기존 응용 프로그램을 Azure 클라우드 및 컨테이너로 리프트 앤 시프트하는 방법 알아보기"
...
이 책은 <http://dot.net/architecture>와 같은 Microsoft의 여러 채널을 통해 제공되는 전자책의 형태로 무료로 볼 수 있습니다.
...
.NET Framework 응용 프로그램은 2001년 후반에 출시된 .NET Framework 버전 1.0으로 처음 시작되었습니다. 그런 다음 회사에서 최신 버전을 출시했습니다(예: 2.0, 3.5 및 .NET 4.x). 이러한 응용 프로그램은 대부분 Windows Server 및 Internet Information Server(IIS)에서 실행되었으며, SQL Server, Oracle, MySQL 또는 다른 RDBMS 같은 관계형 데이터베이스를 사용했습니다.
...
다음은 마이그레이션할 기존의 여러 .NET Framework 응용 프로그램에 이상적인 시나리오로, 클라우드 DevOps 지원 응용 프로그램으로 마이그레이션하여 적은 노력으로 큰 이점을 얻을 수 있습니다. 이 접근법은 이후 단계인 클라우드 최적화에 준비할 수 있게 해줍니다. 그림 1-6에 그 예가 나와 있습니다.
...
이 가이드는 특정 부분에 중점을 둡니다. 재설계와 코드 변경 없이 기존 .NET 응용 프로그램의 리프트 앤 시프트를 달성하는 데 사용할 수 있는 경로를 보여 줍니다. 최종적으로 클라우드 DevOps 지원 응용 프로그램을만드는 방법을 보여 줍니다.

이 가이드에서는 마이크로 서비스 아키텍처로 진화하는 방법처럼 클라우드 전용 응용 프로그램을 사용하는 방법은 보여 주지 않습니다. 응용 프로그램을 다시 설계하거나 마이크로 서비스에 기반을 둔 새로운 응용 프로그램을 만들려면 [컨테이너화된 .NET 응용 프로그램을 위한 아키텍처](https://aka.ms/microservicesebook) 전자책을 참조하세요.
...
이 가이드는 응용 프로그램 배송 및 출시 민첩성 향상을 위해 .NET Framework에 기반을 둔 기존 ASP.NET 응용 프로그램을 현대화하려는 개발자와 솔루션 설계자를 대상으로 작성되었습니다.

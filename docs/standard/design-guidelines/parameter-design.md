 ...
 **✓ CONSIDER** 는 적은 수의 요소 배열을 전달 하는 최종 사용자가 예상 되는 경우에 params 키워드 배열 매개 변수를 추가 합니다. 예상 하지만 다양 한 요소 전달할 공통 시나리오, 사용자가 이러한 요소를 인라인으로 누르고, 통과 하지 못할는 및에 params 키워드 이므로 필요 하지 않습니다.  

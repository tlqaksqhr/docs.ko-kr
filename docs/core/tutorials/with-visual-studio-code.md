...
2. C# 프로젝트 초기화
    * <kbd>CTRL</kbd>+<kbd>\` </kbd>(백틱 기호)을 입력하여 Visual Studio Code에서 통합 터미널을 엽니다. 또는 주 메뉴에서 **보기** > **통합 터미널**을 선택해도 됩니다.
...
3. 빌드 자산 확인
    ...
    * **.NET Core 1.1**의 경우 `dotnet restore`를 입력합니다. `dotnet restore`를 실행하면 프로젝트를 빌드하는 데 필요한 필수 .NET Core 패키지에 액세스할 수 있습니다.

   ![dotnet restore 명령](media/with-visual-studio-code/dotnetrestore.png)

    * **.NET Core 2.0**의 경우 이 단계는 선택 사항입니다. 새 프로젝트를 만들 때는 `dotnet restore` 명령이 자동으로 실행됩니다.
...
2. Visual Studio Code는 앱을 빌드하고 디버그하기 위해 누락된 자산을 추가하라는 메시지를 표시합니다. **예**를 선택합니다. 

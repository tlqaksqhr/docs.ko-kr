...
*MathService.Tests* 디렉터리를 현재 디렉터리로 만들고 [`dotnet new nunit -lang F#`](../tools/dotnet-new.md)을 사용하여 새 프로젝트를 만듭니다. 그러면 NUnit를 테스트 프레임워크로 사용하는 테스트 프로젝트가 만들어집니다. 생성된 템플릿은 *MathServiceTests.fsproj*에 Test Runner를 구성합니다.

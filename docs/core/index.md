...
[.NET 표준](../standard/net-standard.md)은 개발자가 각 .NET 구현에서 예상할 수 있는 일관된 .NET API 집합을 설명하는 API 사양입니다. .NET 표준 라이브러리를 규격으로 간주하고 .NET 표준 라이브러리를 대상으로 하는 라이브러리를 지원하려면 .NET 구현에서 이 사양을 구현해야 합니다. 

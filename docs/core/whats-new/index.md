...
이전 버전의 .NET Core에서 [dotnet new](../tools/dotnet-new.md) 명령을 사용하여 새 프로젝트를 만든 이후뿐만 아니라 프로젝트에 새 종속성을 추가할 때마다 [dotnet restore](../tools/dotnet-restore.md) 명령을 실행하여 즉시 종속성을 다운로드합니다. .NET Core 2.0에서 `dotnet new` 명령을 실행할 때 `dotnet restore`은 암시적으로 실행됩니다. 또한 `run`, `build`, 및 `publish` 명령과 같은 다른 명령을 실행할 때 종속성을 업데이트해야 할 경우에도 암시적으로 실행됩니다.

...
# .NET Core 도구의 변경 내용에 대한 대략적인 개요
...
도구 계층은 매우 간단합니다. 맨 아래 기본으로 .NET Core 명령줄 도구가 있습니다. Visual Studio 또는 Visual Studio Code와 같은 높은 수준의 다른 도구는 모두 프로젝트 빌드와 종속성 복원을 위해 CLI를 사용합니다. 다시 말하면 예를 들어 Visual Studio에서 복원 작업을 수행하려면 CLI에서 `dotnet restore` 명령을 호출합니다. 

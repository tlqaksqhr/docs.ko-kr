...
컴파일러는 기본적으로 사용 하지 않는 바인딩에 대 한 진단 발생 하지 않습니다. 이러한 메시지를 받으려면 사용 컴파일러를 호출할 때 또는 프로젝트 파일에서 1182 경고 (참조 `--warnon` 아래 [컴파일러 옵션](../compiler-options.md)).
...
표시 된 값 `mutable` 를 자동으로 승격 될 수 있습니다 `'a ref` 클로저를 캡처, 클로저의 등 작성 하는 폼을 포함 하 여 `seq` 작성기입니다. 이 경우 알림이 표시 하려는 경우 경고를 활성화 3180 컴파일러를 호출할 때 또는 프로젝트 파일에 있습니다.

...
|   |   |
|---|---|
|설명|기본적으로 직렬화 측과 역직렬화 측이 모두 동일한 CLR 형식을 공유하는 경우에만 <xref:System.Runtime.Serialization.NetDataContractSerializer?displayProperty=name>을 사용할 수 있습니다. 따라서 한 버전의 .NET Framework로 직렬화된 개체를 다른 버전으로 역직렬화할 수 있도록 보장하지 않습니다. <xref:System.Collections.Concurrent.ConcurrentDictionary%602?displayProperty=name>는 .NET Framework 4.5 이하 버전으로 직렬화하고 .NET Framework 4.5.1 이상으로 역직렬화하는 경우 제대로 역직렬화하지 않는 것으로 알려진 형식입니다.|
